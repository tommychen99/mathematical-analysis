...
bound at every $x$, while a uniform error bound of
\[
\eps/2
\]
 forces the
supremum to be at most
\[
\eps/2<\eps.
\]
...
x_n=\sqrt[n]{1/2}
\]
 gives $x_n<1$ and hence $f(x_n)=0$, but
\[
...
g_n(x)=x/n
\]
 on $[0,1]$ converges uniformly to zero because
\[
...
f_n(x)=1/(1+nx^2)
\]
 on $\R$, fixing $x=0$ gives the limit $1$.
For fixed $x\ne0$,
\[
0<f_n(x)\leq1/(nx^2)\to0.
\]

Thus the limit has two cases. The witnesses
\[
x_n=1/n
\]
 have
$f(x_n)=0$ but
\[
f_n(x_n)=1/(1+1/n)\geq1/2,
\]
 so convergence is
not uniform. On $D=\{x:|x|\geq d\}$, where $d>0$,

\[
\sup_D|f_n|=1/(1+nd^2)\to0:
\]
 the same formula converges uniformly
...
g_n(x)=x/n,
\]
 the pointwise limit is zero on $\R$.
...
N>A/\eps
\]
 for a uniform tolerance $\eps$. On $\R$, however,
...
\[
0\leq x(1-x)=\frac14-\left(x-\frac12\right)^2\leq\frac14,
\]
...
N>1/(4\eps)
\]
 guarantees output error below $\eps$ for
...
from the uniform Cauchy hypothesis with tolerance
\[
\eps/2.
\]
...
\end{theorem}
\begin{proof}
Given $\eps>0$, choose $N$ such that
\[
\abs{f_N(x)-f(x)}<\eps/3
\]
 for every $x\in D$. Continuity of $f_N$ at $a$ supplies $\delta>0$ such that $\abs{x-a}<\delta$ implies
\[
\abs{f_N(x)-f_N(a)}<\eps/3.
\]
 For such $x$,
\[
...
\end{proposition}
\begin{proof}
For the sum choose $N_1,N_2$ for the uniform errors below
\[
\eps/2;
\]
...
Choose $N_1$ with
\[
\|g_n-g\|_\infty<\eps/(2(M+1))
\]
 for $n\geq N_1$
and $N_2$ with
\[
\|f_n-f\|_\infty<\eps/(2(K+1))
\]
 for $n\geq N_2$.
...
\]
 Each graph is a continuous
triangular spike. At $0$ its value is zero; for fixed $x>0$, it is zero once

\[
n>2/x.
\]
 Thus $f_n\to0$ pointwise, with a continuous limit. Nevertheless

\[
f_n(1/n)=1,
\]
 so $\|f_n\|_\infty=1$ for every $n$. Continuity of the limit
...
Let \(f_n\in C([a,b])\), suppose \(f_n\uparrow f\) pointwise on \([a,b]\),
and suppose \(f\in C([a,b])\). Then \(f_n\to f\) uniformly. The same
conclusion holds when \(f_n\downarrow f\).
\end{theorem}
Here $f_n\uparrow f$ pointwise means that for every fixed $x$, the numerical
sequence $f_n(x)$ is increasing and converges to $f(x)$; the decreasing
notation has the analogous meaning.
\begin{proof}
Suppose the increasing case is not uniform. Then there are \(\eps>0\), integers
\(n_j\) that are strictly increasing with $n_j\to\infty$, and points \(x_j\in[a,b]\) such that
\[
f(x_j)-f_{n_j}(x_j)\geq\eps.
\]
By sequential compactness, pass to a subsequence \(x_{j_k}\to x\in[a,b]\).
For any fixed \(N\), once \(n_{j_k}\geq N\), monotonicity gives
\[
0\leq f(x_{j_k})-f_{n_{j_k}}(x_{j_k})
\leq f(x_{j_k})-f_N(x_{j_k}).
\]
Letting \(k\to\infty\) and using continuity of \(f\) and \(f_N\) shows
that the right side tends to \(f(x)-f_N(x)\). Pointwise convergence at
the fixed point \(x\) supplies \(N\) with
\(f(x)-f_N(x)<\eps/2\), contradicting the lower bound. For a decreasing
sequence apply the increasing case to \(-f_n\).
\end{proof}
...
B_2(f)(x)=(x+x^2)/2,
\]
 not $x^2$ itself. The approximating
...
theorem, so $B_n(f)(x)$ is a weighted average of sampled values
\[
f(k/n).
\]
...
\[
\sum_{k=0}^n k(k-1)w_{n,k}=n(n-1)x^2.
\]
Since $k^2=k(k-1)+k$,
...
\]
sum with the binomial theorem
(admitted elementary background; an optional induction proof is in
\cref{thm:appendix-binomial}),
and write $k^2=k(k-1)+k$ (the case $n=1$ is direct).
Expanding the square now shows that the weights concentrate near
\[
k/n=x:
\]
...
The mechanism is
\[
\boxed{\text{near samples are accurate; far samples carry little total weight}.}
\]
Indeed,
\[
|B_n(f)(x)-f(x)|
\leq\sum_{k=0}^nw_{n,k}(x)\left|f(k/n)-f(x)\right|.
\]
Split this sum according as
\[
|k/n-x|<\delta
\]
or not. The near part is at most
\[
\frac\eps2\sum_{|k/n-x|<\delta}w_{n,k}(x)
\leq\frac\eps2.
\]
On the far part, the error is at most $2M$, and therefore its contribution
is at most
\[
2M\sum_{|k/n-x|\geq\delta}w_{n,k}(x).
\]
The second-moment estimate gives
\[
\sum_{|k/n-x|\geq\delta}w_{n,k}(x)
...
Choose
\[
N>\max\{1,M/(\eps\delta^2)\}.
\]
 For every $n\geq N$,
the second contribution is at most
\[
M/(2n\delta^2)<\eps/2,
\]
...
result belongs naturally to functional analysis and is not used here; the
term \emph{Hausdorff} is defined in \cref{def:hausdorff}.
\end{remark}
...
\delta=\eps/(L+1)
\]
 proves equicontinuity at every point.
...
equicontinuous at zero: $f_n(0)=0$,
\[
f_n(1/n)=1,,\qquad 1/n\to0.
\]
...
\eps/3,
\]
 supplies $\delta>0$ working for every $h_n$.
Choose $N_1,N_2$ so that $|x_n-x|<\delta$ for $n\geq N_1$ and
$|y_n-x|<\delta$ for $n\geq N_2$. For $n\geq\max\{N_1,N_2\}$,
both $|h_n(x_n)-h_n(x)|$ and $|h_n(y_n)-h_n(x)|$ are below
\[
\eps/3.
\]
  The triangle inequality gives a contradiction.
\end{proof}
...
\qquad(k\in\N).
\]
Partition $[a,b]$ into finitely many cells of length less than $\delta/2$.
For each cell with nonempty relative interior, density supplies one $q_j$
in that interior; include an endpoint separately if needed. Every
$x\in[a,b]$ then lies within less than $\delta$ of one selected point.
List them as $q_{j_1},\ldots,q_{j_\ell}$.
At $q_{j_i}$ choose a Cauchy stage $N_i$ for tolerance
\[
\eps/3.
\]

Put $N=\max\{N_1,\ldots,N_\ell\}$. For $r,s\geq N$ the values
at every selected point differ by less than
\[
\eps/3.
\]
  For a nearby selected point $q$, the
...
f_n(x)=x/(1+nx)
\]
 on $[0,\infty)$, find the pointwise limit and an input-independent error bound. Does unboundedness of the domain prevent uniform convergence?
\end{exercise}

\begin{hexercise}{uniform-model}
On a normalized input interval $[0,1]$, an approximation adds
\[
x(1-x)/n
\]
 to the desired output. Derive a worst-case error, and choose a level guaranteeing error below $10^{-3}$ for every input.
\end{hexercise}
