...
\end{theorem}
\begin{proof}
Write \(a_n^+=\max(a_n,0)\) and \(a_n^-=\min(a_n,0)\). The positive
subseries must diverge to \(+\infty\) and the negative subseries to
\(-\infty\): if either had a finite sum, convergence of the original
series would force the other to have a finite sum, and then
...
first falls below \(L\). Repeat. Both divergent subseries guarantee that
every crossing occurs and that every term is eventually used. At an upper
crossing the overshoot is at most the last positive term; at a lower
crossing its magnitude is at most the absolute value of the last negative
term. Since \(a_n\to0\), these overshoots tend to zero. The running sums
are trapped between successive crossings and therefore converge to \(L\).
\end{proof}
