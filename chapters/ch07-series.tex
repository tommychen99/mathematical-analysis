...
the limit definition with tolerance
\[
L/2.
\]
 For every
...
\]
The upper bound compares
\[
\sum a_n
\]
 with a convergent multiple of

\[
\sum b_n;
\]
 the lower bound can be rewritten
\[
b_n<(2/L)a_n
\]
 for
...
\]
If $r=0$, the conclusion is immediate. If $0<r<1$, write
\[
r=1/(1+h)
\]
 with $h>0$. Bernoulli's inequality gives
\[
(1+h)^N\geq1+Nh,
\]
so
...
\begin{example}[Two canonical positive series]
The terms of both
\[
...
\]
tend to zero, but only the second series converges.

For the square-reciprocal series, put
\[
S_n=\sum_{k=1}^n\frac1{k^2}.
\]
We claim by induction that
\[
\boxed{S_n\leq2-\frac1n.}
\]
The base case is \(S_1=1=2-1\). If the estimate holds at \(n\), then
\[
S_{n+1}\leq2-\frac1n+\frac1{(n+1)^2}
\leq2-\frac1n+\frac1{n(n+1)}
=2-\frac1{n+1}.
\]
Thus \((S_n)\) is increasing and bounded above, so it converges by the
monotone convergence theorem for sequences. Euler's solution of the Basel
problem gives the deeper exact value
\[
\boxed{\sum_{n=1}^{\infty}\frac1{n^2}=\frac{\pi^2}{6},}
\]
which is stated here without proof and is not needed for convergence.

For the harmonic series, group the terms dyadically:
\[
1+\frac12+
\left(\frac13+\frac14\right)+
\left(\frac15+\cdots+\frac18\right)+\cdots.
\]
Each displayed block after the first is bounded below by \(1/2\): for
example,
\[
\frac13+\frac14\geq\frac14+\frac14=\frac12,
\qquad
\frac15+\cdots+\frac18\geq4\cdot\frac18=\frac12.
\]
In general, formalizing the same count gives
\[
H_{2^m}=\sum_{n=1}^{2^m}\frac1n\geq1+\frac m2\longrightarrow+\infty.
\]
Hence the harmonic series diverges. It is the first canonical example in
which \(a_n\to0\) but \(\sum a_n\) diverges; tending to zero is necessary,
not sufficient. Cauchy condensation below abstracts this same dyadic
grouping mechanism. The later rational \(p\)-series theorem recovers these
as the model cases \(p=1\) and \(p=2\).
\end{example}
...
A sequence of corrections adds
\[
1/2
\]
 millimetre initially, and each
subsequent correction adds half the preceding amount. After $N$
corrections the total, in millimetres, is
\[
s_N=\sum_{n=1}^N2^{-n}=1-2^{-N}.
\]
The geometric-series theorem proves a limiting total of
$1$ millimetre, and the unperformed corrections total $2^{-N}$
millimetres. Thus $N=10$ gives an error below
\[
1/1000
\]
 millimetre.
...
\sum_{n=1}^{\infty}2^{-n}
\]
 converges, the M-test shows that
\[
...
Put \[S=\sum_{n=1}^\infty a_n,\qquad A=\sum_{n=1}^\infty|a_n|.\]
...
\begin{theorem}[Riemann rearrangement phenomenon]
\label{thm:conditional-rearrangement}
If \(\sum a_n\) converges conditionally, then for every \(L\in\R\) there is
a rearrangement of the series that converges to \(L\).
\end{theorem}
\begin{proof}
Write $a_n^+=\max(a_n,0)$ and $a_n^-=\min(a_n,0)$.

\textbf{Stage 1: the two one-sign subseries diverge.}
The positive subseries diverges to $+\infty$ and the negative subseries to
$-\infty$. If either had a finite sum, convergence of the original series
would force the other to have a finite sum, and then
$\sum|a_n|=\sum a_n^+-\sum a_n^-$ would converge, contradicting
conditional convergence.

\textbf{Stage 2: alternate crossings.}
Starting from zero, add unused positive terms in their original order until
the running sum first exceeds $L$. Then add unused negative terms until it
first falls below $L$, and repeat. Stage~1 guarantees every crossing.

\textbf{Stage 3: no nonzero term is lost.}
If a particular positive term were never used, only finitely many earlier
positive terms could have been used, contradicting the infinitely many
upper crossings. The same argument applies to negative terms. Thus every
nonzero term eventually appears.

\textbf{Stage 4: crossing errors vanish.}
At an upper crossing the overshoot is no larger than the last positive term
used; at a lower crossing the undershoot is no larger than the absolute
value of the last negative term used. Since convergence of the original
series implies $a_n\to0$, both errors tend to zero. Every intervening
partial sum lies between the most recent upper and lower crossing values,
so squeezing gives convergence to $L$.

After arranging all nonzero terms, insert zero terms at arbitrary unused
positions; this changes neither convergence nor the sum and completes a
rearrangement of the entire original series.
\end{proof}

Thus absolute convergence gives rearrangement invariance, whereas
conditional convergence permits the sum to be changed arbitrarily.
...
\longrightarrow\text{rectangle versus triangle}
\longrightarrow\text{small missing corner}.}
\]
The partial sums of
\[
...
Put
\[
A_N=\sum_{j=0}^Na_j,\qquad B_N=\sum_{k=0}^Nb_k,
\qquad C_N=\sum_{n=0}^Nc_n.
\]
Then
\[
A_NB_N=\sum_{0\leq j,k\leq N}a_jb_k,
\qquad
C_N=\sum_{j+k\leq N}a_jb_k,
\]
and hence
\[
A_NB_N-C_N
...
\]
For $N\geq2K$, every pair $(j,k)$ in this missing corner has
\[
 j>K\quad\hbox{or}\quad k>K,
\]
because $j\leq K$ and $k\leq K$ would imply $j+k\leq2K\leq N$.
...
\]
converges exactly when $p>1$. The Integral Test says that for a positive
continuous decreasing $f$ on $[1,\infty)$, the series and improper integral
\[
\sum_{n=1}^\infty f(n),
\qquad
...
\]
The preview predicts convergence of
\[
\sum1/n^2
\]
 and divergence of
...
\sum1/\sqrt n.
\]
 At singular endpoints use the one-sided limits
\[
\lim_{c\to a^+}\int_c^b f,
\qquad
\lim_{c\to b^-}\int_a^c f.
\]
Different improper pieces must converge
...
\end{proof}
The cases \(p=1\) and \(p=2\) are precisely the harmonic and
square-reciprocal series treated before condensation.
...
Then
\[
a_n/b_n=(3+1/n)/(1+2/n^3)\to3,
\]
 so limit comparison and
...
1/\sqrt{1+1/n}\to1,
\]
 hence divergence. These conclusions use
...
1/a
\]
 and the quotient limit law.
For $v_n=\sqrt[n]{n}-1\geq0$ and $n\geq2$, the binomial theorem
(admitted elementary background; see \cref{thm:appendix-binomial}
for an optional proof) gives
\[
n=(1+v_n)^n\geq\frac{n(n-1)}2v_n^2,
\qquad
v_n\leq\sqrt{\frac{2}{n-1}}.
\] Choose
\[
N>1+2/\eps^2;
\]
...
|a_{n+1}/a_n|\to\rho<1,
\]
 choose $\rho<r<1$, for instance

\[
r=(1+\rho)/2.
\]
 The definition of the limit, with error $r-\rho>0$,
gives $N_1$ such that
\[
|a_{n+1}/a_n|<r
\]
 for every $n\geq N_1$. The theorem gives geometric
...
\[
s_{2k}\leq s_{2k+1}\leq s_1=b_1.
\]
 Thus both parity subsequences
...
Its error after $N$ terms is at most
\[
1/\sqrt{N+1};
\]
 choose
...
1/a<r<1
\]
 and $N_0$ with this ratio at most $r$
for $n\geq N_0$. Induction gives $u_{N_0+j}\leq u_{N_0}r^j$.
For any $\eps>0$, choose $J$ with
\[
r^j<\eps/u_{N_0}
\]
 for
...
a^n/n!
\]
 with $a>0$, choose $N_0$ so that

\[
a/(n+1)\leq1/2
\]
 for $n\geq N_0$ and repeat the argument.
...
\begin{remark}[Choosing a convergence test]
Once the full collection of tests is available, rational functions of $n$ usually invite
...
R=\sup E,
\]
with $R=\infty$ if $E$ is unbounded. Thus $E$ is downward closed.

Suppose first that $0<R<\infty$. If $\abs{x-a}<R$, the defining property
of the supremum supplies $t\in E$ with
\[
\abs{x-a}<t;
\]
...
z=\abs{x-a}>0.
\]
The convergent terms $|c_n|z^n$ tend to zero and are therefore bounded;
choose $M>0$ such that $|c_n|z^n\leq M$ for every $n$. Thus, for
$0\leq t<z$ and every $n$,
\[
\abs{c_n}t^n\leq M\left(\frac tz\right)^n.
\]
Geometric comparison gives $t\in E$. If $z>R$, choosing $t$ strictly
between $R$ and $z$ contradicts the definition of $R$. Hence the series
diverges outside its radius.

If $R=\infty$, then for every finite $z=|x-a|$ there is $t\in E$ with
$t>z$, so comparison gives absolute convergence everywhere. If $R=0$,
there are no points with $0<|x-a|<R$; and convergence at any $z>0$ would,
by the preceding geometric comparison, put every $t<z$ in $E$, contradicting
$\sup E=0$. These cases complete the theorem.
\end{proof}
...
\sum x^n/n
\]
 above, these show three possible endpoint
...
Choose and justify a test for each series:
\[
\sum\frac{n}{n+1},\qquad
\sum\frac{1}{n(n+2)},\qquad
\sum\frac{2n+1}{n^3+1},\qquad
\sum\frac{2^n}{n!},\qquad
\sum\left(\frac{n+2}{4n}\right)^n.
\]
Compute the sum only for the telescoping example.
\end{hexercise}

\begin{exercise}
Classify
\[
\sum\frac{(-1)^{n-1}}{n^2},\qquad
\sum\frac{(-1)^{n-1}}{\sqrt n}
\]
as absolutely convergent, conditionally convergent, or divergent. Give error bounds after $N$ terms.
\end{exercise}
...
Using only the explicitly informal calculus preview, classify
\[
\sum1/n^2,\qquad \sum1/\sqrt n
\]
 by truncated integrals. Then replace each preview argument by the independent rational-$p$ theorem. Identify what Chapter~\ref{ch:riemann} must prove to make the first method formal.
\end{hexercise}

\begin{exercise}
Find the radius and test both endpoints of
\[
\sum_{n\geq1}\frac{(x-2)^n}{n2^n},\qquad
\sum_{n\geq1}\frac{(x+1)^n}{n^2 3^n}.
\]
State the full intervals of convergence.
\end{exercise}
...
\begin{exercise}
Reconstruct the induction proving
\(S_n\leq2-1/n\) for the square-reciprocal partial sums, and use Cauchy
condensation to recover the harmonic-series conclusion from the main-text
dyadic grouping.
\end{exercise}
...
\[
\sum_{n\geq1}x^n,\qquad
\sum_{n\geq1}\frac{x^n}{n},\qquad
\sum_{n\geq1}\frac{x^n}{n^2}.
\]
