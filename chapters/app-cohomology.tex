...
and therefore factors uniquely through
\[
Z^k(N)/B^k(N).
\]
...
absent. Define
\[
K\omega=\int_0^1\alpha(t,\cdot)\,dt
\]
 coefficientwise.
...
\[
\int_0^1\partial_i a(t,x+su e_i)\,ds,
\]
 where $u$ is the increment.
...
\[
u(x)=\int_0^1\sum_i a_i(tx)x^i\,dt
\]
 satisfies $du=\omega$.
...
\int_\gamma\eta=2\pi.
\]
 An exact form would have integral zero around
...
\[
c=(2\pi)^{-1}\int_{S^1}\omega;
\]
 then $\omega-c\alpha$ has integral
zero and is exact. Finally
\[
\int_{S^1}\alpha=2\pi,
\]
 so $[\alpha]\ne0$.
...
c=\sum_j a_j\sigma_j
\]
 is a chain. Its boundary is the signed
...
sum of these maps. Set
\[
\int_c\omega=\sum_j a_j\int_{[0,1]^k}\sigma_j^*\omega.
\]
...
\begin{theorem}[Long exact sequences]\label{thm:long-exact-sequence}
...
\begin{theorem}[Five Lemma, isomorphism form]
...
\]
\par
\input{figures/mayer-vietoris-circle}
\FloatBarrier
...
\begin{theorem}[de Rham theorem]\label{thm:de-rham-comparison}
...
isomorphisms form a natural isomorphism between the two cohomology functors,
viewed as contravariant functors on smooth manifolds, equivalently as
covariant functors on \(\mathbf{Man}^{\mathrm{op}}\). This is the structural meaning
...
\[
\int_{T^2}\alpha_1\wedge\alpha_2=(2\pi)^2,
\]
