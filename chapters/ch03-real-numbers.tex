...
(x+1)/2.
\]
 For $[0,1]$ the same ceiling is also
...
\begin{remark}[A first-reading route through this chapter]
The detailed proofs in this chapter are important, but a first reading has
a more modest goal. Before continuing, understand why \(\Q\) is inadequate
for analysis and why the real numbers are characterized structurally as a
complete ordered field. ``Uniqueness of \(\R\)'' means that any two complete
ordered fields are related by the unique order-preserving field isomorphism
that fixes their canonical rational copies; it does not mean that two
concrete constructions have literally the same underlying set.

Keep the architecture of the uniqueness argument in view:
\[
\boxed{
\begin{gathered}
\text{completeness}
...
\Q\text{ dense}\\
\Longrightarrow
\text{rational lower cuts determine points}
\Longrightarrow
\text{canonical isomorphism}
\end{gathered}}
\]
The existence argument has a parallel architecture:
\[
\boxed{
\begin{gathered}
\text{rational Cauchy sequences}
\Longrightarrow
\text{equivalence classes}\\
\Longrightarrow
\text{arithmetic and order}
\Longrightarrow
\text{completeness}
\end{gathered}}
\]
Study the well-definedness, quotient, order, and completeness verifications,
but do not let difficulty reproducing every implementation detail prevent
progress to sequences and limits in Chapter~\ref{ch:sequences}. On a first
reading, it is enough to understand what the real numbers are structurally,
what uniqueness means, why an existence construction is needed, and how the
two arguments are organized.
\end{remark}
...
1=u^{-1}u<0,
\]
contrary to \(1=1^2>0\). A positive rational has a representation
\[
m/n
\]
...
\]
\end{definition}
Indeed, the large-scale condition applied to
\[
1/\varepsilon
\]
 gives
...
\begin{theorem}[Integer parts and rational density in an Archimedean field]
\label{thm:archimedean-integer-density}
Let \(F\) be an Archimedean ordered field, with its canonical copy of
\(\Q\). Then:
\begin{enumerate}
\item for every \(x\in F\), there is \(k\in\Z\) such that
\[
k\leq x<k+1;
\]
\item if \(x<y\) in \(F\), there is \(q\in\Q\) such that \(x<q<y\).
\end{enumerate}
\end{theorem}
\begin{proof}
For the first assertion, choose \(M\in\N\) with \(M>-x\). The set
\[
E=\{n\in\N:n>x+M\}
\]
is nonempty, so it has a least member \(n\). If \(n>1\), minimality gives
\(n-1\leq x+M<n\); if \(n=1\), the left inequality follows from
\(x+M>0\). Thus \(k=n-M-1\) has the required property.

For \(x<y\), choose \(N\in\N\) with \(N(y-x)>1\), and apply the first
assertion to \(Nx\). If \(k\leq Nx<k+1\), then
\[
Nx<k+1\leq Nx+1<Ny.
\]
Dividing by \(N>0\) gives \(x<(k+1)/N<y\).
\end{proof}
...
\(\varepsilon>0\), there is \(N\in\N\) with
\[
1/N<\varepsilon.
\]
...
\end{theorem}
\begin{proof}
By \cref{thm:complete-fields-archimedean}, \(F\) is Archimedean, so the
second assertion of \cref{thm:archimedean-integer-density} applies.
\end{proof}
...
q/p<y;
\]
 rational density again gives a positive
rational \(r\) with
\[
\frac{q}{p}<r<y.
\]
...
(1/n)
\]
 should represent zero, and the constant sequences $(1)$ and $(2)$
must remain different. Eventual equality would be much too restrictive:
the first two sequences never agree, although their difference
\[
2/n
\]
 becomes
...
\end{lemma}
\begin{proof}
Reflexivity and symmetry are immediate. If $q\sim r$ and $r\sim s$, then for a given $\eps>0$, both
\[
\abs{q_n-r_n}<\eps/2,\qquad \abs{r_n-s_n}<\eps/2
\]
 eventually hold. The triangle inequality gives $\abs{q_n-s_n}<\eps$, proving transitivity.
\end{proof}
...
\end{lemma}
\begin{proof}
Choose $N$ such that $m,n\geq N$ gives $\abs{q_m-q_n}<1$. Then $\abs{q_n}\leq\abs{q_N}+1$ for $n\geq N$; enlarge this rational bound to cover the finitely many earlier terms. For sums, make each difference less than
\[
\eps/2
\]
 and apply the triangle inequality. For products, first bound the potentially large factors, then make the differences small. Take a common rational bound $M\geq1$ for both sequences and use
\[
\abs{q_mr_m-q_nr_n}\leq\abs{q_m}\abs{r_m-r_n}+\abs{r_n}\abs{q_m-q_n}\leq M\bigl(\abs{r_m-r_n}+\abs{q_m-q_n}\bigr).
\]
Given $\eps>0$, choose a stage after which both differences are less than
\[
\eps/(2M).
\]
 The displayed bound is then less than $\eps$, proving Cauchyness. For the final assertion, the triangle inequality gives the sum statement. Choose a common rational bound $M\geq1$ for the four sequences. The identity
\[
...
\]
which is less than $\eps$ once both null differences are less than
\[
\eps/(2M).
\]
 Thus the product class does not depend on the representatives.
\end{proof}
...
with $|q_n|\geq\eta$. Thus there is $\eta>0$ for which $\abs{q_n}\geq\eta$ for infinitely many $n$. Choose $N$ so that
\[
\abs{q_m-q_n}<\eta/2
\]
 whenever $m,n\geq N$, and choose $k\geq N$ with $\abs{q_k}\geq\eta$. If $q_k\geq\eta$, then
\[
q_n>\eta/2
\]
 for every $n\geq N$. If $q_k\leq-\eta$, then
\[
q_n< -\eta/2
\]
 for every $n\geq N$. Take
\[
\delta=\eta/2.
\]
...
(1/n)
\]
 represents zero. We need a positive
...
\end{lemma}
\begin{proof}
If $q_n>\delta$ eventually and $q\sim r$, then
\[
\abs{q_n-r_n}<\delta/2
\]
 eventually, so
...
r_n>\delta/2
\]
 eventually. Thus positivity is well defined. Positive eventual lower bounds are closed under addition and multiplication. For a nonzero class, \cref{lem:eventual-separation} gives exactly one eventual sign. Therefore \cref{prop:positive-cone-characterization} makes the resulting relation a compatible total order, and \cref{prop:ordered-field-rules} supplies its basic rules. Constant sequences visibly preserve rational order.
\end{proof}

\begin{theorem}[The constructed field is Archimedean]
\label{thm:archimedean}
For every $x\in\R$, there is $N\in\N$ with $x<N$. Consequently, for every $\eps>0$, there is $N\in\N$ with
\[
1/N<\eps.
\]
...
\end{theorem}
\begin{proof}
Let $x=[(q_n)]$. By \cref{lem:cauchy-facts-q}, the rational sequence $(q_n)$ has a rational upper bound $M$. Choose a natural number $N>M$, using the elementary Chapter~2 fact that natural numbers are unbounded above in $\Q$. Then $N-q_n\geq N-M>0$ for all $n$, so the definition of positivity, with margin
\[
(N-M)/2,
\]
 gives $x<N$. Thus the constructed field is Archimedean in the sense of \cref{def:archimedean-field}. The reciprocal consequence follows from the equivalent small-scale form.
\end{proof}
...
\end{lemma}
\begin{proof}
The constructed field is Archimedean by \cref{thm:archimedean}; apply the
first assertion of \cref{thm:archimedean-integer-density}.
\end{proof}
...
\end{theorem}
\begin{proof}
Apply the second assertion of \cref{thm:archimedean-integer-density}, using
\cref{thm:archimedean}.
\end{proof}
...
\end{proof}
Strict bounds need not remain strict:
\[
u_n=1-1/(n+1)<1
\]
 represents $1$.
...
\]
For a fixed $n\geq N$, the real number $\iota(q_n)-x$ is represented by the sequence $(q_n-q_k)_{k\in\N}$, whose tail has absolute value less than
\[
e/2.
\]
 By \cref{lem:tail-estimates-classes},
\[
...
\end{theorem}
\begin{proof}
For each $j$, choose a rational $r_j$ within
\[
1/j
\]
 of the $j$th real
...
both
\[
1/j<\eps/2,\qquad |\iota(r_j)-x|<\eps/2.
\]
 Hence
\[
...
\]
The elementary induction inequality $2^n\geq n+1$ gives
\[
0\leq 2^{-n}\leq\frac{1}{n+1}\longrightarrow0
\]
by \cref{thm:archimedean}; hence the right side can be made arbitrarily small.
...
\]
Finally, for fixed $n$, the inequalities
\[
s_n\leq s_m\leq u_m\leq u_n
\]
 for $m\geq n$ pass to the limit: if, for example, $s_n>a$, then $s_m$ cannot lie within
\[
(s_n-a)/2
\]
 of $a$ for any $m\geq n$. Hence $s_n\leq a\leq u_n$.
...
\min\{1,a\}/2,
\]
 and is bounded above by $a+1$. Let $s=\sup S>0$.
...
than $s^2-a$, while $h<s$ keeps $s-h$ positive. Choose
\[
0<h<\min\{s,(s^2-a)/(2s)\}.
\]
...
If $d$ belongs to every interval, then
\[
\abs{c-d}\leq b_n-a_n\qquad(n\in\N).
\]
If $\abs{c-d}>0$, choose $n$ so that $b_n-a_n<\abs{c-d}$,
contradicting this inequality. Thus $c=d$, and the common point is unique.
\end{proof}
...
\[
x=m+a_1/10+a_2/10^2+\cdots
\]
 means that these finite truncations
...
x_n=1/n
\]
 is Cauchy for $d$ and is not Cauchy for $\rho$,
...
Consequently,
\[
\abs{x_1+\cdots+x_n}\leq\sum_{j=1}^n\abs{x_j}.
\]
...
\label{prop:quadratic-bernoulli}
For $u,v\in\R$,
\[
2uv\leq u^2+v^2.
\]
...
Part~I obtains the finite scalar Cauchy--Schwarz inequality as the
\(p=2\) case of H\"older in \cref{thm:finite-holder}.
Chapter~\ref{ch:topology-linear-algebra} independently develops its
geometric inner-product form and the norm comparisons
\[
\abs{x_i}\leq\norm{x}_2\leq\sqrt n\,\norm{x}_\infty
\]
...
Suppose rational approximations satisfy
\[
|x_j-r_j|<1/j.
\]
 Explain why
...
1/j
\]
 by any positive error tending to zero leaves the argument valid.
\end{hexercise}

\begin{exercise}
Give a bounded nonempty subset of $\R$ whose supremum is not a maximum,
and one whose supremum is a maximum. State the additional condition on
$\sup S$ that distinguishes the two cases. Identify where completeness
enters the assertion that every nonempty bounded-above set has a supremum,
and explain why the corresponding assertion fails in $\Q$.
\end{exercise}
