...
integrals. Thus the following definitions are $\int_a^b g(t)\,\dd t$
and $\int_a^b h(t)\,\dd t$. The curve lies in $\R^n$ but the integration
...
integral $\int_a^bF(\gamma(t))\,\dd t$.
...
\end{proof}
Thus $\int_C f\,\dd s$ is independent of orientation, but it need not
be positive: if $f=-1$ on a nonconstant curve, it equals minus the length.
In contrast,
$\int_C F\cdot\dd r$ is signed. The notation $C$ presupposes a specified
...
the integral is $\int_0^1h'(s)\,\dd s=h(1)-h(0)$.
...
and using a potential. Compute $\int_C y\,\dd s$ as well and explain how
...
Let $R>0$ and $\gamma(t)=(R\cos t,R\sin t)$ for $0\leq t\leq\pi/2$.
...
\]
Consequently $\int_\gamma F\cdot\dd r=\int_0^1 4t\,\dd t=2$.
...
therefore $\int_{\partial D}P\,\dd x$. Similarly,
\[
...
For $P=-y/2$, $Q=x/2$, one has $Q_x-P_y=1$. Thus for these regions
\[
...
is $\dd t$, yielding area $1/2$. Internal-edge cancellation is already
...
=\|a\|^2\|b\|^2-(a\cdot b)^2.
\]
If $a\ne0$, write $b=ta+h$ with $t=(a\cdot b)/\|a\|^2$ and
...
and $n=(\sigma_u\times\sigma_v)/a_\sigma$. The area factor $a_\sigma$ is
...
\input{figures/surface-area-mechanism}
...
\]
Thus $\operatorname{area}(S)=\int_0^1\int_0^1\sqrt6\,\dd v\,\dd u=\sqrt6$.
\begin{center}
...
\]
Area $\sqrt6$, scalar-weighted area $\frac{3\sqrt6}{2}$, and flux
$\frac32$ are
...
The upward flux of $(0,0,1)$ is $\int_D1=\operatorname{area}(D)$,
...
\input{figures/patch-stokes-mechanism}
...
$F=(-y/2,x/2,0)$. Its curl is $(0,0,1)$, so the flux is $1$.
Boundary work pulls back to $(-v\,\dd u+u\,\dd v)/2$; the bottom and
left edges contribute zero and the other two contribute $1/2$ each.
\end{example}
...
\input{figures/intrinsic-boundary}
...
\end{array}
\]
\input{figures/measurement-degrees}
...
Let $A$ be any alternating bilinear form and write $v=\sum_i v_i e_i$,
$w=\sum_j w_j e_j$. Expanding both slots gives
\[
\begin{aligned}
...
basis-tuple values agree. Explicitly, for $v_s=\sum_i a_{is}e_i$,
\[
(A-B)(v_1,\ldots,v_k)
...
\emph{Independence.} If $\sum_I c_Ie^I=0$, evaluate on the increasing
tuple indexed by $J$ to obtain $0=\sum_Ic_I\delta_{IJ}=c_J$.
Consequently the displayed family is a basis, and counting increasing
$k$-subsets gives dimension $\binom nk$. For $k=0$ the basis is $\{1\}$.
\end{proof}
Write an arbitrary $k$-covector as $A=\sum_I a_Ie^I$ over increasing $I$.
...
$\sum_{p+q=r}\alpha_p\wedge\beta_q$. An element supported in degree $p$
...
In coordinates this means $\omega=\sum_{i=1}^n a_i\,dx^i$ with smooth
functions $a_i$ and $\omega_p(v)=\sum_i a_i(p)v^i$.
...
$\int_\gamma P\,dx+Q\,dy$. For piecewise smooth curves apply the rule
...
$\int_{S^1}\omega_0=2\pi$. An exact form would integrate to zero
...
\]
Thus \(Df(x)\) is a \(1\)-covector. Recall from
\cref{def:coordinate-differentials} that \((df)_x=Df(x)\) is the same
...
In $\omega=\sum_I a_I(x)\,\dd x_I$, each $\dd x_I$ is a fixed
coordinate $k$-covector basis element at every point; $a_I(x)$ is a scalar
...
df=\sum_{i=1}^{n}\frac{\partial f}{\partial x_i}\,\dd x_i.
\]
At each \(x\in U\), the form \((df)_x\) is the derivative \(Df(x)\).
\end{proposition}
\begin{proof}
...
on $U$ defined by
\[
(h^*\omega)_x(v_1,\ldots,v_k)
=
\omega_{h(x)}(Dh(x)v_1,\ldots,Dh(x)v_k).
\]
Composition in each input with the linear map $Dh(x)$ preserves
...
$Dh(x)e_i$ in the target basis expresses this coefficient as a finite
...
=\det Dh(x)\,\dd x_1\wedge\cdots\wedge\dd x_n.
\]
To prove this directly, evaluate the pullback on $v_1,\ldots,v_n$.
The result is
\[
\det[Dh(x)v_1\ \cdots\ Dh(x)v_n]
=\det Dh(x)\det[v_1\ \cdots\ v_n].
\]
...
\[
D(g\circ h)(x)=Dg(h(x))Dh(x).
\]
For an arbitrary $k$-form $\omega$ on $W$, evaluate both proposed
pullbacks at $x\in U$ on $v_1,\ldots,v_k$. The left side gives
$\omega_{g(h(x))}(D(g\circ h)(x)v_1,\ldots,D(g\circ h)(x)v_k)$.
The chain rule makes this the same scalar as the right side:
\[
\omega_{g(h(x))}\bigl(
Dg(h(x))Dh(x)v_1,\ldots,
Dg(h(x))Dh(x)v_k\bigr).
\]
...
evaluating $g^*\omega$ on the vectors $Dh(x)v_i$.
For the wedge rule, its alternating finite sum has the same terms on
both sides after each argument is replaced by $Dh(x)v_i$, proving
the second identity.
\end{proof}
...
$\int_\gamma\omega_0=\int_0^{2\pi}R^2\,\dd\theta=2\pi R^2$ for one
...
operation that makes $\int_{\partial M}\omega=\int_M d\omega$ type-correct:
...
\]
Over $[0,1]^2$ its integral is $1/3-1/3=0$.
...
$f(x)\,\dd x_1\wedge\cdots\wedge\dd x_n$ corresponds to the scalar
Riemann integral $\int_Q f(x)\,\dd(x_1,\ldots,x_n)$ of Chapter~12.

For a smooth parametrized curve $\gamma:[a,b]\to\R^n$ and a smooth
$1$-form $\omega=\sum_i a_i\,\dd x_i$ near its image, this becomes
\[
\begin{aligned}
...
the inclusion, $\int_{\partial Q}\omega$ abbreviates
$\int_{\partial Q}\iota^*\omega$, interpreted face by face here.
\end{definition}
...
\]
Write \(\sigma^*\omega=g(x)\,\dd x_1\wedge\cdots\wedge\dd x_k\).  Then
\[
\psi^*(\sigma^*\omega)
=g(\psi(u))\det D\psi(u)\,
...
$\int_{V'}\psi^*\eta=\int_V\eta$. This holds both for open Euclidean sets
...
\[
\Gamma_f=\{(x,f(x)):x\in U\},\qquad
\varphi:\Gamma_f\to U,\quad\varphi(x,f(x))=x.
\]
With the subspace topology this is a homeomorphism, with inverse
\[
\varphi^{-1}:U\to\Gamma_f,\qquad x\mapsto(x,f(x)).
\]
...
u\mapsto\sqrt{1-u^2}.
\]
Its derivative is $-u/\sqrt{1-u^2}$, nonzero on $(0,1)$.
...
Give $S^2=\{x\in\R^3:\norm{x}=1\}$ its subspace topology. For
$i=1,2,3$ and $\epsilon\in\{+1,-1\}$ let
\[
U_i^\epsilon=\{x\in S^2:\epsilon x_i>0\},\qquad
...
coordinate.
\input{figures/sphere-atlas-cover}
\input{figures/sphere-chart-disk}
\FloatBarrier
...
\input{figures/sphere-transition}
\FloatBarrier
...
$S^n/(x\sim-x)$. Appendix~\ref{app:quotients},
...
$(x/s,y/s)$ and $((s-R)/r,z/r)$. These smooth formulas identify the
...
\end{example}
\input{figures/embedded-torus}
\input{figures/torus-parameter-square}
\FloatBarrier
...
On the part of the closed unit disk with $|x|<1/2$, $y>1/2$, put
\[
\varphi(x,y)=(x,\sqrt{1-x^2}-y)=(u,t).
\]
Its image is the relatively open half-space set
$|u|<1/2$, $0\leq t<\sqrt{1-u^2}-1/2$.
...
\input{figures/tangent-representatives}
...
At $p=(3/5,4/5)$ the transition derivative is $-3/4$. Hence
\[
[\varphi,1]=[\psi,-3/4],\qquad
\left.\frac{\partial}{\partial u}\right|_p
=-\frac34\left.\frac{\partial}{\partial v}\right|_p.
\]
The first graph parametrization has velocity $(1,-3/4)$; the second
has velocity $(-4/3,1)$ per unit $v$. Multiplying the latter by $-3/4$
gives $(1,-3/4)$ again. The different columns describe the same direction.
\end{example}
...
$\sum_i\lambda((\partial/\partial x^i)|_p)\,\dd x^i|_p$; these covectors
...
Specifically, $\partial/\partial x^i$ represents motion in the $x^i$
...
\end{proof}
\input{figures/map-differential}
...
that $dx^i|_p$ is dual to $(\partial/\partial x^i)|_p$.
...
is $\tfrac12 ds$ in the second, and
\[
d\theta_p([\theta,a])=a
=\tfrac12 ds_p([s,2a]).
\]
...
\]
Indeed both sides apply $\omega_\psi(h(x))$ to
$Dh(x)a_1,\ldots,Dh(x)a_k$. The right side is the already constructed
...
The loop represented by $(1/2,y)$, $0\leq y\leq1$, closes under $B$.
...
\input{figures/annulus-orientation}
\FloatBarrier
...
In a chart write $\omega=a(x)\,\dd x^1\wedge\cdots\wedge\dd x^n$.
...
\input{figures/local-global-integration}
...
smoothness. Instead use smooth weights with $\sum_\alpha\rho_\alpha=1$,
so $\omega=\sum_\alpha\rho_\alpha\omega$ and each localized form is smooth.
...
For closed $A_i$, let $x\notin\bigcup_iA_i$. Choose an open neighborhood
$N$ of $x$ meeting only $A_{i_1},\ldots,A_{i_m}$. The set
$N\setminus(A_{i_1}\cup\cdots\cup A_{i_m})$ is open, contains $x$,
...
Let $r_n=1/(4n(n+1))$ for $n\geq1$, and use the smooth function
...
$I_n=[1/n-r_n,1/n+r_n]$. These intervals avoid zero and are disjoint:
the distance between neighboring centers is $1/(n(n+1))$, larger than
$r_n+r_{n+1}$. Thus at each point at most one bump is nonzero.
Nevertheless $S(x)=\sum_{n\geq1}\chi_n(x)$ satisfies $S(0)=0$ and
$S(1/n)=1$, so it is not even continuous at zero. Every neighborhood of
...
\end{aligned}
\]
The first equality uses $\sum_\beta\sigma_\beta=1$ and linearity in a
fixed chart. The middle equality is the overlap formula just verified.
The last uses $\sum_\alpha\rho_\alpha=1$. Compact support and local
...
$(-\infty,0]$ and $e^{-1/s}$ for $s>0$. For $(x,y)\in S^1$, define
\[
...
The support of $\rho_+$ lies in $\{x\geq-1/2\}\subset U_+$, and that
of $\rho_-$ lies in $\{x\leq1/2\}\subset U_-$. Both supports are compact.
...
$\int_0^{2\pi}r_+(s)\,\dd s$. Consequently
\[
...
=\int_\Omega^{\mathrm{Riemann}} f(x)\,\dd x.
\]
\end{proposition}
\begin{proof}
...
\[
\int_{\psi_\alpha(E_\alpha)}h_\alpha(x)\,\dd x
...
is therefore $\int_\Omega h_\alpha$: the omitted difference is Jordan
...
Since $\sum_\alpha\rho_\alpha=1$ on $\Omega$,
$\sum_\alpha h_\alpha=f$. Sum the finitely many integral identities.
...
$\int_{\partial M}\omega$ always means $\int_{\partial M}\iota^*\omega$.
...
\input{figures/local-global-stokes}
...
\omega=P\,\dd x+Q\,\dd y.
\]
Its derivative is $(\partial Q/\partial x-\partial P/\partial y)
\,\dd x\wedge\dd y$. By \cref{prop:manifold-riemann-agreement}, the
...
\begin{exercise}
Compute $\int_\gamma x^2\,\dd s$ on the quarter circle of radius $R>0$.
...
Repeat with $h(s)=(s+s^2)/2$ by substitution and under reversal.
...
coordinate at $(1/\sqrt2,1/\sqrt2)$. Check the answer by differentiating
...
At $(\sqrt3/2,1/2)$ convert $[\varphi,2]$ to its $\psi$ representative
...
by $H(x,t)=(1-t)x$, but is not homeomorphic to a point for $n\geq1$.
...
Put $D^n=\{x\in\R^n:\|x\|\leq1\}$ and, for $n\geq1$,
...
$X=\bigcup_{n\geq0}X^n$ with these conditions:
\begin{enumerate}
...
\item For $n\geq1$, $S^n\cong D^n/\partial D^n$ by
...
$\chi(X)=\sum_k(-1)^k\dim H_k(X;\R)$ is an unproved preview.
...
functions $g_{ij}(p)=g_p((\partial/\partial x^i)|_p,
(\partial/\partial x^j)|_p)$ are smooth. For a smooth curve $\gamma$,
write $\dot\gamma(t)=d\gamma_t(\partial/\partial t)$. Its speed and length are
\[
\norm{\dot\gamma(t)}_g=
...
$H^k_{\mathrm{dR}}(M)=\ker d/\operatorname{im}d$, with the maps in
...
on $C([0,1])$, $\int_0^1|f|$ is another norm, but the continuous functions
$f_j(x)=\max(1-jx,0)$ have maximum norm $1$ and integral norm $1/(2j)$.
...
$1$, so finitely many open balls of radius $1/3$ cannot cover the closed unit ball.
...
\]
Its expectation, when $\int_\Omega|X|\,d\mathbb P<\infty$, is the integral
\[
