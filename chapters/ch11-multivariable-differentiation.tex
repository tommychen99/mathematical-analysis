...
In Part~I, differentiability at \(a\) meant
\[
...
In one variable, the hypothesis \(f'(a)\ne0\) says that the linear
approximation \(h\mapsto f'(a)h\) can be inverted.  For a map
\[
\R^n\longrightarrow\R^n,
\]
the exact replacement is that \(Df(a)\) is an invertible linear map.  The
inverse function theorem makes this local linear picture into a genuine
local inverse.
