...
standard multivariable limit methods, and convergence of families of
vector-valued maps. We use the Euclidean default $\|\mathbf{x}\|=\|\mathbf{x}\|_2$ established
...
\[
f(\mathbf{x})\to b\quad\Longleftrightarrow\quad f_j(\mathbf{x})\to b_j
\quad(j=1,\ldots,m),
\]
...
$f(\mathbf{x}_k)\to b$ for every sequence
$\mathbf{x}_k\in D\setminus\{a\}$ tending to $a$.

\paragraph{Start with continuity and algebra.}
Try substitution before estimating. For example,

\[
(x^2+y)/(1+x^2+y^2)\to2/3
\]
 as $(x,y)\to(1,1)$, by the algebra of
...
A particularly useful target is $|f(\mathbf{x})-L|\leq C\|\mathbf{x}-a\|^\alpha$ with
$C>0$ and $\alpha>0$. Then
\[
\delta=(\eps/C)^{1/\alpha}
\]
 works.
...
f(x,y)=xy/(x^2+y^2),
\]
 both axes give zero, but $y=x$ gives
\[
1/2
\]
...
\[
(1/k,1/k),\qquad (1/k,-1/k)
\]
 with different limiting outputs, so the
...
Here $\mathbf{x}_k\to a$ varies the input of one fixed function. In $f_k\to f$,
...
\[
\begin{array}{ll}
\text{pointwise:}&\forall \mathbf{x}\in D\ \forall\eps>0\ \exists N=N(\mathbf{x},\eps)
\ \forall k\geq N:\ \|f_k(\mathbf{x})-f(\mathbf{x})\|<\eps,\\[2pt]
\text{uniform:}&\forall\eps>0\ \exists N=N(\eps)\ \forall \mathbf{x}\in D
\ \forall k\geq N:\ \|f_k(\mathbf{x})-f(\mathbf{x})\|<\eps.
\end{array}
\]
...
$E_k(\mathbf{x})=\|f_k(\mathbf{x})-f(\mathbf{x})\|$. Uniform convergence is exactly
$\sup_{\mathbf{x}\in D}E_k(\mathbf{x})\to0$. One may compute this supremum or bound it
by a number $a_k\to0$ independent of the input. To disprove uniform
convergence, find $\eps_0>0$ and moving inputs $\mathbf{x}_k\in D$ with
$E_k(\mathbf{x}_k)\geq\eps_0$ for arbitrarily large $k$.
For $x^k$, choose
\[
(2^{-1/k},0).
\]
 Its first coordinate is less than $1$,
so its limit-function value is zero and its error is exactly
\[
1/2.
\]
...
f_k(x,y)=(x/k,y/k)
\]
 on $[-1,1]^2$, the pointwise limit is zero and
\[
...
An estimate such as
\[
E_k(x,y)\leq(x^2+y^2)/k
\]
 gives an index depending
...
\|f-f_k\|_\infty<\eps/3,
\]
 then choose $\delta$ by continuity of this
fixed $f_k$ at $a$. For $\mathbf{x}\in D$ within $\delta$ of $a$,
\[
\|f(\mathbf{x})-f(a)\|\leq\|f(\mathbf{x})-f_k(\mathbf{x})\|+
\|f_k(\mathbf{x})-f_k(a)\|+\|f_k(a)-f(a)\|<\eps.
\]
\end{proof}
Uniform convergence does not preserve differentiability. The smooth maps

\[
f_k(x,y)=\sqrt{x^2+1/k}
\]
 satisfy

\[
0\leq f_k(x,y)-|x|\leq1/\sqrt k
\]
...
\[
\mathbf{x}\in K,\ \|\mathbf{x}-a\|<\delta\ \Longrightarrow\
\|g(\mathbf{x})-g(a)\|<\eps\quad\text{for every }g\in\mathcal F.
\]
...
\end{lemma}
\begin{proof}
Otherwise choose $\mathbf{x}_j,y_j\in K$ and $g_j\in\mathcal F$ with

\[
\|\mathbf{x}_j-y_j\|<1/j
\]
 and $\|g_j(\mathbf{x}_j)-g_j(y_j)\|\geq\eps_0>0$.
Pass to a subsequence $\mathbf{x}_j\to a\in K$; then $y_j\to a$ too.
Equicontinuity at this one $a$ makes both
$\|g_j(\mathbf{x}_j)-g_j(a)\|$ and $\|g_j(y_j)-g_j(a)\|$ less than

\[
\eps_0/2
\]
 eventually, contradicting the triangle inequality.
\end{proof}
...
with $\sup_{j,\mathbf{x}}\|f_j(\mathbf{x})\|<\infty$ has a uniformly convergent subsequence.
...
cover of $K$ by radius-
\[
1/r
\]
 balls centered in $K$. The union of their
...
Given $\eps>0$, choose the uniform equicontinuity scale for
\[
\eps/3.
\]
...
late $f_{k_p},f_{k_q}$ differ by less than
\[
\eps/3.
\]
 For any $\mathbf{x}\in K$
choose one such point $d$ and write
\[
\|f_{k_p}(\mathbf{x})-f_{k_q}(\mathbf{x})\|\leq
\|f_{k_p}(\mathbf{x})-f_{k_p}(d)\|+
\|f_{k_p}(d)-f_{k_q}(d)\|+
\|f_{k_q}(d)-f_{k_q}(\mathbf{x})\|<\eps.
\]
...
-Dg(a)/c^2.
\]
 Applying the already proved product rule now gives the quotient.
...
h=\sum_j h_je_j,
\]
 linearity now explains matrix multiplication:
\[
...
$\R^*$. Thus $(df)_x=f'(x)\,\dd x$. The notation
\[
df/dx=f'(x)
\]
 says that
...
v=\nabla f(a)/\|\nabla f(a)\|.
\]
 Thus it points toward greatest
...
\|h\|_2<r/\sqrt n,
\]
 every point $q$ on one of these segments satisfies
\[
...
of linear maps. This extends the one-variable terminology of
\cref{def:one-variable-ck}: ordinary derivatives are replaced by Fr\'echet
derivatives between finite-dimensional spaces.
\end{definition}

Here $Df:U\to\mathcal L(\R^n,\R^m)$ is itself a function, with
linear maps as values. Its continuity at $a$ means
$\|Df(\mathbf{x})-Df(a)\|_{\mathrm{op}}\to0$.
...
-1/(2\sqrt2)
\]
 for $t>0$ and has the opposite sign for
...
xy/(x^2+y^2)
\]
 showed that partials alone need not
...
the values on $e_1,e_2,e_1+e_2$ are
\[
1,0,1/2.
\]
...
zero; hence $v\mapsto D_vh(0)$ is the zero linear functional. But
\[
h(x,x^2)=x/2
\]
 for $x\ne0$, and
...
|h(x,x^2)|/\|(x,x^2)\|\to1/2.
\]
 Total differentiability requires
...
[f(a+tv)-f(a)]/t\to Lv
\]
 for each fixed unit $v$, with the allowed size
...
For a Euclidean input the preceding notation specializes to
\[
Df(\mathbf{x})h,\qquad \nabla f(\mathbf{x}),\qquad
D^2f(\mathbf{x})[h,h],\qquad h=\mathbf{x}-a.
\]
The increment $h$ and base point $a$ keep their established notation.
...
$[\mathbf{x},y]\subset U$, there is $t\in(0,1)$ such that
\[
f(y)-f(\mathbf{x})=Df(\mathbf{x}+t(y-\mathbf{x}))(y-\mathbf{x}).
\]
\end{theorem}
\begin{proof}
Apply the ordinary mean value theorem to $g(t)=f(\mathbf{x}+t(y-\mathbf{x}))$ on $[0,1]$;
the chain rule gives $g'(t)=Df(\mathbf{x}+t(y-\mathbf{x}))(y-\mathbf{x})$.
\end{proof}
...
the segment from $\mathbf{x}$ to $y$ lies in $U$.  If
\[
\norm{Df((1-t)\mathbf{x}+ty)}_{\mathrm{op}}\leq M
\qquad(0\leq t\leq1),
\]
then
\[
\norm{f(y)-f(\mathbf{x})}\leq M\norm{y-\mathbf{x}}.
\]
\end{theorem}
\begin{proof}
If $f(y)\ne f(\mathbf{x})$, put
\[
u=\frac{f(y)-f(\mathbf{x})}{\norm{f(y)-f(\mathbf{x})}}
\]
The choice of this unit vector is made so that
\[
u\cdot(f(y)-f(\mathbf{x}))
=\frac{\|f(y)-f(\mathbf{x})\|^2}{\|f(y)-f(\mathbf{x})\|}
=\|f(y)-f(\mathbf{x})\|.
\]
...
\[
t\longmapsto u\cdot f((1-t)\mathbf{x}+ty).
\]
For some $c\in(0,1)$,
\[
\norm{f(y)-f(\mathbf{x})}
=
u\cdot Df((1-c)\mathbf{x}+cy)(y-\mathbf{x}).
\]
...
If $U$ is convex and $\|Df(\mathbf{x})\|_{\mathrm{op}}\leq M$ on $U$, then
$f$ is $M$-Lipschitz on $U$. Every $C^1$ map is locally Lipschitz.
\end{corollary}
\begin{proof}
Convexity keeps the segment between any $\mathbf{x},y\in U$ in $U$, so
\cref{thm:multivariable-mvt} applies. For a $C^1$ map, continuity of
$Df$ at any $a$ supplies a ball in $U$ where
$\|Df(\mathbf{x})\|_{\mathrm{op}}\leq\|Df(a)\|_{\mathrm{op}}+1$. Apply the first assertion
...
from $a$ to $\mathbf{x}\in\overline B(a,r)$ stays in the ball. The mean-value estimate gives
\[
\|f_k(\mathbf{x})-f_l(\mathbf{x})\|\leq\|f_k(a)-f_l(a)\|
...
Fix $\mathbf{x}\in B(a,r)$ and put $\eta=r-\|\mathbf{x}-a\|>0$. If $\|h\|<\eta$, then
$\|\mathbf{x}+th-a\|<r$ for $0\leq t\leq1$, so the segment stays in the ball. Apply
the mean-value estimate to $z\mapsto f_k(z)-A(\mathbf{x})z$ to obtain
\[
\|f_k(\mathbf{x}+h)-f_k(\mathbf{x})-A(\mathbf{x})h\|
\leq\sup_{0\leq t\leq1}\|Df_k(\mathbf{x}+th)-A(\mathbf{x})\|\,\|h\|.
\]
Set
\[
...
\[
\sup_{0\leq t\leq1}\|Df_k(\mathbf{x}+th)-A(\mathbf{x})\|_{\mathrm{op}}
\leq e_k+\sup_{0\leq t\leq1}\|A(\mathbf{x}+th)-A(\mathbf{x})\|_{\mathrm{op}}.
\]
For fixed $h$, letting $k\to\infty$ therefore gives
\[
\|f(\mathbf{x}+h)-f(\mathbf{x})-A(\mathbf{x})h\|
\leq\sup_{0\leq t\leq1}\|A(\mathbf{x}+th)-A(\mathbf{x})\|_{\mathrm{op}}\,\|h\|.
\]
Now let $h\to0$. Since $\|th\|\leq\|h\|$, continuity of $A$ at $\mathbf{x}$
makes the remaining supremum tend to zero. Hence $Df(\mathbf{x})=A(\mathbf{x})$.
\end{proof}
...
continuity and differentiation of that array use the equivalent finite-dimensional norms.  Extending the convention of
\cref{def:one-variable-ck}, the map is of class $C^k$ when all Fr\'echet
derivatives through order $k$ exist and are continuous.
\end{definition}
...
$\xi',\eta'$,
\[
\Delta/(st)=\partial_i\partial_j
f(a+\xi'e_i+\eta'e_j).
\]
 Both points tend to $a$ as $s,t\to0^+$.
...
\paragraph{Distinguishing the second-order objects.}
...
\end{corollary}
\begin{proof}
For a nonzero remainder vector $R$, fix
\[
u=R/\|R\|
\]
 and apply the
...
M\|h\|^3/6
\]
 there. With only $C^2$ information the justified conclusion
...
$\alpha_i$ times, the same monomial $h^\alpha$ occurs
\[
j!/\alpha!
\]
...
series in the coordinates of $h=\mathbf{x}-a$. It is real analytic on its domain
...
where
\[
C_{\beta,r,R}=r^{-q}\sup_{m\geq0}m^q(r/R)^m<\infty.
\]
...
at zero is the zero series, although $f(\mathbf{x})>0$ whenever $x_1>0$.
...
\[
Dg(f(\mathbf{x}))=[Df(\mathbf{x})]^{-1},\qquad
...
\ \Longrightarrow\ (\mathbf{x}\mapsto[Df(\mathbf{x})]^{-1})\in C^{k-1}.
\]
...
Here $[Df(\mathbf{x})]^{-1}$ means inversion of a linear map, not differentiation
...
enough that the absolute remainder is at most
\[
c\|h\|^2/4.
\]
 Then
...
\end{tikzpicture}
\end{center}
More generally, for $a,b>0$ the ellipse
\[
x^2/a^2+y^2/b^2=1
\]
 has
...
(2x/a^2,2y/b^2)
\]
 at $(x,y)$. At $(x_0,y_0)$ its tangent line is
...
(2x/a^2,-2y/b^2)
\]
 everywhere, although it has two
...
\[
f((1-t)\mathbf{x}+ty)\leq(1-t)f(\mathbf{x})+tf(y)\qquad(\mathbf{x},y\in U,\ 0\leq t\leq1).
\]
...
\[
f(y)\geq f(\mathbf{x})+Df(\mathbf{x})(y-\mathbf{x})\qquad(\mathbf{x},y\in U).
\]
For $C^2$ functions it is equivalent to
$D^2f(\mathbf{x})[h,h]\geq0$ for all $\mathbf{x}\in U$, $h\in\R^n$;
in coordinates, the Hessian is positive semidefinite everywhere.
\end{proposition}
\begin{proof}
Restrict to $g(t)=f(\mathbf{x}+t(y-\mathbf{x}))$. Convexity gives

\[
(g(t)-g(0))/t\leq g(1)-g(0)
\]
 for $0<t<1$; let $t\downarrow0$
to obtain the supporting inequality. Conversely put $z=(1-t)\mathbf{x}+ty$,
apply that inequality based at $z$ to $\mathbf{x}$ and $y$, and multiply by
$1-t$ and $t$. The derivative terms sum to
$Df(z)((1-t)\mathbf{x}+ty-z)=0$, proving convexity.
For $C^2$ functions the restriction has
$g''(t)=D^2f(\mathbf{x}+t(y-\mathbf{x}))[y-\mathbf{x},y-\mathbf{x}]$; the one-variable second-derivative
criterion proves sufficiency. Necessity follows by restricting a convex
$f$ to a small line interval $\mathbf{x}+th$ within the open domain and testing at zero.
\end{proof}
...
\norm p\leq\frac{\norm{T(0)}}{1-q}<r.
\]
For every $\mathbf{x}_0\in B_2(0,r)$, the iteration $\mathbf{x}_{j+1}=T(\mathbf{x}_j)$ converges to
$p$.
\end{corollary}
\begin{proof}
Put
...
(1-q)\norm p\leq\norm{T(0)}
\]
gives the asserted bound. Given an arbitrary $\mathbf{x}_0$ in the open ball,
choose instead $s$ with $\max\{c,\norm{\mathbf{x}_0}\}<s<r$. The same invariant
...
\]
 has
\[
q=1/2,\qquad |T(0)|=(1-q)r=1/2,
\]
 but its only fixed point is the boundary point $1$.
...
X=[1/2,1]
\]
 with the usual metric, $T(x)=\cos x$ maps $X$ into itself.
Indeed, the elementary-function theory gives

\[
1/2<\cos1\leq\cos x<1
\]
 there.  The Mean Value Theorem and
...
the Cauchy limit in that same space. For
\[
T(x)=x/2
\]
 on $(0,1)$,
...
There are two related questions. An inverse problem solves $y=f(\mathbf{x})$ for
$\mathbf{x}$ as a function of $y$. An implicit problem solves
$f(\mathbf{x},y)=0$ for one block of variables, say $y$, in terms of
the independent Euclidean block $\mathbf{x}$.
...
For
\[
\|h\|_{\mathrm{op}}<1/(2\|A^{-1}\|_{\mathrm{op}}),
\]
 the preceding
...
The correction map $T_y(\mathbf{x})=\mathbf{x}-Df(a)^{-1}(f(\mathbf{x})-y)$ comes from solving
the affine model. If $f(\mathbf{x})=f(a)+B(\mathbf{x}-a)$ with $B$ invertible, then
\[
T_y(\mathbf{x})=a+B^{-1}(y-f(a)),
\]
independent of $\mathbf{x}$ and exactly the solution of $f(\mathbf{x})=y$. For a nonlinear
...
\]
be $C^k$, $1\leq k\leq\infty$. If $\mathbf{x}_0\in U$ and $Df_0(\mathbf{x}_0)$ is
invertible, then there are open neighborhoods $U_0$ of $\mathbf{x}_0$ and $V_0$
of $f_0(\mathbf{x}_0)$ such that
\[
f_0:U_0\longrightarrow V_0
\]
is a $C^k$ diffeomorphism, and
\[
D(f_0^{-1})(f_0(\mathbf{x}))=(Df_0(\mathbf{x}))^{-1}
\qquad(\mathbf{x}\in U_0).
\]
\end{theorem}
\begin{proof}
...
A=Df_0(\mathbf{x}_0).
\]
For $a\in\R^n$, let $\tau_a(\mathbf{x})=\mathbf{x}+a$. Directly,
\[
\tau_a(\mathbf{x}+h)-\tau_a(\mathbf{x})-Ih
=(\mathbf{x}+h+a)-(\mathbf{x}+a)-h=0.
\]
Hence $D\tau_a(\mathbf{x})=I$ at every $\mathbf{x}$, with zero remainder. The derivative map
is constant, so $D^j\tau_a=0$ for $j\geq2$; thus $\tau_a\in C^\infty$.
Its inverse is $\tau_{-a}$, making $\tau_a$ a global smooth diffeomorphism.

Define
\[
g_0(h)=f_0(\mathbf{x}_0+h)-f_0(\mathbf{x}_0).
\]
Then
\[
g_0=\tau_{-f_0(\mathbf{x}_0)}\circ f_0\circ\tau_{\mathbf{x}_0}.
\]
Consequently $g_0$ is $C^k$ on the open set
$\widetilde U=U-\mathbf{x}_0$, and the chain rule gives
\[
Dg_0(h)=Df_0(\mathbf{x}_0+h),
\qquad Dg_0(0)=Df_0(\mathbf{x}_0)=A.
\]
...
=A^{-1}\bigl(f_0(\mathbf{x}_0+h)-f_0(\mathbf{x}_0)\bigr).
\]
The chain rule yields
\[
D\widetilde f(h)=A^{-1}Df_0(\mathbf{x}_0+h),
\qquad
\widetilde f(0)=0,
\qquad
D\widetilde f(0)=I.
...
\[
g_y(\mathbf{x})=\mathbf{x}-f(\mathbf{x})+y.
\]
Then
\[
f(\mathbf{x})=y\quad\Longleftrightarrow\quad g_y(\mathbf{x})=\mathbf{x}.
\]
Set
\[
g_0(\mathbf{x})=\mathbf{x}-f(\mathbf{x}).
\]
Thus $g_y(\mathbf{x})=g_0(\mathbf{x})+y$ and
\[
Dg_0(\mathbf{x})=I-Df(\mathbf{x}).
\]
...
$0$ means
\[
\|Df(\mathbf{x})-Df(0)\|_{\mathrm{op}}\longrightarrow0.
\]
Since $Df(0)=I$, use
\[
\eps=1/2
\]
 in this continuity statement. There is
$\delta>0$ such that
\[
\|\mathbf{x}\|<\delta
\quad\Longrightarrow\quad
\|Df(\mathbf{x})-I\|_{\mathrm{op}}<\frac12.
\]
...
\qquad
\|Df(\mathbf{x})-I\|_{\mathrm{op}}<\frac12
\quad(\mathbf{x}\in\overline B(0,r)).
\]
By \cref{thm:operator-norm-characterizations},
\[
\|(Df(\mathbf{x})-I)v\|
\leq\|Df(\mathbf{x})-I\|_{\mathrm{op}}\|v\|
\leq\frac12\|v\|
\qquad(v\in\R^n).
\]
Since $Dg_0(\mathbf{x})=I-Df(\mathbf{x})$,
\[
\|Dg_0(\mathbf{x})\|_{\mathrm{op}}<\frac12
\qquad(\mathbf{x}\in\overline B(0,r)).
\]
If $\mathbf{x}_1,\mathbf{x}_2\in\overline B(0,r)$ and $0\leq t\leq1$, then
\[
\|(1-t)\mathbf{x}_1+t\mathbf{x}_2\|
\leq(1-t)\|\mathbf{x}_1\|+t\|\mathbf{x}_2\|
\leq r.
\]
...
\[
\|g_0(\mathbf{x}_1)-g_0(\mathbf{x}_2)\|
\leq\frac12\|\mathbf{x}_1-\mathbf{x}_2\|.
\]
...
\[
\|g_y(\mathbf{x}_1)-g_y(\mathbf{x}_2)\|
\leq\frac12\|\mathbf{x}_1-\mathbf{x}_2\|.
\]
...
V_0=B(0,r/2).
\]
Because $g_0(0)=0$, for $\mathbf{x}\in\overline B(0,r)$ and $y\in V_0$,
\[
\begin{aligned}
\|g_y(\mathbf{x})\|
&\leq\|g_0(\mathbf{x})-g_0(0)\|+\|y\|\\
&\leq\frac12\|\mathbf{x}\|+\|y\|\\
&<\frac r2+\frac r2=r.
\end{aligned}
\]
Therefore
...
fixed point $\mathbf{x}\in\overline B(0,r)$. The strict self-map estimate places that
fixed point in $B(0,r)$. Hence each $y\in V_0$ has exactly one solution of
$f(\mathbf{x})=y$ inside $B(0,r)$.
...
Since $f(\mathbf{x})=\mathbf{x}-g_0(\mathbf{x})$, for $\mathbf{x},z\in U_0$,
\[
\begin{aligned}
\|f(\mathbf{x})-f(z)\|
&=\|(\mathbf{x}-z)-(g_0(\mathbf{x})-g_0(z))\|\\
&\geq\|\mathbf{x}-z\|-\|g_0(\mathbf{x})-g_0(z)\|\\
&\geq\frac12\|\mathbf{x}-z\|.
\end{aligned}
\]
...
For every $\mathbf{x}\in\overline B(0,r)$,
\[
\|I-Df(\mathbf{x})\|_{\mathrm{op}}<\frac12<1.
\]
Apply \cref{prop:perturbation} with the
invertible map $A=I$ and $B=Df(\mathbf{x})$. It follows that $Df(\mathbf{x})$ is invertible
throughout the closed ball.

Let $g=f^{-1}$. Fix $y=f(\mathbf{x})\in V_0$ and, for small $h$ with
$y+h\in V_0$, set
\[
\Delta \mathbf{x}=g(y+h)-g(y).
\]
Since $f(\mathbf{x}+\Delta \mathbf{x})-f(\mathbf{x})=h$, differentiability of $f$ at $\mathbf{x}$ gives
\[
h=Df(\mathbf{x})\Delta \mathbf{x}+R(\Delta \mathbf{x}),
\qquad R(\Delta \mathbf{x})=o(\|\Delta \mathbf{x}\|).
\]
The inverse Lipschitz estimate yields
\[
\|\Delta \mathbf{x}\|\leq2\|h\|,
\]
so $\Delta \mathbf{x}=O(\|h\|)$ and in particular $\Delta \mathbf{x}\to0$ as $h\to0$.
For $h\ne0$, injectivity of $g$ gives $\Delta \mathbf{x}\ne0$, and
\[
\frac{\|R(\Delta \mathbf{x})\|}{\|h\|}
=
\frac{\|R(\Delta \mathbf{x})\|}{\|\Delta \mathbf{x}\|}
\frac{\|\Delta \mathbf{x}\|}{\|h\|}
\longrightarrow0.
\]
Thus $R(\Delta \mathbf{x})=o(\|h\|)$. Multiplication by the fixed bounded linear
map $Df(\mathbf{x})^{-1}$ preserves this estimate, because
\[
\|Df(\mathbf{x})^{-1}R(\Delta \mathbf{x})\|
\leq\|Df(\mathbf{x})^{-1}\|_{\mathrm{op}}\|R(\Delta \mathbf{x})\|.
\]
Solving the remainder equation gives
\[
\Delta \mathbf{x}=Df(\mathbf{x})^{-1}h+o(\|h\|),
\]
and therefore
\[
Dg(y)=Df(\mathbf{x})^{-1}.
...
Return to the original map $f_0$, the point $\mathbf{x}_0$, and
$A=Df_0(\mathbf{x}_0)$. The normalized relation was
\[
f(h)=A^{-1}\bigl(f_0(\mathbf{x}_0+h)-f_0(\mathbf{x}_0)\bigr).
\]
Thus the normalized bijection $f:U_0\to V_0$ corresponds to
\[
f_0:\mathbf{x}_0+U_0\longrightarrow f_0(\mathbf{x}_0)+AV_0.
\]
The two displayed sets are open neighborhoods of $\mathbf{x}_0$ and $f_0(\mathbf{x}_0)$.
With $g=f^{-1}$, the inverse in the original coordinates is
\[
f_0^{-1}(y)=\mathbf{x}_0+g\bigl(A^{-1}(y-f_0(\mathbf{x}_0))\bigr).
\]
These affine coordinate changes preserve the inverse identities and the
$C^k$ regularity. Moreover,
\[
Df(h)=A^{-1}Df_0(\mathbf{x}_0+h),
\]
and hence
\[
[Df(h)]^{-1}
=[Df_0(\mathbf{x}_0+h)]^{-1}A.
\]
The chain rule for the displayed formula for $f_0^{-1}$ gives
\[
\begin{aligned}
D(f_0^{-1})(f_0(\mathbf{x}_0+h))
&=[Df(h)]^{-1}A^{-1}\\
&=[Df_0(\mathbf{x}_0+h)]^{-1}AA^{-1}\\
&=[Df_0(\mathbf{x}_0+h)]^{-1}.
\end{aligned}
\]
Writing $\mathbf{x}=\mathbf{x}_0+h$ proves
$D(f_0^{-1})(f_0(\mathbf{x}))=(Df_0(\mathbf{x}))^{-1}$ on the original neighborhood.
...
does not make a global claim; on the curve
\[
uv=1/4
\]
 the derivative is
...
Here $\mathbf{x},y$ are blocks of variables and $f(\mathbf{x},y)=0$ means $m$ scalar
...
The auxiliary map
\[
h:U\to\R^{n+m},\qquad h(\mathbf{x},y)=(\mathbf{x},f(\mathbf{x},y))
\]
...
T(\mathbf{x},y)=0&f(\mathbf{x},y)=0\\
T_y\text{ invertible}&D_yf(a,b)\text{ invertible}\\
y=-T_y^{-1}T_x\mathbf{x}&y=\varphi(\mathbf{x})\\
-T_y^{-1}T_x&-(D_yf(\mathbf{x},\varphi(\mathbf{x})))^{-1}D_xf(\mathbf{x},\varphi(\mathbf{x})).
\end{array}
\]
...
\[
\varphi(a)=b,\qquad f(\mathbf{x},\varphi(\mathbf{x}))=0,
\]
and
\[
\{(\mathbf{x},y)\in A\times B:f(\mathbf{x},y)=0\}
=\{(\mathbf{x},\varphi(\mathbf{x})):\mathbf{x}\in A\}.
\]
Moreover,
\[
D\varphi(\mathbf{x})
=-\bigl(D_yf(\mathbf{x},\varphi(\mathbf{x}))\bigr)^{-1}
  D_xf(\mathbf{x},\varphi(\mathbf{x})).
\]
\end{theorem}
\begin{proof}
...
$\R^n\times\{0\}$. Define
\[
h(\mathbf{x},y)=(\mathbf{x},f(\mathbf{x},y)).
\]
It keeps the independent variable $\mathbf{x}$ and replaces $y$ by the residual
$f(\mathbf{x},y)$, so $f(\mathbf{x},y)=0$ exactly when $h(\mathbf{x},y)=(\mathbf{x},0)$.
\par\medskip
\Needspace{12\baselineskip}
...
\[
\varphi_0(\mathbf{x})=\pi_2\bigl(h^{-1}(\mathbf{x},0)\bigr)
\qquad(\mathbf{x}\in A_0).
\]
If $h^{-1}(\mathbf{x},0)=(\mathbf{x}',y')$, then $h(\mathbf{x}',y')=(\mathbf{x},0)$, and the first component
of $h$ forces $\mathbf{x}'=\mathbf{x}$. Hence
\[
h^{-1}(\mathbf{x},0)=(\mathbf{x},\varphi_0(\mathbf{x})),\qquad
f(\mathbf{x},\varphi_0(\mathbf{x}))=0.
\]
...
$\varphi=\varphi_0|_A$. If $(\mathbf{x},y)\in A\times B$ and $f(\mathbf{x},y)=0$, then
\[
h(\mathbf{x},y)=(\mathbf{x},0)=h(\mathbf{x},\varphi(\mathbf{x})).
\]
Both points lie in $W$, so injectivity of $h|_W$ gives $y=\varphi(\mathbf{x})$.
...
$h^{-1}\circ h$
shows that $Dh(\mathbf{x},y)$ is invertible at every $(\mathbf{x},y)\in W$. At a graph
point, if $D_yf(\mathbf{x},\varphi(\mathbf{x}))v=0$, then
\[
Dh(\mathbf{x},\varphi(\mathbf{x}))(0,v)=(0,0).
\]
Thus $v=0$. The endomorphism
$D_yf(\mathbf{x},\varphi(\mathbf{x})):\R^m\to\R^m$ is injective and hence, in finite
dimension, invertible.

The graph function $\varphi$ has now been constructed. Differentiating
$f(\mathbf{x},\varphi(\mathbf{x}))=0$ as an identity of maps from $\R^n$ to $\R^m$ gives
\[
D_xf(\mathbf{x},\varphi(\mathbf{x}))
+D_yf(\mathbf{x},\varphi(\mathbf{x}))D\varphi(\mathbf{x})=0.
\]
Here
\[
...
D\varphi:\R^n\to\R^m.
\]
Multiplication on the left by $D_yf(\mathbf{x},\varphi(\mathbf{x}))^{-1}$ proves
\[
D\varphi(\mathbf{x})
=-D_yf(\mathbf{x},\varphi(\mathbf{x}))^{-1}D_xf(\mathbf{x},\varphi(\mathbf{x})).
\]
\end{proof}
...
\[
0\leq c_j\leq\binom{2j}{j}\leq 2^{2j}=4^j.
\]
 Thus, for
\[
|z|<1/2,
...
first $r$ rows and columns form an invertible submatrix. Write $\mathbf{x}=(s,t)$,
...
The remaining coordinates are $\mathbf{x}\in\R^{n-m}$, and $D_yf(a)$ is invertible.
Apply the implicit theorem to $f(\mathbf{x},y)-c$ to obtain $y=\phi(\mathbf{x})$ locally.
...
$(\mathbf{x},y)=\gamma(t)$ become, in higher dimensions, a graph $y=\varphi(\mathbf{x})$,
a level set $f(\mathbf{x})=c$, and a parametrization $\mathbf{x}=\sigma(u)$.
...
\end{remark}
\input{figures/implicit-surface}
\input{figures/conics-quadrics}
The auxiliary map already used in the proof,
$h(\mathbf{x},y)=(\mathbf{x},f(\mathbf{x},y))$, is a local diffeomorphism when $D_yf(a)$ is invertible.
In coordinates $(\mathbf{x},z)=h(\mathbf{x},y)$, the equation
$f(h^{-1}(\mathbf{x},z))=z$ says that $f$ becomes projection onto the second block.
...
$\mathbf{x}\in V$; its intersection with $V$ has open image under that local
...
f=-1/2
\]
 at the two points with $y=-x$.  These are respectively the
...
\[
q(\mathbf{x})=(x_1,\ldots,x_{i-1},p(\mathbf{x}),x_{i+1},\ldots,x_n).
\]
...
$q(\mathbf{x}',t)=(\mathbf{x}',p(\mathbf{x}',t))$. Continuity keeps $\partial_t p$ of one sign
on a sufficiently small open product box $B$ around $a$. For fixed $\mathbf{x}'$,
the mean value theorem makes $t\mapsto p(\mathbf{x}',t)$ strictly monotone.
If $q(\mathbf{x}',t)=q(y',s)$, the unchanged coordinates give $\mathbf{x}'=y'$, and
...
\[
q_j(\mathbf{x})=(x_1,\ldots,x_{j-1},g_{j,j}(\mathbf{x}),x_{j+1},\ldots,x_n).
\]
...
\]
For $y=q_j(\mathbf{x})$, the first $j-1$ components of $g_j(\mathbf{x})$ are
$x_l=y_l$, and its $j$th component is $g_{j,j}(\mathbf{x})=y_j$. Consequently
\[
...
\[
b_j(\mathbf{x})=s\left(\frac{4r_j^2-\|\mathbf{x}-a_j\|^2}{3r_j^2}\right),
\qquad \chi(\mathbf{x})=1-\prod_j(1-b_j(\mathbf{x})).
\]
...
\arctan(y/x)
\]
 alone selects the wrong quadrant.
\end{exercise}
\begin{exercise}
For
\[
...
\]
 extended by zero, decide which of continuity,
...
point. Find the tangent line and a unit normal to
\[
x^2/4+y^2=1
\]
 at
...
y=\pm x^{3/2},
\]
 $x\geq0$, are $C^1$ up to the origin.
\end{exercise}
...
T(x)=\cos x/2
\]
 on $[0,1]$. Check self-map, contraction,
