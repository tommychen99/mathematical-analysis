...
logical roles and gives practical instruction in induction before turning to
...
and
\[
n\in A\Longrightarrow n+1\in A\qquad(n\in\N),
\]
then \(A=\N\).
\end{enumerate}
...
\]
In particular, \(n<k\) implies \(\binom nk=0\). This convention simplifies
...
$f=\bigcup_{g\in\mathcal C}g$ is a function: two members that assign a value
...
partial choice function $f:J\to\bigcup_iX_i$. If $J\ne I$, choose one
...
\(\mathcal C\), let \(U=\bigcup_{E\in\mathcal C}E\). Every finite collection
