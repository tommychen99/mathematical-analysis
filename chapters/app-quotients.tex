...
The practical idea is to specify which points represent the same point,
then determine continuity and local coordinates from that specification.
...
can be tested. The quotient tools are reusable structural constructions,
...
subspace (scalar multiplication by $1/2$ does not preserve it).
\end{example}

\section{Practical Quotient-Space Tools}\label{sec:quotient-tools}
...
$\pi^{-1}(\pi(A))=\bigcup_g g(A)$ is open.
...
$[0,1]/(0\sim1)\cong S^1$. Joining endpoints is therefore a precise
...
\]
Continuity at zero follows from $\sin(\pi r)/r\to\pi$.
...
$r=\arccos(t)/\pi\in(0,1)$ and $x=ru/\sin(\pi r)$. Since
...
a continuous bijection $\widetilde f:D^n/\partial D^n\to S^n$.
...
\input{figures/quotient-torus}
\FloatBarrier

\paragraph{Coordinate conventions.}
The linear rescaling $(\theta,\varphi)\mapsto
(\theta/(2\pi),\varphi/(2\pi))$ and its inverse respect the respective
...
less than $1/2$ project to quotient neighborhoods. In particular the
...
Hausdorff, so it is a homeomorphism. Rescaling $x$ by $1/2$ identifies the
...
one otherwise. A disk of radius $\rho<1/4$ therefore has disjoint translates.
...
There is a natural homeomorphism $\mathbb{RP}^n\cong S^n/(x\sim-x)$.
\end{proposition}
\begin{proof}
Let $\pi_S:S^n\to S^n/{\pm1}$ be the antipodal projection and let
$N(x)=x/\|x\|$. The continuous composite $\pi_S\circ N$ is unchanged
by nonzero scaling, including negative scaling, so it induces a continuous
map $\widetilde N:\mathbb{RP}^n\to S^n/{\pm1}$.
...
\end{proof}
\input{figures/projective-sphere}
\FloatBarrier
...
ratio map $\gamma_i(x)=(x_j/x_i)_{j\ne i}$. The restriction of $\pi$
...
the ratios of $x$ returns $x/x_i$, representing the same line as $x$.
...
continuous map $S^n\to S^n/{\pm1}\cong\mathbb{RP}^n$ is onto and $S^n$
...
$D^2/\partial D^2\cong S^2$ collapses the entire boundary to one point.
\end{example}
\input{figures/projective-disk}
\FloatBarrier
...
For $\sigma=-1$, write the seam chart centered at $[0,1/2]$ using a disk
of radius $1/8$. State its two finite-rectangle domains and its inverse.
