...
difference is horizontal. Thus
\[
F^2/R\cong F
\]
 by these specific maps.
...
$(v,w)\in V\times W$. Write such a family as
\[
\sum c_{v,w}[v,w].
\]
...
\emph{Spanning.} Write
\[
v=\sum_i x_ie_i,\qquad w=\sum_j y_jf_j.
\]

Bilinearity expands
\[
v\otimes w=\sum_{i,j}x_iy_j(e_i\otimes f_j).
\]
...
Then set
\[
\Lambda^2V=(V\otimes V)/R_2
\]
 and let $u\wedge v$ denote the
...
\sum_Jc_Je_J=0,
\]
 fix $I$ and apply this constructed map:

\[
0=\widetilde A_I\Bigl(\sum_Jc_Je_J\Bigr)=\sum_Jc_J\widetilde A_I(e_J)=c_I.
\]
All coefficients vanish. There are
\[
\binom nk
\]
 increasing index sets;
...
\]
 for $1\leq j\leq k$. The linear maps just constructed give the full coordinate expansion
\[
...
\end{proof}
For $k=1$ this is the usual basis-image formula
\[
T(e_j)=\sum_i a_{ij}f_i.
\]
...
Write
\[
v_i=\sum_j a_{ij}s_{ij}
\]
 with $s_{ij}\in S_i$. Expanding each
slot gives for either map the finite sum over index tuples with coefficient

\[
\prod_i a_{ij_i}
\]
 and value at $(s_{1j_1},\ldots,s_{rj_r})$.
...
\[
\sum_{p+q=k}\alpha_p\wedge\beta_q.
\]
 This finite sum is well defined
...
F^2/R,
\]
 including characteristic two. Construct an
