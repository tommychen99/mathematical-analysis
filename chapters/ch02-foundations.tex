...
Sections A.1--A.2 of Appendix~\ref{app:foundations} complement this chapter
with induction, recursion, factorials, binomial coefficients, Pascal's
identity, and the finite binomial theorem. They also explain why induction
and well-ordering of $\N$ are equivalent. Elementary finite algebraic
identities such as the binomial theorem are admitted background in the main
text; the appendix supplies optional proofs by induction.
...
numbers natural. Its induction property says that a statement true at $1$, and inherited from each number to its successor, is true for every
...
\]
\[
x\leq y\text{ and }y\leq x\Longrightarrow x=y,,\qquad x\leq y\text{ and }y\leq z\Longrightarrow x\leq z.
\]
...
Two different specializations of binary relations will recur:
...
\label{def:function-image}
A function $f:A\to B$ assigns exactly one $f(a)\in B$ to every $a\in A$.
The set \(A\) is its \textbf{domain}, \(B\) is its \textbf{codomain}, and
its \textbf{range} is the subset \(f(A)\subseteq B\). The codomain is part
of the declared function and need not equal its range.
Its graph
\[
...
\end{theorem}
\begin{proof}
For each $b\in B$, surjectivity supplies at least one $a\in A$ with
$f(a)=b$. If $a'$ is another, injectivity gives $a'=a$. Thus there is
exactly one such preimage; define $g(b)$ to be it. This defines a function
$g:B\to A$, not a choice depending on an arbitrary preimage.
By its definition $f(g(b))=b$ for every $b$. For $a\in A$, the point $a$
is a preimage of $f(a)$, and its uniqueness gives $g(f(a))=a$.
Hence $fg=\operatorname{id}_B$ and $gf=\operatorname{id}_A$.
If $h:B\to A$ also satisfies the two identities, then

\[
f(h(b))=b=f(g(b)),
\]
 so injectivity gives $h(b)=g(b)$ for every $b$.
The inverse is unique, and we denote it $f^{-1}$.
This inverse function is distinct from the inverse-image notation
$f^{-1}(E)$, which is defined even when $f$ is not bijective.
\end{proof}
...
=g\circ(f\circ f^{-1})\circ g^{-1}
=g\circ\operatorname{id}_B\circ g^{-1}=\operatorname{id}_C.
\] A two-sided inverse is unique: if $u$ and $v$ are both two-sided inverses of a function $w$, then
\[
...
\]
\[
[a,b]_{\Z}[c,d]_{\Z}=[ac+bd,ad+bc]_{\Z},,\qquad -[a,b]_{\Z}=[b,a]_{\Z}.
\]
Set
\[
0_{\Z}=[0,0]_{\Z},
...
\]
so addition is well defined. For multiplication we must avoid assuming
integer subtraction before integers have been constructed. First change
only $(a,b)$, using $a+b'=b+a'$. Multiply that equality by $c$ and by $d$:
\[
ac+b'c=bc+a'c,\qquad ad+b'd=bd+a'd.
\]
Adding these equalities in the required order gives
\[
ac+bd+a'd+b'c=ad+bc+a'c+b'd.
\]
This is precisely
$(ac+bd,ad+bc)\sim_{\Z}(a'c+b'd,a'd+b'c)$.
Next change only $(c,d)$, using $c+d'=d+c'$. Multiplication by $a'$
and $b'$ gives $a'c+a'd'=a'd+a'c'$ and
$b'c+b'd'=b'd+b'c'$. Thus
\[
a'c+b'd+a'd'+b'c'=a'd+b'c+a'c'+b'd',
\]
the equivalence between the new product and
$(a'c'+b'd',a'd'+b'c')$. Transitivity proves independence when both
representatives change. A formula on representatives was only a proposed
operation; these checks turn it into an operation on classes.
...
\]
so rational addition is well defined. For multiplication the proposed
pairs are $(ac,bd)$ and $(a'c',b'd')$. Their denominators are nonzero,
and the hypotheses give
\[
(ac)(b'd')=(ab')(cd')=(a'b)(c'd)=(a'c')(bd).
\]
This is the defining equality of their rational classes. Thus multiplication
too is independent of both representatives.

The formulas for additive and multiplicative inverses verify the
...
An infinite set may still be exhausted by a list indexed by natural numbers.
The question is whether every element appears at some finite stage, not
whether the listing ever ends. Bijections distinguish a complete listing
without repetitions from a list that misses elements.
...
\begin{corollary}\label{cor:rationals-countable}
...
\begin{theorem}[Cantor's diagonal argument---informal preview]
...
constructs the real numbers and interprets decimals through shrinking
intervals. Chapter~\ref{ch:sequences} revisits that interpretation through
limits of finite truncations. The argument is included here
to display the diagonal idea before that analytic machinery is available.
The formal result is \cref{thm:cantor-formal}; later proofs use that result.
...
A statement that each set contains an element does not itself specify
one function selecting elements from all the sets at once. Finite selections
can be made successively; an arbitrary family may have no given selection
rule. The following axiom supplies that simultaneous existence.
...
A useful mental image is an unlabelled collection of nonempty boxes: a choice function selects one item from every box at once, without requiring an algorithm for the selections.

Choice allows simultaneous selections even when no selection rule has been given. In analysis, one application is the familiar unrestricted form of the countable-union theorem: for a sequence of nonempty countable sets, it permits one to select a listing for each set, after which the proof of \cref{thm:diagonal-enumeration} applies. It also supports the standard existence of vector-space bases and, in more advanced analysis, the extension principles underlying Hahn--Banach and the existence of ultrafilters. Those later results are not prerequisites for this text.

A proof using choice may establish existence without describing an object or giving a procedure for finding one. In some foundational settings, very strong forms of choice also yield counterintuitive decompositions of sets. Accordingly, we distinguish an explicit construction from an existence proof using choice, and state the latter use when it occurs. The standard equivalents of the full axiom are Zorn's Lemma and the Well-Ordering Theorem. Their equivalence is proved in Appendix~\ref{app:foundations}, although the core development through Chapters~\ref{ch:reals}--\ref{ch:riemann} does not require those proofs.
...
\begin{exercise}
Negate each statement, retaining the order of its quantifiers:
\[
\forall x\in A\ \exists y\in B:\ f(x)=y,
\qquad
\exists y\in B\ \forall x\in A:\ f(x)=y.
\]
When $A=B=\N$ and $f(x)=x$, decide which statements are true. Explain
why interchanging the quantifiers changes the assertion.
\end{exercise}
