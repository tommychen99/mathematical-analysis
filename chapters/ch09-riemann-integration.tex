...
\]
For any nonempty interval $I\subseteq[a,b]$, its \textbf{interval oscillation} is
\[
\operatorname{osc}_I(f)=\sup_I f-\inf_I f
=\sup_{x,y\in I}|f(x)-f(y)|.
\]
The last equality follows by choosing values arbitrarily close to the
supremum and infimum; neither needs to be attained.
...
\]
For a partition of mesh less than $\delta$, each cell has oscillation at
most $\eta$. Consequently
\[
U(f,P)-L(f,P)=\sum_i\operatorname{osc}_{I_i}(f)|I_i|
\leq\eta\sum_i|I_i|=\eta(b-a)<\eps.
\]
...
\end{aligned}
\]
Thus $f+g$ is integrable. To prove additivity, place both candidate values
\[
\int_a^b(f+g)\,\dd x\qquad\text{and}\qquad
\int_a^b f\,\dd x+\int_a^b g\,\dd x
\]
in the same interval, because on every cell the infimum of the sum
is at least the sum of the infima, and its supremum is at most the sum
of the suprema:
\[
...
that coarse cell's length, and their oscillations are no larger. Hence
their combined contribution is at most the coarse Darboux gap. There are
at most $r$ crossing cells, each of length at most $|P|$ and oscillation
at most $2M$. Therefore
\[
U(f,P)-L(f,P)<\frac\eps2+2Mr|P|.
\]
...
works. For $|P|<\delta$, both $I$ and every tagged sum lie between
$L(f,P)$ and $U(f,P)$, whose gap is less than $\eps$.
...
A bounded function continuous except at finitely many points is also
integrable:
...
\eps/2.
\]
 Include the covering endpoints in a common partition.
Writing $\mathcal G$ for cells in the remaining good pieces and $\mathcal E$
for cells in the exceptional neighborhoods, the estimate has the form
\[
\begin{aligned}
U(f,P)-L(f,P)
&=\sum_{I\in\mathcal G}\operatorname{osc}_I(f)|I|
 +\sum_{I\in\mathcal E}\operatorname{osc}_I(f)|I|,\\
\sum_{I\in\mathcal E}\operatorname{osc}_I(f)|I|
&\leq2M\sum_{I\in\mathcal E}|I|\leq2M\ell.
\end{aligned}
\]
...
\eps/2.
\]
 Thus the whole gap is below $\eps$.
...
\eps/2.
\]
  On each subinterval of \(P\), the oscillation of \(f\) is at
...
\paragraph{Interval covers, without a Chapter 10 dependency.}
For $K\subseteq[a,b]$, an \emph{open cover} is a family $\mathcal U$ of
open subsets of $\R$ whose union contains $K$. A \emph{finite subcover}
is a finite selection from that family whose union still contains $K$.
Open subsets of the line were defined in Chapter~5. A set is relatively
open in $[a,b]$ if it is the intersection of $[a,b]$ with an open subset
of $\R$; relative closedness means that the relative complement is open.
These local meanings suffice below. We now prove the required cover
properties from \cref{thm:compact-interval}; the general topological
definitions and compactness equivalences are postponed to Chapter~10.
...
1/n
\]
 that is contained in no member of $\mathcal U$, and choose
...
\end{lemma}
\begin{proof}
Reduce comparisons to finite interval covers by
\cref{cor:closed-subset-finite-subcover}.
...
\sum_j\ell(G_j),
\]
 and taking the supremum proves the
first assertion. For the second, retain finitely many components whose
omitted tail has length less than
\[
\eta/2;
\]
 shorten the retained finitely
many intervals by a combined length less than
\[
\eta/2.
\]
 Their compact
...
\int g_n>2\eps
\]
 for a fixed $\eps>0$. For each $n$, integrability
...
is available on each connected component of a disconnected domain.
...
\input{figures/velocity-distance}
...
F_n=\int_a^{a+n}f
\]
 form a Cauchy sequence and converge to $L$.
Choose $N_1$ with
\[
|F_n-L|<\eps/2
\]
 for $n\geq N_1$ and fix an
...
&=\sum_{j=1}^{\infty}\operatorname{length}(I_j)<\varepsilon.
\end{aligned}
\]
...
$\delta$, then the relative
\[
\delta/2
\]
-neighborhood of every sufficiently
...
total length of the indicated intervals is less than
\[
\eta/4.
\]
  Enlarge the finitely many
closed partition intervals in question to open intervals whose total length
is less than
\[
\eta/2.
\]
  Cover the finitely many partition points by open
intervals having total length less than
\[
\eta/2.
\]
  These two finite families
...
Only the minimum measure-theoretic terminology needed for the comparison is
used here. A \emph{measurable function} has its standard measure-theoretic
meaning, and
\[
\int_E f\,\dd\mu
\]
denotes its Lebesgue integral over a measurable set \(E\subseteq\R\).
The constructions of measurable functions and the Lebesgue integral belong
to a subsequent real-analysis course.

\begin{theorem}[Lebesgue monotone convergence theorem; stated without proof]
...
\begin{theorem}[Lebesgue dominated convergence theorem; stated without proof]
...
dominator \(g(x)\), and it supplies integrability of the limit. The common
\(L^1\)-error conclusion makes the analogy visible, but the hypotheses and
...
enough to distinguish a dense null set from an interval. The Lebesgue integral
