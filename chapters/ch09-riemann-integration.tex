...
\end{proposition}
\begin{proof}
Every lower sum is at most every upper sum: refine both partitions by taking their union, then compare each subinterval. Thus the lower supremum is at most the upper infimum. If they are equal to $I$, choose partitions $P,Q$ with $U(f,P)<I+\eps/2$ and $L(f,Q)>I-\eps/2$. Their common refinement $R$ satisfies $U(f,R)\leq U(f,P)$ and $L(f,R)\geq L(f,Q)$, giving the claim. Conversely, the displayed condition forces the gap between the lower supremum and upper infimum to be at most every positive $\eps$, hence zero by \cref{lem:positive-small}.
\end{proof}
...
\sup(f+g)-\inf(f+g)
\leq
\bigl(\sup f-\inf f\bigr)+\bigl(\sup g-\inf g\bigr).
\]
Equivalently,
\[
...
\end{theorem}
\begin{proof}
For (1), insert $c$ into a partition. Upper and lower sums then split exactly into the corresponding sums on the two subintervals. Refining partitions and applying \cref{prop:darboux-criterion} proves integrability of the restrictions and the displayed equality.

For (2), the inequality
\[
\bigl\lvert\abs{u}-\abs{v}\bigr\rvert\leq\abs{u-v}
\]
shows on every partition interval that the oscillation of $\abs f$ is at most the oscillation of $f$. The Darboux criterion proves integrability of $\abs f$. Since
\[
-\abs f\leq f\leq\abs f,
\]
\cref{thm:integral-properties} gives the stated inequality. Part (3) follows from (2) and
\[
0\leq\abs f\leq M.
\]
\end{proof}
