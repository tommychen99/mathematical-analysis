...
The scalar outputs are ordered real numbers, so on every nonempty cell
$R$ a bounded $f$ has a finite infimum and supremum by completeness:
\[
\inf_{\mathbf{x}\in R}f(\mathbf{x}),\qquad
\sup_{\mathbf{x}\in R}f(\mathbf{x}).
\]
\begin{center}
\begin{tabular}{ll}
...
$(x_i-x_{i-1})(y_j-y_{j-1})$. There are four cells, each of area
\[
1/2.
\]
...
\node[below] at (0,0) {$0$}; \node[below] at (1,0) {$1$};
\node[below] at (2,0) {$2$};
\node[left] at (0,.5) {$\displaystyle 1/2$}; \node[left] at (0,1) {$1$};
\end{tikzpicture}
\end{center}
...
r=\prod_jm_j
\]
 cells. From now on, when only this finite
...
\]
We may subsequently write the same finite sums as
\[
\sum_{R\in\mathcal P}
\]
...
In scalar-integral notation we use
\[
\mathbf{x}=(x_1,\ldots,x_n),\qquad
\int_Q f(\mathbf{x})\,\dd\mathbf{x}.
\]
Here $\dd\mathbf{x}$ is shorthand for unsigned scalar coordinate-volume
integration in the variables $x_1,\ldots,x_n$. It is not an exterior
derivative applied to the vector $\mathbf{x}$. In particular, it is distinct
from the individual coordinate covectors $\dd x_i$ of
\cref{def:coordinate-differentials} and from the oriented top-degree form
\[
\dd x_1\wedge\cdots\wedge\dd x_n
\]
introduced in Chapter~13. That form retains orientation information that
scalar volume does not. We retain the useful geometric notations
\[
\iint_Qf(x,y)\,\dd A,\qquad
\iiint_Qf(x,y,z)\,\dd V,
\]
as well as explicit coordinate lists in scalar integrals.
...
For $f(x,y)=x+y$ on $[0,1]^2$, split each axis at
\[
1/2.
\]
...
(i+j)/m,\qquad (i+j+2)/m,
\]
 $0\leq i,j<m$. Finite summation gives
...
\[
\mu_f(\delta)=\sup\{|f(\mathbf{x})-f(y)|:\mathbf{x},y\in K,\ \|\mathbf{x}-y\|\leq\delta\}.
\]
...
\[
c\operatorname{vol}(R)=\sum_j c\operatorname{vol}(R_j),
\]
 so refinement
...
\[
\int_Q(f+g)\,\dd\mathbf{x}
=\int_Qf\,\dd\mathbf{x}+\int_Qg\,\dd\mathbf{x},
\qquad
\int_Qcf\,\dd\mathbf{x}
=c\int_Qf\,\dd\mathbf{x}.
\]
If
\[
f\leq g,
...
\[
\left\lvert\int_Qf\,\dd\mathbf{x}\right\rvert
\leq
\int_Q\abs f\,\dd\mathbf{x},
\]
and
\[
\left\lvert\int_Qf\,\dd\mathbf{x}\right\rvert
...
small proves integrability of the sum. Both
\[
\int_Q(f+g),\qquad \int_Qf+\int_Qg
\]
 lie in
\[
...
Write
\[
Q=Q_1\cup\cdots\cup Q_N,
\]
 with disjoint interiors and possible
...
on each $Q_i$ with gap below
\[
\eps/N
\]
 and extend all its coordinate cuts
...
For $a\in\R^n$, set $Q+a=\{\mathbf{x}+a:\mathbf{x}\in Q\}$ and $g(y)=f(y-a)$ there.
Then $g$ is integrable exactly when $f$ is, and
\[
\int_{Q+a}g=\int_Qf.
\]
...
\sum_jt_{ij}\int_Qf_j.
\]
 Continuity also passes to each component, so
...
integrations. On $Q=[a,b]\times[c,d]$,
\[
\iint_Qf\,\dd A
\]
 is defined
by two-dimensional grids, whereas
\[
...
$\mathbf{x}=(x_1,\ldots,x_p)$ and $y=(y_1,\ldots,y_q)$. With $\mathbf{x}$ fixed,
$f(\mathbf{x},\cdot):B\to\R$ is a scalar function of $q$ variables.
Its integral
\[
g(\mathbf{x})=\int_B f(\mathbf{x},y)\,\dd y
\]
 is one real number, defining
$g:A\to\R$. The integral
\[
\int_A g(\mathbf{x})\,\dd\mathbf{x}
\]
 handles the remaining
$p$ variables. In order, these operations are
\[
f(\mathbf{x},y)\ \xrightarrow{\text{integrate in }y\text{ with }\mathbf{x}\text{ fixed}}\
g(\mathbf{x})\ \xrightarrow{\text{integrate in }\mathbf{x}}\ \text{a scalar}.
\]
...
\[
\mathbf{x}\longmapsto\int_Bf(\mathbf{x},y)\,\dd y,
\qquad
y\longmapsto\int_Af(\mathbf{x},y)\,\dd\mathbf{x}
\]
are continuous, and
\[
\int_{A\times B}f(\mathbf{x},y)\,\dd(\mathbf{x},y)
=
\int_A\left(\int_Bf(\mathbf{x},y)\,\dd y\right)\dd\mathbf{x}
=
\int_B\left(\int_Af(\mathbf{x},y)\,\dd\mathbf{x}\right)\dd y.
\]
\end{theorem}
\begin{proof}
...
G(\mathbf{x})=\int_B f(\mathbf{x},y)\,\dd y.
\]
 The bounded-integral estimate gives
\[
|G(\mathbf{x}')-G(\mathbf{x})|\leq b\sup_{y\in B}|f(\mathbf{x}',y)-f(\mathbf{x},y)|.
\]
Uniform continuity of $f$ makes the supremum less than
\[
\eps/b
\]
 whenever
$\mathbf{x}'$ is sufficiently close to $\mathbf{x}$, proving continuity of $G$.
...
For $\mathbf{x}\in R$, integration over the $S$ cells gives
\[
\sum_Sm_{R,S}\operatorname{vol}(S)\leq G(\mathbf{x})
...
a section $f(\mathbf{x},\cdot):B\to\R$ need not be integrable for every $\mathbf{x}$.
Thus
\[
\mathbf{x}\mapsto\int_B f(\mathbf{x},y)\,\dd y
\]
 may not even be defined everywhere.
...
\[
\underline I_B(\mathbf{x})=\underline{\int_B}f(\mathbf{x},y)\,\dd y
 :=\sup_{\mathcal P_B}L(f(\mathbf{x},\cdot),\mathcal P_B),\qquad
\overline I_B(\mathbf{x})=\overline{\int_B}f(\mathbf{x},y)\,\dd y
 :=\inf_{\mathcal P_B}U(f(\mathbf{x},\cdot),\mathcal P_B).
\]
...
at $\mathbf{x}$ holds exactly when $f(\mathbf{x},\cdot)$ is Riemann integrable.
...
\[
\begin{aligned}
\int_A\underline I_B(\mathbf{x})\,\dd\mathbf{x}
 &=\int_A\overline I_B(\mathbf{x})\,\dd\mathbf{x}=\int_{A\times B}f,\\
...
\]
For each $\mathbf{x}\in R$, comparison with the section sums gives
\[
\sum_Sm_{R,S}\operatorname{vol}(S)
\leq\underline I_B(\mathbf{x})\leq\overline I_B(\mathbf{x})
\leq\sum_SM_{R,S}\operatorname{vol}(S).
\]
...
supremum over $\mathbf{x}\in R$, multiply by $\operatorname{vol}(R)$, and sum:
\[
...
Both
\[
\int_Ah,\qquad \int_{A\times B}f
\]
 lie between the same product
...
$f(\mathbf{x},\cdot)$ is Riemann integrable, then

\[
G(\mathbf{x})=\int_Bf(\mathbf{x},y)\,\dd y
\]
 is Riemann integrable on $A$ and
\[
\int_{A\times B}f=\int_A G(\mathbf{x})\,\dd\mathbf{x}.
\]
...
t(a/b)=1/b
\]
 for a reduced rational with $b>0$ (including
\[
0/1
\]
).
...
two-dimensional Darboux criterion gives
\[
\int_{[0,1]^2}f=0.
\]
...
absolute-integrability formulation, Fubini assumes
\[
\int|f|<\infty.
\]
...
Let $F$ be $C^1$ on a neighborhood of
\[
Q=\prod_{i=1}^n[a_i,b_i].
\]

Write $\mathbf{x}_{\widehat j}$ for the tuple omitting coordinate $j$, and
...
\]
 Then
\[
\int_Q\partial_jF(\mathbf{x})\,\dd\mathbf{x}
=\int_{Q_{\widehat j}}\bigl[F(\mathbf{x})\bigr]_{x_j=a_j}^{x_j=b_j}\,\dd\mathbf{x}_{\widehat j}.
\]
...
$\mathbf{x}_{\widehat j}=(x_1,\ldots,x_{j-1},x_{j+1},\ldots,x_n)$.
...
$\dd\mathbf{x}_{\widehat j}$ is their scalar coordinate volume. The face term
...
\qquad
\dd\mathbf{x}_{\widehat j}
=
\dd x_1\cdots\dd x_{j-1}\dd x_{j+1}\cdots\dd x_n.
\]
Then
\[
\int_Q u(\mathbf{x})\frac{\partial v}{\partial x_j}(\mathbf{x})\,\dd\mathbf{x}
=
\int_{Q_{\widehat j}}
\left[
u(\mathbf{x})v(\mathbf{x})
\right]_{x_j=a_j}^{x_j=b_j}\,\dd\mathbf{x}_{\widehat j}
-
\int_Q v(\mathbf{x})\frac{\partial u}{\partial x_j}(\mathbf{x})\,\dd\mathbf{x}.
\]
In the middle integral, the coordinates other than \(x_j\) are held fixed
while the bracket is evaluated at the two endpoints.
\end{theorem}
\begin{proof}
For fixed \(\mathbf{x}_{\widehat j}\in Q_{\widehat j}\), apply
...
dependence on \(\mathbf{x}_{\widehat j}\).  Integrating it over \(Q_{\widehat j}\) and
...
Thus the answer is
\[
\frac56-\frac14=\frac7{12}.
\]
 Direct integration of
$xy+y^2$ checks it:
\[
\frac14+\frac13=\frac7{12}.
\]
 The endpoint term has
...
\[
\operatorname{vol}(E)=\int_Q\mathbf 1_E(\mathbf{x})\,\dd\mathbf{x}.
\]
\end{definition}
...
Chapter~\ref{ch:riemann} uses Lebesgue measure-zero (null) sets,
allowing countably many covering intervals;
Jordan-nullness requires finitely many rectangles. The rationals in $[0,1]$
have Lebesgue measure zero, by \cref{prop:countable-negligible},
...
\end{lemma}
\begin{proof}
For each $\mathbf{x}\in K$ choose $r_{\mathbf{x}}>0$ with $B(\mathbf{x},2r_{\mathbf{x}})\subset G$.
Finitely many balls $B(\mathbf{x},r_{\mathbf{x}})$ cover $K$; let $d$ be the minimum of
their radii. If a cell contains $y\in K\cap B(\mathbf{x},r_{\mathbf{x}})$, then for every
point $z$ in the cell, $|z-\mathbf{x}|<d+r_{\mathbf{x}}\leq2r_{\mathbf{x}}$, so $z\in G$.
...
Conversely choose a partition whose indicator gap is less than
\[
\eps/2.
\]
...
\eps/2.
\]
 Any boundary point in the interior of a
...
of combined volume less than
\[
\eps/2.
\]
 Together these rectangles cover
...
\end{proposition}
\begin{proof}
All three sets are bounded. If $\mathbf{x}\notin\partial E\cup\partial F$, choose
one ball about $\mathbf{x}$ on which membership in $E$ is constant and another on
...
parts with total volumes below
\[
\eps/2
\]
 each. Each boundary on the left is
...
\widetilde f(\mathbf{x})=\begin{cases}f(\mathbf{x}),&\mathbf{x}\in E,\\0,&\mathbf{x}\in Q\setminus E.\end{cases}
\]
...
\]
 At every
$\mathbf{x}\in Q\setminus\partial E$, the extension is continuous: locally the
point is inside $E^\circ$ or outside $\overline E$.
Choose a relatively open neighborhood $U_{\mathbf{x}}$ in $Q$ with
$\operatorname{osc}_{U_{\mathbf{x}}}\widetilde f<\eta$, where

\[
\eta=\eps/[2(1+\operatorname{vol}Q)].
\]

The open cover consisting of $G\cap Q$ and these $U_{\mathbf{x}}$ has a Lebesgue
...
the bad collection; every remaining cell lies in some $U_{\mathbf{x}}$.
...
Their Darboux contribution is less than
\[
2M\eps/(4M)=\eps/2.
\]

All other cells contribute at most
\[
\eta\operatorname{vol}Q<\eps/2.
\]
...
combined volume less than
\[
\eps/2.
\]
 Their union also contains
...
\[
\int_Ef(\mathbf{x})\,\dd\mathbf{x}=\int_Q\widetilde f(\mathbf{x})\,\dd\mathbf{x}.
\]
\end{definition}
...
$y\mapsto g(\mathbf{x},y)$ is integrable on $B$. Then
\[
G(\mathbf{x})=\int_Bg(\mathbf{x},y)\,\dd y
\]

is integrable on $A$ and
\[
\int_A G=\int_{A\times B}g.
\]
...
\[
\int_0^1\int_y^1(x+y)\,\dd x\,\dd y=1/2.
\]
...
s/k.
\]
 Each image lies in an $m$-box of side
\[
2L\sqrt n\,s/k.
\]
...
R/\sqrt3;
\]
 the corresponding square-root graphs
...
\]
fixing $z$ leaves a disk with radius
\[
r(z)=R(1-z/h),
\]
 since
...
Integrating this quadratic profile is what produces the factor
\[
1/3:
\]
...
\draw[->] (0,1.2)--(.52,1.2);
\node[right] at (.6,1.35) {$\displaystyle R(1-z/h)$};
...
\int_{L(E)}f(y)\,\dd y
=
\int_Ef(L\mathbf{x})\abs{\det L}\,\dd\mathbf{x}.
\]
\end{theorem}
\begin{proof}
...
=
\abs{\det L}\int_Rs(\mathbf{x})\,\dd\mathbf{x}.
\]
...
=
\abs{\det L}\int_Rh(\mathbf{x})\,\dd\mathbf{x}.
\]
...
that
\[
\mathbf{x}\longmapsto\widetilde f(L\mathbf{x})
\]
has an integrable zero extension from \(L^{-1}(P)\), and
\[
\int_P\widetilde f(y)\,\dd y
=
\abs{\det L}\int_{L^{-1}(P)}\widetilde f(L\mathbf{x})\,\dd\mathbf{x}.
\]
...
4\pi abc/3.
\]
 Slicing found the ball's volume; linear scaling
...
\[
\int_V f(y)\,\dd y=\int_U f(q(\mathbf{x}))|\det Dq(\mathbf{x})|\,\dd\mathbf{x}.
\]
\end{lemma}
\begin{proof}
First suppose the changed coordinate is the last one, so
$q(\mathbf{x}',t)=(\mathbf{x}',p(\mathbf{x}',t))$ and $\det Dq=\partial_t p$.
...
Fix $\mathbf{x}'$. The source slice $U_{\mathbf{x}'}=\{t:(\mathbf{x}',t)\in U\}$ is open in
...
the mean value theorem makes $p(\mathbf{x}',\cdot)$ strictly monotone there.
Injectivity of $q$ makes the images of distinct component intervals
disjoint, and surjectivity onto $V$ makes their union $V_{\mathbf{x}'}$.
...
an interval neighborhood contained in $U_{\mathbf{x}'}$, and a finite subcover
...
\[
\begin{aligned}
s_-&=\min\{p(\mathbf{x}',\alpha),p(\mathbf{x}',\beta)\},\\
s_+&=\max\{p(\mathbf{x}',\alpha),p(\mathbf{x}',\beta)\}.
\end{aligned}
\]
One-variable substitution, \cref{thm:one-variable-substitution}, gives
\[
\int_\alpha^\beta f(\mathbf{x}',p(\mathbf{x}',t))|\partial_t p(\mathbf{x}',t)|\,\dd t
=\int_{s_-}^{s_+}f(\mathbf{x}',s)\,\dd s.
\]
...
This uses only finitely many ordinary interval integrals for each $\mathbf{x}'$,
...
over $\mathbf{x}'$. For $n=1$ there are no parameter coordinates. Any other
...
\int_W f
 ...
 &=\int_U(f\circ\tau\circ\sigma)
       |\det D\tau(\sigma(\mathbf{x}))|\,|\det D\sigma(\mathbf{x})|\,\dd\mathbf{x}.
\end{aligned}
\]
The chain rule and multiplicativity of the determinant identify the
product as $|\det D(\tau\circ\sigma)(\mathbf{x})|$.
...
=\int_U f(\sigma(\mathbf{x}))|\det D\sigma(\mathbf{x})|\,\dd\mathbf{x}.}
\]
...
where $f(y)=0$, all $f_i$ vanish. Thus
\[
\sum_i f_i=f
\]
 on $V$.
...
\[
\int_{\sigma(N_i)}f_i
=\int_{N_i}\rho_i(\mathbf{x})f(\sigma(\mathbf{x}))|\det D\sigma(\mathbf{x})|\,\dd\mathbf{x}.
\]
...
$f(\sigma(\mathbf{x}))|\det D\sigma(\mathbf{x})|$, because that function vanishes outside
...
$\sigma(E)$. Then
\[
\int_{\sigma(E)}f=\int_E f(\sigma(\mathbf{x}))|\det D\sigma(\mathbf{x})|\,\dd\mathbf{x}.
\]
\end{corollary}
\begin{proof}
...
and $\det D\sigma(\mathbf{x})\ne0$ for $\mathbf{x}\in Q$. Then $\sigma(Q)$ is Jordan
measurable and, for continuous $f$ on $\sigma(Q)$,
\[
\int_{\sigma(Q)}f(y)\,\dd y
=\int_Q f(\sigma(\mathbf{x}))|\det D\sigma(\mathbf{x})|\,\dd\mathbf{x}.
\]
\end{theorem}
\begin{proof}
There is an open neighborhood of $Q$ on which $\sigma$ is injective.
Otherwise, in neighborhoods of distance less than
\[
1/j
\]
 from $Q$
inside a fixed compact neighborhood in $U$, there would be distinct
$\mathbf{x}_j,y_j$ with $\sigma(\mathbf{x}_j)=\sigma(y_j)$. Subsequences converge to
$\mathbf{x},y\in Q$. Continuity and injectivity on $Q$ give $\mathbf{x}=y$.
...
neighborhood of $[\mathbf{x},y]$ and suppose
$\|DH(z)\|_{\infty\to\infty}\leq M$ along that segment.
Choose a coordinate $i$ with
$|H_i(y)-H_i(\mathbf{x})|=\|H(y)-H(\mathbf{x})\|_\infty$.
The scalar mean value theorem applied to
$t\mapsto H_i(\mathbf{x}+t(y-\mathbf{x}))$ on $[0,1]$ gives some $z\in[\mathbf{x},y]$ with
\[
\begin{aligned}
\|H(y)-H(\mathbf{x})\|_\infty
&=|DH_i(z)(y-\mathbf{x})|\\
&\leq\|DH(z)(y-\mathbf{x})\|_\infty
\leq M\|y-\mathbf{x}\|_\infty.
\end{aligned}
\]
For $\mathbf{x}=y$ the conclusion is immediate. In particular the estimate applies
...
\end{proposition}
\begin{proof}
Put $A=D\sigma(a)$ and $T(\mathbf{x})=a+A^{-1}(\sigma(\mathbf{x})-\sigma(a))$.
...
radius $R$, take $N=B_\infty(a,r)$ with
\[
r<R/4,
\]
 inside the source
ball, and
\[
T(N)\subset B_\infty(a,R/2).
\]
 A cube in $N$ has side
$h\leq2r$, so these target points lie in
\[
B_\infty(a,3R/4).
\]
...
\[
\|T(\mathbf{x})-T(c)-(\mathbf{x}-c)\|_\infty\leq\delta\|\mathbf{x}-c\|_\infty
\qquad(\mathbf{x}\in C).
\]
Thus $T(C)$ lies in the cube centered at $T(c)$ of side $(1+\delta)h$.
Conversely, if
\[
\|y-T(c)\|_\infty\leq h/(2(1+\delta)),
\]
 the segment
...
\cite{schwartz-change}. Its proof principle is to obtain the easy
...
bound
\[
\int_{\sigma(E)}q\leq\int_E(q\circ\sigma)J_\sigma
\]
 is known for
nonnegative continuous $q$, apply it to $\sigma^{-1}$ and
$h=(q\circ\sigma)J_\sigma$. Since
$J_{\sigma^{-1}}(\sigma(\mathbf{x}))J_\sigma(\mathbf{x})=1$, this gives the reverse inequality.
...
\[
m(\sigma(E))=\int_E |\det D\sigma(\mathbf{x})|\,\dd\mathbf{x}
\]
...
$y_j\in K$ such that, with $\mathbf{x}_j=\sigma^{-1}(y_j)$,
\[
0\leq(\mathbf{x}_j)_n\leq2\delta_j,
\qquad (y_j)_n\geq\varepsilon_0.
\]
A subsequence of $(\mathbf{x}_j)$ converges in the compact set $\sigma^{-1}(K)$ to
$\mathbf{x}$ with $x_n=0$. Continuity gives $y_j=\sigma(\mathbf{x}_j)\to\sigma(\mathbf{x})$, while boundary
preservation gives $(\sigma(\mathbf{x}))_n=0$, a contradiction. In a fixed containing
...
\int_V f(y)\,\dd y
=
\int_U f(\sigma(\mathbf{x}))\abs{\det D\sigma(\mathbf{x})}\,\dd\mathbf{x}.
\]
...
\sigma^*\omega
=
f(\sigma(\mathbf{x}))\det D\sigma(\mathbf{x})\,
\dd x_1\wedge\cdots\wedge\dd x_n.
\]
Indeed,
\[
...
so its area is at most $4\varepsilon^2$. For
\[
0<\eta<\pi/2,
\]
 the omitted
...
\input{figures/jacobian-distortion}
...
Use \cref{ex:ellipse-scaling}. For
\[
...
\]
With integrand $1$ the area is
\[
\int_0^1(1+u)\,\dd u=3/2.
\]
...
$0\leq z\leq h$, $0\leq\theta\leq2\pi$,
\[
0\leq r\leq R(1-z/h).
\]

On $r>0$, $0<\theta<2\pi$ the map is a diffeomorphism onto the slit
space. For $0<\varepsilon<R$ and
\[
0<\eta<\pi/2,
\]
 retain the compact source
\[
\begin{split}
...
\[
\varepsilon\leq R(1-z/h)
\]
 for $z$; it prevents reversed radial bounds near
...
\varphi=\arccos(z/\rho),
\]
 and the unique azimuth in that interval.
...
\end{example}
\input{figures/spherical-coordinates}
\begin{center}
...
\[
\lim_{k\to\infty}\int_Qf_k(\mathbf{x})\,\dd\mathbf{x}
=
\int_Qf(\mathbf{x})\,\dd\mathbf{x}.
\]
Moreover,
\[
\lim_{k\to\infty}\int_Q\abs{f_k(\mathbf{x})-f(\mathbf{x})}\,\dd\mathbf{x}=0.
\]
\end{theorem}
\begin{proof}
...
\[
\left\lvert\int_Qf_k\,\dd\mathbf{x}-\int_Qf\,\dd\mathbf{x}\right\rvert
\leq
\int_Q\abs{f_k-f}\,\dd\mathbf{x}
\leq
\operatorname{vol}(Q)\sup_{\mathbf{x}\in Q}\abs{f_k(\mathbf{x})-f(\mathbf{x})}.
\]
...
For (3), choose $K_i\subset G_i$ as in (2) with error
\[
c_0/2^{i+1}.
\]
...
The compact sets
\[
H_N=K_1\cap\cdots\cap K_N
\]
 decrease. If $H_N$ were
empty, nesting would imply

\[
G_N\subseteq\bigcup_{i=1}^N(G_i\setminus K_i);
\]
 (1) would give
...
c(G_N)<c_0/2,
\]
 a contradiction. Thus all $H_N$ are nonempty and
...
and suppose $f_k\to f$ pointwise and $|f_k(\mathbf{x})|\leq M$ for all $k,\mathbf{x}$.
...
The case $M=0$ is immediate. Otherwise replace $g_k$ by
\[
g_k/(2M),
\]
...
$g_k(\mathbf{x})>\eps$ throughout $U_k$. The open sets

\[
G_N=\bigcup_{k\geq N}U_k
\]
 decrease and each has content at least $\eps V$.
By \cref{lem:open-box-content}, there is a fixed
\[
\mathbf{x}\in\bigcap_NG_N.
\]

For every $N$ some $k\geq N$ then satisfies $g_k(\mathbf{x})>\eps$, contrary to
$g_k(\mathbf{x})\to0$. Undoing the height normalization proves the first limit.
Finally
\[
|\int_Q f_k-\int_Q f|\leq\int_Q|f_k-f|
\]
 proves the second.
\end{proof}
...
\[
f_k(\mathbf{x})\leq f_{k+1}(\mathbf{x})
\qquad(\mathbf{x}\in Q,\ k\in\N).
\]
...
\[
U_n=\{\mathbf{x}\in K:h_n(\mathbf{x})<\eps/2\}
\]
are increasing, and pointwise convergence gives
\[
K=\bigcup_n U_n.
\]

Compactness of $K$ supplies a finite subcover. Its largest-index member
$U_N$ already covers $K$. For $n\geq N$ and every $\mathbf{x}\in K$,

\[
0\leq h_n(\mathbf{x})<\eps/2,
\]
 hence
\[
\sup_K|f-f_n|\leq\eps/2<\eps.
\]
...
Jacobian determinant is $1$ and substitution gives
\[
\int_Ef=-\int_Ef=0.
\]
...
\[
0\leq\int_Ef\leq M\operatorname{vol}(E).
\]
 Scaling every length by
$c>0$ scales $n$-dimensional volume by $c^n$.
\end{enumerate}
For the cone, cross-sections immediately give
\[
A(z)=\pi R^2(1-z/h)^2.
\]
...
\int_E x^2.
\]
 Label a representative slice of each kind.
\item Compute the volume of $0\leq z\leq h$,
\[
x^2+y^2\leq R^2z/h
\]

by cross-sections, and independently by cylindrical coordinates.
\item Find the volume of
\[
x^2/4+y^2/9+z^2/16\leq1
\]
 by scaling.
