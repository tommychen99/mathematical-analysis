...
Part~I introduced the real-line versions of open and closed sets, relative
topology, compactness, and sequential compactness in
\cref{sec:real-line-topology,sec:real-line-compactness}. This chapter now
generalizes those notions systematically to metric and topological spaces.
It is not trying to reproduce full courses in topology and linear algebra.
It develops the parts of those subjects needed for Chapters 11--13,
...
\paragraph{Euclidean input convention.}
When the input is a full Euclidean coordinate tuple, we write
\[
\mathbf{x}=(x_1,\ldots,x_n),\qquad f(\mathbf{x}),\qquad Df(\mathbf{x}).
\]
The components $x_i$ and individual scalar coordinates retain ordinary type.
Points of an abstract metric space and established vector letters such as
$v\in V$ retain their existing notation. For a sequence of Euclidean inputs,
$\mathbf{x}_k$ denotes its $k$th point, whereas $x_i$ denotes a component.

\section{Distances, Balls, and Neighborhoods}
On $\R$, distance is $|x-y|$. In the plane the Pythagorean theorem gives
\[
d_2(\mathbf{x},y)=\sqrt{(x_1-y_1)^2+(x_2-y_2)^2}.
\]
In $\R^n$ define the \emph{Euclidean length}
\[
\|\mathbf{x}\|_2=\left(\sum_{j=1}^n x_j^2\right)^{1/2},
\qquad d_2(\mathbf{x},y)=\|\mathbf{x}-y\|_2.
\]
...
Writing
\[
\mathbf{x}\cdot y=\sum_jx_jy_j,
\qquad
\|\mathbf{x}\|_2=\sqrt{\mathbf{x}\cdot \mathbf{x}},
\]
we have
\[
|\mathbf{x}\cdot y|\leq\|\mathbf{x}\|_2\|y\|_2.
\]
Consequently $\|\cdot\|_2$ is a norm
and $d_2(\mathbf{x},y)=\|\mathbf{x}-y\|_2$ is its associated metric.
\end{theorem}
\begin{proof}
...
\[
t=(\mathbf{x}\cdot y)/\|y\|_2^2
\]
 into a sum of squares. This value is chosen
because
\[
\norm{\mathbf{x}-ty}_2^2
=\norm{\mathbf{x}}_2^2-2t(\mathbf{x}\cdot y)+t^2\norm y_2^2
\]
...
t=(\mathbf{x}\cdot y)/\norm y_2^2
\]
 (equivalently, this choice completes the
square). At its minimum,
\[
0\leq\sum_j(x_j-ty_j)^2
=\|\mathbf{x}\|_2^2-\frac{(\mathbf{x}\cdot y)^2}{\|y\|_2^2}.
\]
Multiplication by $\|y\|_2^2$ and square roots give the assertion. For
$u=\mathbf{x}-y$ and $v=y-z$, it follows that
\[
...
$\|\alpha \mathbf{x}\|_2=|\alpha|\|\mathbf{x}\|_2$. Thus Euclidean length satisfies all
three norm axioms. Applying definiteness to $\mathbf{x}-y$ and observing that
$\|\mathbf{x}-y\|_2=\|y-\mathbf{x}\|_2$ verify the remaining metric axioms for $d_2$.
\end{proof}

From now on, unless another norm is explicitly indicated, the standing
convention on Euclidean space is
\[
\boxed{\|\mathbf{x}\|:=\|\mathbf{x}\|_2\qquad(\mathbf{x}\in\R^n).}
\]
...
On $\R^n$ set $d_\infty(\mathbf{x},y)=\max_j|x_j-y_j|$. Each coordinate
distance is at most $d_\infty(\mathbf{x},z)+d_\infty(z,y)$, proving the triangle
...
\[
d_\infty(\mathbf{x},y)\leq d_2(\mathbf{x},y)\leq\sqrt n\,d_\infty(\mathbf{x},y).
\]
...
\min(x,1-x)/2
\]
 keeps the ball inside the interval. The set $[0,1)$
...
\]
 choose a member containing $x$ and a
ball inside that member. That same ball lies in the union. If

\[
x\in U_1\cap\cdots\cap U_m,
\]
 choose $r_i>0$ with $B(x,r_i)\subseteq U_i$.
...
and applying De Morgan's laws. Finiteness matters:
\[
\bigcap_{m\geq1}(-1/m,1/m)
=\{0\}
\]
 is not open in $\R$.
\end{proof}
...
\[
U=\bigcup_{a\in A}B_X(a,r_a),
\]
 which is open. Every $a\in A$ belongs
...
\[
d(x_n,x)<\eps/2,\qquad d(x_n,y)<\eps/2.
\]
 Hence
...
\eps/2
\]
 of $x$ and use the triangle inequality to obtain
$d(x_m,x_n)<\eps$.
\end{proof}
...
$\|\mathbf{x}_n-\mathbf{x}\|_2\leq\sqrt n\max_j|x_{n,j}-x_j|\to0$.
In the discrete metric $d(x,y)=1$ for $x\ne y$, a Cauchy sequence is
eventually constant by the test
\[
\eps=1/2,
\]
 so any such space is complete.
\end{example}
...
\end{theorem}
\begin{proof}
If $x\in\overline E$, each
\[
E\cap B(x,1/n)
\]
 is nonempty. Choose $x_n$
in that intersection. Then
\[
d(x_n,x)<1/n\to0.
\]
 Conversely, if $x_n\in E$
...
\[
x_n\in(E\setminus\{x\})\cap B(x,1/n);
\]
 then $x_n\to x$. Conversely,
...
$\R$ is not complete: the Cauchy sequence
\[
1-1/n
\]
 converges in $\R$ to
...
$|f(x,y)-f(a,b)|\leq3\|(x-a,y-b)\|_2$, so
\[
\delta=\eps/3
\]
 works.
...
\]
One fixed $\eps_0$ works at every scale. Set
\[
\delta=1/n
\]
 and choose
...
countable-product argument finitely many times. Given $\mathbf{x}\in U$ open,
choose $B(\mathbf{x},\varepsilon)\subseteq U$. For $n\geq1$, choose rational
$q_i$ with
\[
|q_i-x_i|<\varepsilon/(3\sqrt n)
\]
 for each $i$.
Then
\[
\|q-\mathbf{x}\|^2=\sum_i|q_i-x_i|^2<\varepsilon^2/9.
\]

Choose a rational radius satisfying
\[
\|q-\mathbf{x}\|<r<\varepsilon-\|q-\mathbf{x}\|.
\]
Density of the rationals supplies these choices.
Then $\mathbf{x}\in B(q,r)\subseteq B(\mathbf{x},\varepsilon)\subseteq U$ by the triangle
...
\end{definition}
Metric spaces are first countable: take
\[
B(x,1/n).
\]
...
a countable open local basis by
\[
...
\[
G=\bigcup_\alpha(U_\alpha\times V_\alpha).
\]
 Then
\[
...
is compact by \cref{thm:compact-closed-interval}; the general
...
\input{figures/compact-subcover}
...
Every metric space is Hausdorff: balls of radius
\[
d(x,y)/3
\]
 about distinct
...
Conversely
\[
\{a_1,\ldots,a_r\}=V(\prod_{j=1}^r(t-a_j));
\]
...
\[
K\subseteq\bigcup_{i=1}^m B(x_i,r_i)
\]
 and put $\delta=\min_i r_i>0$.
...
B(x_n,1/n)
\]
 lies in no cover member. Sequential compactness gives
...
\[
d(x_{n_j},x)<r/2,\qquad 1/n_j<r/2.
\]
 For any
\[
z\in B(x_{n_j},1/n_j),
...
with $n_0=0$, choose $n_k>n_{k-1}$ with
\[
x_{n_k}\in B(p,1/k).
\]
...
lemma, then cover the space by finitely many radius-
\[
\lambda/2
\]
 balls
...
Every point is within
\[
\lambda/2
\]
 of a center, so lies in one of these
...
\]
 for $n,m\geq N$, and choose $j$
with $n_j\geq N$ and
\[
d(x_{n_j},x)<\varepsilon/2.
\]
 For every $n\geq N$,
\[
...
subspace. The sequence
\[
1/n
\]
 escapes $(0,1)$; rational approximations to

\[
\sqrt2/2
\]
 escape $\Q\cap[0,1]$. Both are Cauchy. Finitely many balls of
...
1/2
\]
 is a singleton. Thus $D$ is bounded and complete but not
...
supremum. For each $n$ choose an image value
\[
y_n>s-1/n,
\]
 with $y_n\leq s$.
...
\[
\R^n=\bigcup_{m\geq1}B(0,m).
\]
 A finite subcover of $K$ has a largest
...
$\mathbf{x}_j=(x_j^1,x_j^2)$ in bounded $K$, Bolzano--Weierstrass gives indices
...
\[
(\mathbf{x}_j)\supset(\mathbf{x}_{j_k^{(1)}})\supset\cdots\supset(\mathbf{x}_{j_k^{(n)}}).
\]
...
\[
|x_{j_k}^i-a_i|<\varepsilon/\sqrt n
\]
 for $k\geq K_i$. The single index
$k\geq\max_{1\leq i\leq n}K_i$ then works for every coordinate, and
\[
\|\mathbf{x}_{j_k}-a\|_2\leq\sqrt n\max_i|x_{j_k}^i-a_i|<\varepsilon.
\]
...
a<\varepsilon/\sqrt n
\]
 and choose a point of the subset in each cube
...
length estimate $|\|\mathbf{x}\|_2-\|y\|_2|\leq\|\mathbf{x}-y\|_2$ makes it closed.
...
$(1-t)\mathbf{x}+ty\in C$ for $\mathbf{x},y\in C$ and $0\leq t\leq1$.
Thus convex sets have the straight-line paths $\gamma(t)=(1-t)\mathbf{x}+ty$.
\end{definition}
...
\input{figures/connected-hierarchy}
...
Let
\[
E=\{0\}\cup\{1/n:n\in\mathbb N\}\subset\R.
\]
 A connected subset
...
Yet $0$ is not isolated: every relative neighborhood contains
\[
1/n
\]
 for
...
p+1/n\to p
\]
 with distinct rational terms. Total disconnectedness
concerns connected subsets, whereas discreteness concerns open singletons.
\end{example}
\input{figures/components-accumulation}
...
and its failure are both open conditions. For $\mathbf{x}\in U$, choose
$B(\mathbf{x},r)\subseteq U$. If $\mathbf{x}\in A$, append the segment from $\mathbf{x}$ to any
$y\in B(\mathbf{x},r)$ to a path reaching $\mathbf{x}$; convexity of the ball gives
$B(\mathbf{x},r)\subseteq A$. If $\mathbf{x}\notin A$, no $y\in B(\mathbf{x},r)$ can belong to
$A$, because appending the reverse segment would reach $\mathbf{x}$. Thus
$B(\mathbf{x},r)\subseteq U\setminus A$. Both sets are open in $U$.
...
\end{corollary}
\begin{proof}
For $\mathbf{x}$ in a component $C$, a small ball $B(\mathbf{x},r)\subseteq U$ is connected.
...
b=(x-y)/2.
\]
 Thus this pair produces every vector in the plane.
...
\end{proposition}
\begin{proof}
The empty sum gives zero. If
\[
x=\sum_{i=1}^r a_iu_i,\qquad y=\sum_{j=1}^s b_jv_j
\]
 belong to the span, then

\[
cx+dy=\sum_i(ca_i)u_i+\sum_j(db_j)v_j
\]
 is another finite combination
...
v_j=-\sum_{i\ne j}(a_i/a_j)v_i.
\]
 Thus this vector lies in the span of the others.
\end{example}
...
\sum_j(x_j-y_j)v_j=0;
\]
 independence gives $x_j=y_j$ for every $j$.
...
\qquad s_{\mathcal B}:\R^n\to V,\quad \mathbf{x}\mapsto\sum_jx_jv_j
\]
...
If
\[
u=\sum_jx_jv_j,\qquad v=\sum_jy_jv_j,
\]
 then

\[
au+bv=\sum_j(ax_j+by_j)v_j.
\]
 Uniqueness of coordinates proves
$\kappa_{\mathcal B}(au+bv)=a\kappa_{\mathcal B}(u)+b\kappa_{\mathcal B}(v)$.
Distributing a finite sum proves
$s_{\mathcal B}(a\mathbf{x}+by)=as_{\mathcal B}(\mathbf{x})+bs_{\mathcal B}(y)$.
For any $\mathbf{x}\in\R^n$, the coefficients of $s_{\mathcal B}(\mathbf{x})$ are precisely
$\mathbf{x}$, so $\kappa_{\mathcal B}s_{\mathcal B}(\mathbf{x})=\mathbf{x}$.
...
e_1=(u-e_2)/2.
\]
 Thus $u,e_2$ still produces both old generators,
and every new generator was already in the old span. The spans coincide.
For example
\[
(3,4)=(3/2)u+(5/2)e_2.
\]
 In the general proof we must
...
\[
(v_1,\ldots,v_r)=\sum_i\iota_i(v_i).
\]
...
\sum_i(u_i-u_i')=0,
\]
 so each difference is zero; this proves (a).
For (b)$\Rightarrow$(c), write a point in the intersection as

\[
u_i=\sum_{j\ne i}u_j.
\]
 Move the right side to the left and use (b).
For (c)$\Rightarrow$(b), a zero sum gives

\[
u_i=-\sum_{j\ne i}u_j
\]
 in that intersection, so $u_i=0$ for each $i$.
...
\input{figures/internal-direct-sum}
\FloatBarrier
...
If
\[
\sum c_iz_i+\sum d_ju_j+\sum e_\ell w_\ell=0,
\]
 then

\[
\sum d_ju_j=-\sum c_iz_i-\sum e_\ell w_\ell
\]
 lies in $U\cap W$.
...
\end{theorem}
\begin{proof}
For the unique coordinate expansion
\[
v=\sum_i x_iv_i,
\]
 define
...
T(v)=\sum_i x_iw_i.
\]
 Uniqueness of the input coordinates makes this a
...
then
\[
L(v)=\sum_i x_iL(v_i)=\sum_i x_iw_i=T(v)
\]
 for every $v$. This proves
...
$u+vt$ is the derivative of
\[
ut+(v/2)t^2.
\]
 Thus $D$ is surjective but
...
exactly the tuples $(u_1,\ldots,u_r)$ with
\[
\sum_i u_i=0.
\]
 These are the
...
$T(0,1)=(-1,3)$ have target columns
\[
\binom21,\qquad \binom{-1}3.
\]
...
Its output column is
\[
\binom3{-2},
\]
 and the table acting on
$(2,3,-1)^{\mathsf T}$ gives
\[
2\binom00+3\binom10-\binom02=\binom3{-2}.
\]
...
$T(v_j)$: linearity then determines
\[
T(\sum_j x_jv_j).
\]
...
v=\sum_j x_jv_j,
\]
 apply linearity first and then collect coefficients:
\[
...
$\mathbf{x}=(2,-1,3)^{\mathsf T}$. By columns,
\[
...
of $A$, and therefore has entries
\[
\sum_k b_{ik}a_{kj}.
\]
 This forces:
...
\]
 Both preserve linear combinations.
...
\input{figures/coordinate-vector}
\FloatBarrier
...
Near the start of this chapter, $\|\mathbf{x}\|$ was fixed as the standard Euclidean
...
On $\R^n$ and $P_2$, respectively, use
\[
\langle \mathbf{x},y\rangle=\sum_i x_i y_i,
\qquad
...
For $\mathbf{x}=(x_1,\ldots,x_n)\in\R^n$, $n\geq1$, the standard formulas are
\[
\begin{aligned}
\|\mathbf{x}\|_1&=\sum_i|x_i| &&\text{(sum norm or $1$-norm)},\\
\|\mathbf{x}\|_2&=\left(\sum_i x_i^2\right)^{1/2}
 &&\text{(Euclidean norm or $2$-norm)},\\
\|\mathbf{x}\|_3&=\left(\sum_i|x_i|^3\right)^{1/3},\qquad
\|\mathbf{x}\|_4=\left(\sum_i|x_i|^4\right)^{1/4},\\
\|\mathbf{x}\|_p&=\left(\sum_i|x_i|^p\right)^{1/p}
 &&(1\leq p<\infty),\\
\|\mathbf{x}\|_\infty&=\max_i|x_i| &&\text{(maximum or sup norm)}.
\end{aligned}
\]
For the $1$-norm, summing $|x_i+y_i|\leq|x_i|+|y_i|$ proves the triangle
inequality. For the maximum norm, each $|x_i+y_i|$ is at most
$\|\mathbf{x}\|_\infty+\|y\|_\infty$; take the maximum. For the Euclidean norm,
...
\[
\|\mathbf{x}\|_\infty\leq\|\mathbf{x}\|_2\leq\|\mathbf{x}\|_1\leq\sqrt n\|\mathbf{x}\|_2,
\qquad \|\mathbf{x}\|_2\leq\sqrt n\|\mathbf{x}\|_\infty,
\qquad |x_i|\leq\|\mathbf{x}\|_2.
\]
A coordinate square is at most the sum of squares. Also

\[
\sum_i|x_i|^2\leq(\sum_i|x_i|)^2,
\]
 and Cauchy--Schwarz applied to
$(|x_i|)$ and $(1)$ gives the $1$-to-$2$ comparison. Finally each square
is at most $\|\mathbf{x}\|_\infty^2$; summing gives the last norm comparison.
...
This and $\|\mathbf{x}\|_\infty=\max_i|x_i|$ express the same largest-magnitude
...
is immediate. Otherwise set
\[
t=\langle u,v\rangle/\langle v,v\rangle.
\]
...
$\norm{\mathbf{x}}_1=|x_1|+|x_2|$, $\norm{\mathbf{x}}_2$, and $\norm{\mathbf{x}}_\infty$ are,
...
\node at (0,-1.9) {$\|\mathbf{x}\|_{\lab}\leq1$};
\end{scope}}
...
For any norm on $\R^n$,
\[
\norm{\mathbf{x}}_b\leq\sum_{j=1}^{n}\abs{x_j}\norm{e_j}_b
\leq C\norm{\mathbf{x}}_2
\]
...
\leq C\norm{\mathbf{x}-y}_2,
\]
so $\mathbf{x}\mapsto\norm{\mathbf{x}}_b$ is Euclidean-continuous.  It has a positive minimum
$c$ on the Euclidean
...
positive because every sphere point is nonzero. For $\mathbf{x}\ne0$, the vector

\[
u=\mathbf{x}/\norm{\mathbf{x}}_2
\]
 is on that sphere, so homogeneity gives
$\norm{\mathbf{x}}_b=\norm{\mathbf{x}}_2\norm{u}_b\geq c\norm{\mathbf{x}}_2$; the same inequality
holds at zero. Apply these bounds to each given norm, writing
$c_a\norm{\mathbf{x}}_2\leq\norm{\mathbf{x}}_a\leq C_a\norm{\mathbf{x}}_2$ and
$c_b\norm{\mathbf{x}}_2\leq\norm{\mathbf{x}}_b\leq C_b\norm{\mathbf{x}}_2$. Then
\[
\frac{c_b}{C_a}\norm{\mathbf{x}}_a\leq\norm{\mathbf{x}}_b
\leq\frac{C_b}{c_a}\norm{\mathbf{x}}_a.
\]
...
\[
B_a(\mathbf{x},\eps/C)\subseteq B_b(\mathbf{x},\eps)
\]
 and
$B_b(\mathbf{x},c\eps)\subseteq B_a(\mathbf{x},\eps)$, so the open sets agree.
Applying the same inequalities to $\mathbf{x}_j-\mathbf{x}$ and $\mathbf{x}_j-\mathbf{x}_k$ proves the
...
\|v\|_c=\sum_i|a_i|.
\]
 Norm equivalence gives $\|v\|_c\leq K\|v\|_V$.
...
proves equality. The coefficient bound
\[
...
For example $\lambda(x,y)=3x-4y$ has norm $5$, attained at
\[
(3/5,-4/5).
\]
...
\sum_{k=0}^\infty v_k
\]
 in a finite-dimensional normed space $V$
...
\[
\|\sum_{k=p}^qv_k\|\leq\sum_{k=p}^q\|v_k\|
\]
 makes partial sums Cauchy;
finite-dimensional completeness supplies their limit. For a linear map
$L:V\to W$, continuity applied to partial sums gives

\[
L(\sum_kv_k)=\sum_kL(v_k).
\]
 Scalar convergence tests need no redevelopment.
...
\[
e^i(\sum_j v^j e_j)=v^i.
\]
 Unique vector coordinates make this well defined;
...
b^2(x,y)=(x-y)/2.
\]
 Each extracts the coefficient of
...
for the usual dot product, but by
\[
(1/2,0)
\]
 for
...
\[
B(\mathbf{x},y)=x_1y_1+2x_1y_2-x_2y_1+3x_2y_2.
\]
The four basis values force this expression. Since every coordinate
magnitude is at most the Euclidean norm,
$|B(\mathbf{x},y)|\leq7\|\mathbf{x}\|_2\|y\|_2$. This is an upper bound, not a
...
\]
An upper or lower triangular matrix has determinant
\[
\prod_i a_{ii}.
\]
...
If one scales the last row by
\[
1/3
\]
 to obtain reduced form, the new
...
\input{figures/determinant-orientation}
\FloatBarrier
...
For
\[
\norm{\mathbf{x}}_1=\sum_{j=1}^{n}\abs{x_j},
\qquad
\norm{\mathbf{x}}_\infty=\max_{1\leq j\leq n}\abs{x_j},
\]
prove
\[
...
\end{exercise}
\begin{exercise}
Show $B(\mathbf{x},y)=x_1y_2+x_2y_1$ is bilinear and symmetric but not alternating.
