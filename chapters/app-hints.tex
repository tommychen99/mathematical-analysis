...
\exercisehint{representatives}{Compare a constant rational sequence with that constant plus
\[
1/n.
\]
 Equivalence concerns the limit of a difference, not equality of individual terms.}
...
\exercisehint{seq-geometric}{Use Bernoulli on
\[
4/3
\]
 for decay and on
...
3/2
\]
 for growth. For the negative base separate odd and even indices.}
...
\exercisehint{jump-continuity}{At the junction compare the sequences
\[
1/n,\qquad -1/n
\]
 and the assigned value. Away from the junction choose a neighborhood lying in one branch.}
...
\exercisehint{derivative-definition}{Keep
\[
|h|<|a|/2
\]
 in the reciprocal quotient at $a\ne0$. For the piecewise function compare the two one-sided quotients.}
...
\exercisehint{integral-spikes}{Place a triangle on
\[
[1/(2n),1/n]
\]
 and choose its height to keep its area fixed. Check the value at zero separately.}
