...
has entries $\sum_k a_{ik}b_{kj}$, and inversion has entries given by
...
$n^2-1$, $n(n-1)/2$, and $n(n-1)/2$, respectively, for $n\geq1$.
\end{proposition}
\begin{proof}
...
\]
where $\operatorname{tr}H=\sum_i h_{ii}$. Indeed the linear terms choose
...
\]
At determinant $1$, choosing $H=A/n$ makes this derivative $1$.
...
$Xf=\sum_i X^i\partial_i f$, so $Xf$ is smooth and obeys the product rule.
\begin{definition}
