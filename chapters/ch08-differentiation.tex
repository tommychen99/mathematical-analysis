...
\end{theorem}
\begin{proof}
At a local maximum, the difference quotient is nonnegative for nearby
\(x<c\) and nonpositive for nearby \(x>c\). Since its two-sided limit
exists, that limit is zero. At a local minimum the signs are reversed.
\end{proof}
...
Let \(c\) be an interior critical point of \(f\), and suppose \(f\) is
differentiable on each side of \(c\).
\begin{enumerate}
\item If \(f'>0\) immediately to the left of \(c\) and \(f'<0\)
immediately to the right, then \(c\) is a strict local maximum.
\item If \(f'<0\) immediately to the left and \(f'>0\) immediately to the
right, then \(c\) is a strict local minimum.
\item If \(f'\) has the same sign on both sides, this test gives no local
extremum at \(c\).
\end{enumerate}
\end{theorem}
\begin{proof}
The Mean Value Theorem makes \(f\) strictly increasing on any interval
where \(f'>0\) and strictly decreasing where \(f'<0\). The two sign-change
patterns therefore give the stated comparisons with \(f(c)\).
\end{proof}
...
\]
These quantities are finite because values on either side are bounded by
nearby values of \(f\). To see that the first is the left-hand limit, choose
...
\(\operatorname{arsinh}\). Solving \(x=\sinh y\) for \(e^y>0\) gives
\[
...
\]
Similarly, the restriction of \(\cosh\) to \([0,\infty)\) has inverse
\[
...
\end{theorem}
\begin{proof}
The assertion for \(n=2\) is the definition of convexity. Assume it for
