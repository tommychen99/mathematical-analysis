...
\noindent\textbf{Infinite limits.} Let $h:(a,b)\to\R$. We write
\[
\lim_{x\to a^+}h(x)=+\infty
\]
if, for every $M>0$, there is a $\delta>0$ such that
\[
0<x-a<\delta
\quad\Longrightarrow\quad
h(x)>M.
\]
The meanings of $h(x)\to-\infty$ and $\abs{h(x)}\to\infty$ are obtained by
replacing the final inequality respectively by $h(x)<-M$ and
$\abs{h(x)}>M$. Thus, expressions such as $\infty/\infty$ name an
...
\label{thm:lhopital}
Let $f,g:[a,b)\to\R$ be continuous on $[a,b)$ and differentiable on $(a,b)$,
where $a<b$. Assume that
\[
g'(x)\ne0
