...
Where the domain permits, define
\[
f'_+(a)=\lim_{h\to0^+}\frac{f(a+h)-f(a)}h,
\qquad
f'_-(a)=\lim_{h\to0^-}\frac{f(a+h)-f(a)}h.
\]
At an interior point, $f'(a)$ exists if and only if both one-sided
derivatives exist and are equal; their common value is then $f'(a)$.
\end{definition}

Differentiability says precisely that the function has a first-order linear
approximation:
\[
\boxed{f(a+h)=f(a)+f'(a)h+o(|h|)\qquad(h\to0,\ a+h\in D).}
\]
Indeed, after subtracting the first two terms and dividing by $|h|$, the
absolute value of the remainder quotient is
\[
\left|\frac{f(a+h)-f(a)}h-f'(a)\right|\longrightarrow0.
\]
Thus the linearization, or tangent-line approximation,
\[
L_a(x)=f(a)+f'(a)(x-a)
\]
satisfies $f(x)-L_a(x)=o(|x-a|)$ as $x\to a$ through $D$. This is the one-dimensional precursor of
$f(a+h)=f(a)+Df(a)h+o(\|h\|)$ in Chapter~11.
...
\begin{definition}[Classes \(C^k\) and \(C^\infty\) in one variable]
\label{def:one-variable-ck}
...
with endpoint values given by the one-sided derivatives of
\cref{def:one-sided-derivatives}. The notation $C^\infty$
...
\]
This establishes differentiability of
\[
1/g
\]
 before any rule is applied to
it. The product rule applied to
\[
f(1/g)
\]
 now gives the quotient formula.
\end{proof}
...
f(h)/h
\]
 equals $|h|$ on both sides, so $f'(0)=0$.
...
Suppose a differentiable function \(y=y(x)\) is already known to satisfy
\[
F(x,y(x))=0.
\]
One may differentiate this identity by the ordinary chain and product rules
and solve for \(y'(x)\) where the resulting coefficient permits it. For
example, from
\[
x^2+y(x)^2=1
\]
...
y'(x)=-\frac{x}{y(x)}
\]
where \(y(x)\ne0\). This calculation does not prove that a differentiable
branch \(y(x)\) exists. The implicit function theorem
\cref{thm:implicit-function} supplies a local existence result under
appropriate hypotheses in Chapter~\ref{ch:multivariable-differentiation}.

\begin{definition}[Local extrema and critical points]
\label{def:local-extrema-critical}
Let \(f:D\to\R\) and \(c\in D\). The point \(c\) is a \textbf{local
maximum} if some neighborhood \(U\) of \(c\) satisfies
\[
f(x)\leq f(c)\qquad(x\in D\cap U).
\]
A \textbf{local minimum} reverses this inequality, and a
\textbf{local extremum} is either one. Thus the definition is relative to
the domain, and an endpoint can be a local extremum. If \(c\) is an interior
point of \(D\), it is a \textbf{critical point} when either \(f'(c)=0\) or
\(f'(c)\) does not exist.
\end{definition}
The nondifferentiable alternative matters: \(f(x)=|x|\) has a strict local
minimum at \(0\), where its derivative does not exist.

\begin{theorem}[Fermat's theorem on stationary points]
\label{thm:fermat-stationary}
If \(f\) is differentiable at an interior local extremum \(c\), then
\[
f'(c)=0.
\]
\end{theorem}
\begin{proof}
At a local maximum, for all sufficiently nearby points,
\[
x<c\Longrightarrow \frac{f(x)-f(c)}{x-c}\geq0,
\qquad
x>c\Longrightarrow \frac{f(x)-f(c)}{x-c}\leq0.
\]
The two-sided derivative exists, so its left-hand value is nonnegative and
its right-hand value is nonpositive; equality of the two forces
$f'(c)=0$. At a local minimum the signs are reversed.
\end{proof}
...
\end{theorem}
\begin{proof}
By \cref{thm:extreme-value}, $f$ attains a maximum and a minimum. If both
values equal $f(a)$, then $f$ is constant and every interior point works.
Otherwise an extremum occurs at an interior point \(c\), and
\cref{thm:fermat-stationary} gives \(f'(c)=0\).
\end{proof}
...
Suppose $f$ is continuous at $c$ and differentiable on a punctured
neighborhood of $c$.
\begin{enumerate}
\item If some $\delta>0$ satisfies $f'(x)>0$ for
$c-\delta<x<c$ and $f'(x)<0$ for $c<x<c+\delta$, then $c$ is a strict
local maximum.
\item If the two signs are reversed on those same sides, then $c$ is a
strict local minimum.
\item If $f'$ is positive on both sides or negative on both sides, then
$c$ is not a local extremum.
\end{enumerate}
\end{theorem}
\begin{proof}
Take $x\in(c-\delta,c)$. Continuity at $c$ and differentiability on
$(x,c)$ allow the Mean Value Theorem on $[x,c]$, so
\[
f(c)-f(x)=f'(\xi)(c-x)
\]
for some $\xi\in(x,c)$. Thus the left sign determines the strict comparison
between $f(x)$ and $f(c)$. Applying the same argument on $[c,x]$ for
$x\in(c,c+\delta)$ gives the right comparison. The three conclusions now
follow from their respective sign patterns.
\end{proof}
The equation \(f'(c)=0\) alone is not sufficient. For \(f(x)=x^3\),
\(f'(0)=0\), but \(f\) is increasing on both sides and \(0\) is not an
extremum.

\begin{theorem}[Discontinuities of monotone functions]
\label{thm:monotone-discontinuities}
Let \(f:I\to\R\) be monotone on an interval. At every interior point it
has finite one-sided limits, and its set of discontinuities is at most
countable.
\end{theorem}
\begin{proof}
Suppose \(f\) is increasing and \(c\) is interior to \(I\). Define
\[
f(c^-)=\sup\{f(x):x\in I,\ x<c\},
\qquad
f(c^+)=\inf\{f(x):x\in I,\ x>c\}.
\]
Choose $u,v\in I$ with $u<c<v$. Then values to the left of $c$ are bounded
above by $f(c)$ and below by $f(u)$ once $x\geq u$; the remaining farther
left values are also at most $f(u)$. Similarly, values to the right lie
between $f(c)$ and $f(v)$ locally. Hence the displayed supremum and
infimum are finite. To see that the first is the left-hand limit, choose
\(x_0<c\) with \(f(x_0)>f(c^-)-\eps\). For \(x_0<x<c\),
\[
f(c^-)-\eps<f(x)\leq f(c^-).
\]
The right-hand argument is identical. Monotonicity also gives
\[
f(c^-)\leq f(c)\leq f(c^+).
\]
Thus \(f\) is continuous at \(c\) exactly when the two one-sided limits
agree, and every discontinuity has a nonempty jump interval
\((f(c^-),f(c^+))\).

Choose a rational number in each such interval. If \(c<d\), monotonicity
gives \(f(c^+)\leq f(d^-)\), so the two jump intervals are disjoint.
The chosen rationals are therefore distinct, giving an injection of the
discontinuity set into \(\Q\). Endpoint discontinuities, when the endpoints
belong to \(I\), add at most two points. For decreasing \(f\), apply the
increasing case to \(-f\).
\end{proof}
Chapter~\ref{ch:riemann} will use both this countability result and a direct
Darboux proof that monotone functions are Riemann integrable; the latter
does not depend on measure theory.
...
\begin{theorem}[L'H\^opital's rule: the $\displaystyle 0/0$ and $\displaystyle \infty/\infty$ forms]
\label{thm:lhopital}
...
=\frac{f'(\xi_x)}{g'(\xi_x)}.
\]
Thus this quotient differs from $L$ by less than
\[
\eps/4.
\]
 Since
...
f(x)/g(x).
\]
 Here is a proof using only finite
thresholds. Suppose first that
\[
f'/g'\to+\infty.
\]
...
0/0
\]
 form, let $K>0$. Choose $\delta>0$ so that

\[
f'(t)/g'(t)>K
\]
 for $a<t<a+\delta$. For $a<x<a+\delta$, the identity
...
\infty/\infty
\]
 form, fix a finite target $K>0$, and choose
...
positive infinite limit. If
\[
f'/g'\to-\infty,
\]
 apply the positive case
to $-f$ and $g$; it gives
\[
-f/g\to+\infty,
\]
...
of type
\[
0/0,\qquad \infty/\infty.
\]
 Forms such as $0\cdot\infty$,
$\infty-\infty$, $1^\infty$, $0^0$, and $\infty^0$ must first be transformed
into a suitable quotient or logarithmic form.

Applications involving exponential and logarithmic functions follow their
construction in \cref{sec:exponential-logarithm}; applications involving
trigonometric functions follow their construction in
\cref{sec:trigonometric-functions}. The theorem itself and
...
boundary candidates. The EVT guarantees a maximum there. The natural
candidates for an absolute extremum on a closed interval are
\[
\boxed{\text{interior critical points and endpoints}.}
\]
Here the only interior critical point satisfies $A'(x)=6-2x=0$, hence
$x=3$.
...
\eps/(2(1+\beta-\alpha))
\]
 and $N_2$ from Cauchyness
at the anchor with tolerance
\[
\eps/2.
\]
 For
...
the derivatives is below
\[
\eps/3
\]
 for indices at least $N_1$.
Choose $N_2$ so that
\[
|F_j'(x)-g(x)|<\eps/3
\]
...
|F_m'(u)-F_N'(u)|<\eps/3
\]
 for all $m\geq N,u\in I$, and

\[
|F_N'(x)-g(x)|<\eps/3.
\]
 The Mean Value Theorem for $F_m-F_N$ gives
\[
\left|\frac{F_m(x+h)-F_m(x)}h-Q_N(h)\right|<\eps/3.
\]
For each admissible $h$, let $m\to\infty$, using the already established
convergence of the functions. We obtain
\[
|Q_f(h)-Q_N(h)|\leq\eps/3
\]
...
|Q_N(h)-F_N'(x)|<\eps/3,
\]
 and shrink $\delta$ so $x+h\in I$. Then
\[
...
If each $f_n$ is differentiable on $I$,
\[
\sum_{n=0}^\infty f_n(x_0)
\]
converges at one point, and
\[
\sum_{n=0}^\infty f_n'
\]
converges uniformly to $g$, then
\[
\sum_{n=0}^\infty f_n
\]
 converges
uniformly to a differentiable sum $f$ on the
...
F_N=\sum_{n=0}^Nf_n.
\]
 The three hypotheses and the conclusions
...
q=\rho/t<1.
\]
 The positive terms $nq^{n-1}$ have consecutive
ratio
\[
q(1+1/n)\to q<1,
\]
 so the ratio test makes their series converge;
...
at most $n$ can be written uniquely as
\[
\sum_{j=0}^n c_j(x-a)^j,
\]
...
\begin{corollary}[Taylor error bound]\label{cor:ordinary-taylor-bound}
...
\qquad 0<\theta<1.
\]
Continuity of $f^{(n)}$ at $a$ makes the coefficient tend to zero, because
\[
|a+\theta h-a|=|\theta h|\leq|h|\longrightarrow0
\]
for every possible $0<\theta<1$. Divide by $|h|^n$.
\end{proof}
...
Suppose \(f'(c)=0\) and \(f''(c)\) exists. If \(f''(c)>0\), then \(c\) is
a strict local minimum; if \(f''(c)<0\), then \(c\) is a strict local
maximum.
\end{theorem}
\begin{proof}
Taylor--Peano at \(c\) gives
\[
f(c+h)=f(c)+\frac12f''(c)h^2+o(h^2).
\]
For sufficiently small nonzero \(h\), the remainder has absolute value
less than \(|f''(c)|h^2/4\). Since \(h^2>0\), the sign of
\(f(c+h)-f(c)\) is then the sign of \(f''(c)\).
\end{proof}
The case \(f''(c)=0\) is inconclusive. At zero, \(x^3\) has no local
extremum, whereas \(x^4\) has a strict local minimum.
...
\]
 has this behavior, formal differentiation
forces
\[
...
\]
 converges absolutely for every real $x$.
\end{proposition}
\begin{proof}
...
\frac{|x|}{n+1},
\]
 which tends to zero; apply the Ratio Test.
\end{proof}
...
the sum of the coefficient errors is less than
\[
\varepsilon/2
\]
 whenever
...
\qquad(n\in\N),
\]
so for any $M>0$, choose an integer $n>M$; then $x\geq n$ implies
$\exp(x)\geq\exp(n)\geq1+n>M$. Hence $\exp(x)\to\infty$ as
$x\to\infty$. For $t>0$, the addition law gives
\[
\exp(-t)=\frac{1}{\exp(t)}.
\]
Given $\eps>0$, choose $T$ with $\exp(T)>1/\eps$. Then $x<-T$ gives
$0<\exp(x)<\eps$, so $\exp(x)\to0$ as $x\to-\infty$. If $y>0$, the two
limits provide real numbers $A<B$ such that
\[
\exp(A)<y<\exp(B).
\]
...
\log=(\exp)^{-1}.
\]
The symbol \(\log\) is the primary notation in these notes. The alternative
symbol \(\ln\) is widespread and denotes exactly the same natural logarithm.
\end{definition}
...
\begin{proposition}[Arithmetic laws of the natural logarithm]
\label{prop:natural-log-laws}
For $a,b>0$,
\[
\log1=0,\qquad
\log(ab)=\log a+\log b,
...
\end{proposition}
\begin{proof}
Because $\exp0=1$ and $\log$ is the inverse of $\exp$, $\log1=0$.
Write $a=\exp(\log a)$ and $b=\exp(\log b)$. The exponential addition
law gives
\[
ab=\exp(\log a+\log b).
\]
Applying the inverse function $\log$ proves the product law. Since

\[
a/b=a\,b^{-1},
\]
 the identity
$\exp(-\log b)=b^{-1}$ and the product law give the remaining two formulas.
\end{proof}
Logarithms turn products, not sums, into sums. In general
\[
\log(a+b)\ne\log a+\log b;
\]
for instance,
...
\cref{def:rational-powers}. Indeed, for
\[
r=m/n,
\]
 \(n>0\), set
...
\(y^n=e^{m\log a}=a^m\), also for negative \(m\) by the reciprocal
identity. Uniqueness of a positive \(n\)th root therefore identifies \(y\)
with the Chapter~\ref{ch:continuity} value
\[
a^{m/n}.
\]
...
law. The inverse identity
\[
\log(a^x)=\log(e^{x\log a})=x\log a
\]
proves the displayed logarithm law, and the third exponent law follows from
it and the definition.
The identity \(\log(ab)=\log a+\log b\), obtained by applying the inverse of
\(\exp\) to its addition law, proves the fourth.
\end{proof}

\begin{theorem}[Exponential functions with arbitrary positive base]
\label{thm:arbitrary-base-exponential}
Fix $a>0$ and define
\[
E_a:\R\to(0,\infty),
\qquad E_a(x)=a^x=e^{x\log a}.
\]
Then
\[
\boxed{E_a'(x)=a^x\log a.}
\]
If $a>1$, then $E_a$ is strictly increasing; if $0<a<1$, then $E_a$ is
strictly decreasing; and if $a=1$, then $E_a(x)=1$ for every $x$.
For $a\ne1$, the function $E_a$ is a bijection from $\R$ onto
$(0,\infty)$.
\end{theorem}
\begin{proof}
\textbf{Strategy.} Differentiate the defining composite, then let the sign of
$\log a$ control monotonicity. Surjectivity will come from solving the
equation $a^x=y$ explicitly with the already constructed natural logarithm.
The chain rule gives
\[
E_a'(x)=e^{x\log a}\log a=a^x\log a.
\]
The natural logarithm is strictly increasing because it is the inverse of
the strictly increasing exponential, and $\log1=0$. Thus $\log a$ is
positive when $a>1$ and negative when $0<a<1$. Since $a^x>0$, the Mean
Value Theorem gives the stated strict monotonicity. If $a=1$, the definition
gives $1^x=e^{x\log1}=1$.

Now suppose $a\ne1$. Strict monotonicity gives injectivity. For any $y>0$,
$\log a\ne0$, and
\[
x=\frac{\log y}{\log a}
\]
satisfies
\[
a^x=e^{\frac{\log y}{\log a}\log a}
=e^{\log y}=y.
\]
Hence the actual range is all of $(0,\infty)$, proving bijectivity with the
displayed codomain.
\end{proof}

\begin{definition}[Logarithm to base $a$]
\label{def:arbitrary-base-logarithm}
Let $a>0$ and $a\ne1$. After restricting to such a base,
\cref{thm:arbitrary-base-exponential} proves that
$E_a:\R\to(0,\infty)$ is a bijection. Define its inverse
\[
\log_a:(0,\infty)\to\R.
\]
Equivalently,
...
\]
The base $a=1$ is excluded because $1^x$ is constant and therefore is not
invertible onto $(0,\infty)$. No logarithm with a nonpositive base is being
defined here.
\end{definition}

By the two inverse identities,
\[
\log_a(a^x)=x\quad(x\in\R),
\qquad
a^{\log_a x}=x\quad(x>0).
\]
Taking the natural logarithm of $a^y=x$ gives
$y\log a=\log x$. Therefore
\[
\boxed{\log_a x=\frac{\log x}{\log a}}
\qquad(a>0,\ a\ne1,\ x>0).
\]
This change-of-base formula and \cref{prop:natural-log-laws,prop:real-exponent-laws}
give, for $x,y>0$ and $r\in\R$,
\[
...
\log_a(x^r)=r\log_a x.
\]
Differentiating the change-of-base formula yields
\[
\boxed{(\log_a x)'=\frac1{x\log a}.}
\]
Thus $\log_a$ is increasing when $a>1$ and decreasing when $0<a<1$,
as is also forced by its being the inverse of the corresponding increasing
or decreasing exponential.

The notation used in these notes is
\[
\boxed{\log x:=\log_e x},
\qquad
\ln x=\log x,
...
\]
The symbol $\lg$ for the common, or decimal, logarithm is familiar but not
universal, so its meaning is fixed explicitly here. Elementary values include
\[
\log e=1,
\qquad
\lg10=1,
...
The equation $2^x=5$ has exactly one solution because $2^x$ is strictly
increasing. Applying the inverse, or using change of base, gives
\[
x=\log_2 5=\frac{\log5}{\log2}.
\]
Thus arbitrary-base logarithms recover an unknown exponent even when the
base is not \(e\).
\end{example}
...
1+1/n,,\qquad 1+2/n
\]
 already give three different limits.
...
e^x\leq\frac{10}{9}.
\]
 Thus \cref{cor:ordinary-taylor-bound} certifies
\[
...
The exponential also produces a pair of functions adapted to the hyperbola:
\[
\cosh x=\frac{e^x+e^{-x}}2,
\qquad
\sinh x=\frac{e^x-e^{-x}}2.
\]
Their definitions immediately give
\[
...
\]
Expanding the squares yields the fundamental identity
\[
\boxed{\cosh^2x-\sinh^2x=1.}
\]
Since \(\cosh x=(e^x+e^{-x})/2>0\), define
\[
\tanh x=\frac{\sinh x}{\cosh x}.
\]
The quotient rule and the boxed identity give
\[
(\tanh x)'=\frac{\cosh^2x-\sinh^2x}{\cosh^2x}
=1-\tanh^2x.
\]
Structurally,
\[
...
The strictly increasing function \(\sinh:\R\to\R\) has inverse
\(\operatorname{arsinh}\). To solve $x=\sinh y$, put $u=e^y>0$. Then
\[
x=\frac{u-u^{-1}}2
\quad\Longrightarrow\quad
u^2-2xu-1=0.
\]
The positive root is $u=x+\sqrt{x^2+1}$, and therefore
\[
...
\]
Similarly, solving $x=(u+u^{-1})/2$ gives
$u^2-2xu+1=0$; on $y\geq0$ the appropriate root is
$u=x+\sqrt{x^2-1}$. Thus the restriction of \(\cosh\) to
\([0,\infty)\) has inverse
\[
\operatorname{arcosh}x
=\log\!\left(x+\sqrt{x^2-1}\right),\qquad x\geq1,
\]
with
\[
(\operatorname{arcosh}x)'=\frac1{\sqrt{x^2-1}},\qquad x>1.
\]
The derivative formulas follow either by differentiating the logarithmic
expressions or by the inverse-derivative theorem.
...
Recall first the right-triangle ratios for an acute angle \(\theta\), as
motivation rather than definitions:
\[
\sin\theta=\frac{\text{opposite}}{\text{hypotenuse}},\qquad
\cos\theta=\frac{\text{adjacent}}{\text{hypotenuse}},\qquad
\tan\theta=\frac{\text{opposite}}{\text{adjacent}}.
\]
\Cref{fig:right-triangle-trig} restores the familiar school-level picture
before analysis begins.
\input{figures/right-triangle-trig}

The two special triangles in \cref{fig:special-triangles} similarly remind us
of familiar side ratios. We deliberately use degree notation here:
$30^\circ$, $45^\circ$, and $60^\circ$ are geometric targets, not yet
expressions involving an analytically constructed constant $\pi$.
\input{figures/special-triangles}

The desired analytic functions should reproduce those ratios and the
familiar geometric identities. Calculus also suggests the derivative pattern
\[
s'=c,\qquad c'=-s,\qquad s(0)=0,\qquad c(0)=1.
\]
These are still targets, not premises. They motivate equations that force the
power-series coefficients:
if
\[
s(x)=\sum a_nx^n,\qquad c(x)=\sum b_nx^n,
\]
 then formally
\[
...
The construction below will first define sine and cosine by those series,
then prove their derivative and addition laws, then derive the Pythagorean
identity. Only after that analytic work will we recover the global circle
picture and identify the familiar degree angles with radian values.
...
\cref{thm:power-series-differentiation}. In full for sine,
\[
\frac{d}{dx}\sum_{n=0}^\infty\frac{(-1)^nx^{2n+1}}{(2n+1)!}
=\sum_{n=0}^\infty\frac{(-1)^nx^{2n}}{(2n)!}=\cos x.
\]
For cosine, the constant term disappears and reindexing gives
\[
\cos'(x)=\sum_{n=1}^\infty\frac{(-1)^nx^{2n-1}}{(2n-1)!}
=-\sin x.
\]
\end{proof}
...
Fix $y$ and define
\[
h(x)=\sin(x+y)-\sin x\cos y-\cos x\sin y,
\]
\[
k(x)=\cos(x+y)-\cos x\cos y+\sin x\sin y.
\]
By \cref{thm:elementary-series},
\[
h'=k,
\qquad
k'=-h.
...
The Pythagorean identity now makes the promised geometric interpretation
dependency-safe. Define the algebraic locus
\[
C=\{(u,v)\in\R^2:u^2+v^2=1\}.
\]
For every real $t$,
\[
(\cos t,\sin t)\in C.
\]
In familiar coordinate geometry, $C$ is the circle of radius $1$ centered at
the origin. We use that picture here as an interpretation; Chapter~10
formally develops Euclidean distance in $\R^2$. The coordinate labels in
\cref{fig:unit-circle-trig} also reconnect this global picture with the
right-triangle ratios. The figure explains the identity; it is not part of
its proof.
\input{figures/unit-circle-trig}
...
Then
\[
\cos c=0,\quad \sin c=1,\quad \cos\pi=-1,\quad\sin\pi=0,,\qquad \cos(x+2\pi)=\cos x,\qquad\sin(x+2\pi)=\sin x.
\]
...
(-\pi/2,\pi/2),
\]
 cosine is positive, and sine is strictly
...
\[
0=\cos(\pi/2)=\cos^2x-\sin^2x.
\]
 Both functions are positive there,
so they are equal; the Pythagorean identity gives both values as

\[
\frac1{\sqrt2}.
\]
 Next the addition formulas give
\[
...
u=\frac12,
\]
 and positivity plus the Pythagorean identity gives

\[
\sin(\pi/3)=\frac{\sqrt3}{2}.
\]
 Finally,

\[
(\sin(\pi/2-t),\cos(\pi/2-t))=(\cos t,\sin t)
\]
 supplies the
...
\tan'(x)=\frac1{\cos^2x}>0.
\]
Thus tangent is strictly increasing. As
\[
x\to(\pi/2)^-,
\]
 continuity gives $\sin x\to1$ and $\cos x\to0^+$. Given $M>0$, choose $x$ sufficiently close to
\[
\pi/2
\]
 that
...
\]
then $\tan x>M$. Oddness gives the corresponding limit $-\infty$ at
\[
-\pi/2.
\]
 The Intermediate Value Theorem proves that tangent is onto $\R$.
\end{proof}

The identity $\cos^2t+\sin^2t=1$ puts the series-defined pair
$(\cos t,\sin t)$ on the algebraic locus $C$. The proved special values,
signs, and periodicity show the coordinate progression
\[
(1,0)\to(0,1)\to(-1,0)\to(0,-1)\to(1,0)
\]
as
\[
t=0,\ \frac\pi2,\ \pi,\ \frac{3\pi}{2},\ 2\pi.
\]
In the familiar orientation of the coordinate plane, this is the
counterclockwise direction. Formal coordinatewise differentiation, velocity,
and speed are postponed to \cref{sec:elementary-curves-polar}, where those
terms are defined.
...
[-\pi/2,\pi/2],
\]
 has each vertical coordinate exactly once. Similarly,
...
\end{aligned}
\]
They invert, respectively, sine on
\[
[-\pi/2,\pi/2],
\]
 cosine on $[0,\pi]$,
...
\(\sin^{-1}x\) means inverse sine, not
\[
1/\sin x.
\]
...
x\in[-\pi/2,\pi/2].
\]
 Analogous statements hold for cosine
on \([0,\pi]\) and tangent on
\[
(-\pi/2,\pi/2).
\]
 From
...
\[
y=\arcsin x\in(-\pi/2,\pi/2),
\]
 then cosine is positive there, so
\[
...
\paragraph{Derivative quotients and local equivalents.}
The base point in a derivative quotient determines which derivative is
being used. For sine and the exponential, the base point is zero:
\[
\begin{aligned}
\frac{\sin x}{x}&=\frac{\sin x-\sin0}{x-0}
 \longrightarrow(\sin)'(0)=1,\\
\frac{e^x-1}{x}&=\frac{e^x-e^0}{x-0}
 \longrightarrow(e^x)'\big|_{x=0}=1.
\end{aligned}
\]
For the logarithm put $t=1+x$. Then $t\to1$, with $t>0$ locally, and
\[
\frac{\log(1+x)}x=\frac{\log t-\log1}{t-1}
\longrightarrow(\log)'(1)=1.
\]
Continuity of cosine and the quotient limit law now give
\[
\frac{\tan x}{x}=\frac{\sin x}{x}\frac1{\cos x}\longrightarrow1.
\]
The first-order cosine limit is another derivative quotient:
\[
\frac{\cos x-\cos0}{x-0}\longrightarrow-\sin0=0,
\qquad \frac{1-\cos x}{x}\longrightarrow0.
\]
The second-order limit instead uses Taylor--Peano
(\cref{cor:ordinary-peano}):
\[
\begin{aligned}
\cos x&=1-\frac{x^2}{2}+o(x^2),\\
1-\cos x&=\frac{x^2}{2}+o(x^2),\\
\frac{1-\cos x}{x^2}&=\frac12+\frac{o(x^2)}{x^2}
 \longrightarrow\frac12.
\end{aligned}
\]
For parameters, the chain rule for the exponential and the real-power
derivative at $t=1$ give, respectively,
\[
\begin{aligned}
\lim_{x\to0}\frac{a^x-1}{x}&=\log a &&(a>0),\\
\lim_{x\to0}\frac{(1+x)^p-1}{x}
 &=\lim_{t\to1}\frac{t^p-1}{t-1}=p &&(p\in\R).
\end{aligned}
\]
In the second line $1+x>0$ locally. These are derivatives, rather than
additional limit rules to memorize. For example,
\[
\frac{\sin(3x)}x=3\frac{\sin(3x)}{3x}\longrightarrow3,
\qquad
\frac{e^{2x}-1}{\sin x}
=2\frac{e^{2x}-1}{2x}\frac{x}{\sin x}\longrightarrow2.
\]
The normalized denominator tends to one, so the quotient law applies.

\paragraph{Inverse trigonometric limits and principal ranges.}
The derivatives already established, and the zero values of arcsine and
arctangent at zero, give
\[
\lim_{x\to0}\frac{\arcsin x}{x}=1,
\qquad \lim_{x\to0}\frac{\arctan x}{x}=1.
\]
There is also an exact identity, including the endpoints:
\[
\arccos x=\frac\pi2-\arcsin x\qquad(-1\leq x\leq1).
\]
Indeed, put $y=\arcsin x$. The principal range gives
\[
-\frac\pi2\leq y\leq\frac\pi2,
\qquad 0\leq\frac\pi2-y\leq\pi,
\qquad \cos\left(\frac\pi2-y\right)=\sin y=x.
\]
Uniqueness in the principal range of arccosine proves the identity. Hence
\[
\lim_{x\to0}\frac{\arccos x-\pi/2}{x}
=-\lim_{x\to0}\frac{\arcsin x}{x}=-1.
\]
At the endpoint put $t=\arccos x$. Continuity of the inverse gives
$t\to0^+$ as $x\to1^-$, and $x=\cos t$. Thus
\[
\frac{\arccos x}{\sqrt{2(1-x)}}
=\left(2\frac{1-\cos t}{t^2}\right)^{-1/2}\longrightarrow1.
\]
The identity $\arccos(-x)=\pi-\arccos x$, proved by the same principal-range
argument, gives its symmetric companion:
\[
\lim_{x\to-1^+}\frac{\pi-\arccos x}{\sqrt{2(1+x)}}=1.
\]
This square-root behavior describes the endpoints at which
\[
(\arccos)'(x)=-\frac1{\sqrt{1-x^2}}
\]
becomes unbounded.

\paragraph{The continuous-variable exponential limit.}
For $x\ne0$ sufficiently near zero, real powers give
\[
(1+x)^{1/x}=\exp\left(\frac{\log(1+x)}x\right)\longrightarrow e
\]
by the logarithmic limit and continuity of the exponential. Taking
\[
x=\frac1n
\]
recovers the Chapter~4 sequence limit
\[
\left(1+\frac1n\right)^n\longrightarrow e.
\]
The same logarithm-then-exponential argument underlies the exponential
limit family; no new differentiation rule is needed.

\paragraph{Local asymptotic equivalents.}
Here $f(x)\sim g(x)$ means
\[
\frac{f(x)}{g(x)}\longrightarrow1.
\]
As $x\to0$, the preceding calculations give
\[
\begin{gathered}
\sin x\sim x,\qquad \tan x\sim x,\qquad e^x-1\sim x,
\qquad \log(1+x)\sim x,\\
\arcsin x\sim x,\qquad \arctan x\sim x,\\
\arccos x-\frac\pi2\sim-x,\qquad 1-\cos x\sim\frac{x^2}{2}.
\end{gathered}
\]
Scaling, substitution, reciprocals, and derivative quotients give the useful
variants
\[
\begin{gathered}
\frac{\sin(ax)}x\to a,\qquad
\frac{\tan(ax)}x\to a,\qquad
\frac{e^{ax}-1}{x}\to a,\qquad
\frac{\log(1+ax)}x\to a,\\
\frac{(1+ax)^p-1}{x}\to ap,\qquad
\frac{1-\cos(ax)}{x^2}\to\frac{a^2}{2},\\
\frac{x}{\sin x}\to1,\qquad
\frac{x}{e^x-1}\to1,\qquad
\frac{x}{\log(1+x)}\to1.
\end{gathered}
\]
Here \(a,p\in\R\), with the expressions considered where defined. The
logarithm-then-exponential argument also gives
\[
\lim_{x\to0}(1+ax)^{b/x}=e^{ab}
\]
whenever the base is positive near the limiting regime. These are
consequences of the canonical limits, not additional formulas to memorize.
...
\infty/\infty
\]
 form.  In each calculation the equal sign relates
...
(x^2+1)/x
\]
 tends to $+\infty$ as $x\to0^+$, but it is not a

\[
0/0\qquad\text{or}\qquad \infty/\infty
\]
 form, so differentiating numerator and denominator
would incorrectly suggest the limit $0$.  For

\[
x^2\sin(1/x)/x
\]
 at $0$, the derivative quotient is more complicated than
the original expression, while the estimate

\[
\abs{x\sin(1/x)}\leq\abs x
\]
 settles the limit immediately.  Finally, if

\[
f'/g'
\]
 has no limit, the theorem gives no conclusion; the original quotient
...
The pair $(\cos t,\sin t)$ suggests motion described by two ordinary
one-variable functions. This section develops that idea without importing
multivariable differentiation or a previously defined Euclidean norm. The
formal statements are coordinate formulas; familiar plane geometry supplies
interpretation and will be formalized in Chapters~10--11.
\begin{definition}[Parametrized plane curve, velocity, and speed]
\label{def:elementary-plane-curve}
A \textbf{parametrized plane curve} on an interval $I$ is a pair of ordinary
scalar functions $x,y:I\to\R$, written
\[
\gamma(t)=(x(t),y(t)).
\]
When both coordinates are differentiable, define the coordinatewise
\textbf{velocity} and the \textbf{speed} by
\[
\gamma'(t):=(x'(t),y'(t)),\qquad
\operatorname{speed}(t)=\sqrt{x'(t)^2+y'(t)^2}.
\]
The curve is $C^k$ when both coordinate functions are $C^k$, and is smooth
when both are smooth. Endpoint derivatives, when used, are one-sided.
\end{definition}

Geometrically, $\gamma(t)$ represents the position of a moving point in the
plane, $\gamma'(t)$ represents instantaneous motion, and the displayed
nonnegative number is its speed. Chapter~10 will identify the square-root
expression with Euclidean length, and Chapter~11 will show that coordinatewise
differentiation agrees with the Fr\'echet derivative of
$\gamma:I\to\R^2$. Thus ``velocity vector'' is useful familiar language,
while the present definition needs only ordered pairs and one-variable
derivatives.

For $\gamma(t)=(\cos t,\sin t)$, coordinatewise differentiation gives
\[
\gamma'(t)=(-\sin t,\cos t),
\qquad
\operatorname{speed}(t)=1.
\]
The coordinate progression proved in the preceding section gives the
familiar geometric interpretation that, on $0\leq t\leq2\pi$, the curve
starts at $(1,0)$ and moves once counterclockwise around its algebraic locus;
on the same interval, $(\cos2t,\sin2t)$ has speed two and appears to make two
traversals. The exact multiplicities follow from angular nonuniqueness in
\cref{prop:polar-angle-uniqueness}. A parametrization therefore records speed
and multiplicity as well as its geometric image.

\paragraph{Standard parametrizations.}
For a graph over $[a,b]$, use $\gamma(t)=(t,f(t))$, $a\leq t\leq b$;
it moves from $(a,f(a))$ to $(b,f(b))$ in increasing horizontal coordinate.
For two points $(x_0,y_0)$ and $(x_1,y_1)$, first define the set
\[
S=\left\{
\bigl((1-t)x_0+tx_1,(1-t)y_0+ty_1\bigr):0\leq t\leq1
\right\}.
\]
The displayed formula is the formal definition of $S$; familiar coordinate
geometry calls it the line segment from $(x_0,y_0)$ to $(x_1,y_1)$.
Its displayed parameter runs from the first endpoint to the second.

For $R>0$, let $\gamma(t)=(R\cos t,R\sin t)$. Its coordinates satisfy
\[
x(t)^2+y(t)^2=R^2(\cos^2t+\sin^2t)=R^2.
\]
Elementary coordinate geometry recognizes this locus as the circle of radius
$R$. Coordinatewise differentiation gives velocity
$(-R\sin t,R\cos t)$ and speed $R$. Likewise, for $a,b>0$,
$\gamma(t)=(a\cos t,b\sin t)$ satisfies
\[
\frac{x(t)^2}{a^2}+\frac{y(t)^2}{b^2}=1,
\]
the familiar ellipse equation. On $[0,2\pi]$ the coordinate progression
\[
(1,0)\to(0,1)\to(-1,0)\to(0,-1)\to(1,0)
\]
at
...
\]
shows that both parametrizations move
counterclockwise in the familiar orientation.

The hyperbolic identity gives the parallel parametrization
\[
...
\]
Because \(\cosh t>0\), this parametrizes the right branch of the hyperbola;
the left branch is obtained from \(x=-a\cosh t\). Thus
\[
\cos^2t+\sin^2t=1
\qquad\text{and}\qquad
\cosh^2t-\sinh^2t=1
\]
govern, respectively, circle/ellipse geometry and hyperbola geometry. This
comparison motivates the term \emph{hyperbolic functions}.

\begin{definition}[Polar coordinates]\label{def:polar-coordinates}
A \textbf{polar representation} of an ordered pair $(x,y)$ is a pair
$(r,\theta)$ satisfying
\[
\boxed{x=r\cos\theta,\qquad y=r\sin\theta,\qquad r\geq0.}
\]
...
Geometrically, $r$ is the distance from the origin and $\theta$ is a
direction angle. Chapter~10 will formalize the distance interpretation
through the Euclidean metric. In the same familiar interpretation, fixed
$r>0$ gives a circle and fixed $\theta$ with varying $r\geq0$ gives a ray;
neither picture is a prerequisite for the algebraic definition.

\begin{proposition}[Existence of polar representations]
\label{prop:polar-existence}
Every ordered pair $(x,y)$ has a polar representation. If $(x,y)\ne(0,0)$,
one can choose $r>0$ and $0\leq\theta<2\pi$.
\end{proposition}
\begin{proof}
The origin is represented by $r=0$ and any angle. Suppose
$(x,y)\ne(0,0)$ and set
\[
r=\sqrt{x^2+y^2}>0,
\qquad
...
\end{cases}
\]
Because $\cos\alpha=u$ and $\sin\alpha\geq0$, the Pythagorean identity
gives $\sin\alpha=\sqrt{1-u^2}=|v|$. If $v\geq0$, this is $v$. If
$v<0$, the addition formulas and periodicity give
\[
\cos(2\pi-\alpha)=\cos\alpha=u,
\qquad
\sin(2\pi-\alpha)=-\sin\alpha=v.
\]
Thus $x=r\cos\theta$ and $y=r\sin\theta$. In the second case $v<0$
rules out $\alpha=0,\pi$, so $0<2\pi-\alpha<2\pi$; the first case also
has $0\leq\theta\leq\pi$. Hence the stated angular range is valid.
\end{proof}

\begin{proposition}[Exact angular nonuniqueness]
\label{prop:polar-angle-uniqueness}
If $r>0$, then
\[
...
\theta-\phi\in2\pi\Z.
\]
At $r=0$, every angle represents the origin.
\end{proposition}
\begin{proof}
When $r>0$, equality of the ordered pairs gives equality of the two cosines
and of the two sines. The addition formulas then yield
\[
\cos(\theta-\phi)
=\cos\theta\cos\phi+\sin\theta\sin\phi
=\cos^2\phi+\sin^2\phi=1.
\]
Put \(s=\theta-\phi\). If \(\cos s=1\), the Pythagorean identity gives
\(\sin s=0\). The zero-set result in
\cref{prop:trigonometric-special-values} then gives \(s=k\pi\) for some
\(k\in\Z\). Since
\[
\cos(k\pi)=(-1)^k=1,
\]
the integer \(k\) is even, so \(s\in2\pi\Z\). Conversely,
\(2\pi\)-periodicity gives equality whenever the angles differ by a multiple
of \(2\pi\). For \(r=0\), both coordinates vanish for every angle.
\end{proof}

Thus polar coordinates are not globally one-to-one. The quadrant information
cannot be discarded.
For example $(-1,1)$ has $r=\sqrt2$ and
\[
\theta=3\pi/4,
\]
 since
...
\[
\cos(3\pi/4)=-1/\sqrt2,\qquad \sin(3\pi/4)=1/\sqrt2.
\]

The value
\[
\arctan(y/x)=\arctan(-1)=-\pi/4
\]
 would select the wrong
quadrant. For $r>0$, the chosen angle must satisfy \emph{both}

\[
\cos\theta=x/r,\qquad \sin\theta=y/r.
\]
...
origin through one turn, ending at $(2\pi,0)$; ``spiral'' is the familiar
geometric interpretation of this coordinate formula.
...
\end{proof}
Informally, $r'$ measures radial motion toward or away from the origin,
while $r\theta'$ measures motion around the origin. Chapters~10--11 will
supply the formal Euclidean vector interpretation. For a circle $r=R$,
the scalar speed formula gives $R|\theta'|$; for $\theta=t$ it gives $R$.
The derivation used only one-variable differentiation and trigonometric
identities: no Jacobian, norm, inner product, or multivariable derivative was
invoked.

\begin{remark}[Polar coordinates can fail at the origin]
The smooth coordinatewise curve $\gamma(t)=(t,0)$ passes through the origin.
A nonnegative polar radius must satisfy $r(t)^2=t^2$, hence
$r(t)=|t|$, which is not differentiable at $0$; no unique angle is
determined there. Thus a smooth coordinatewise curve
...
are smooth and that the coefficients at $a$ must be
\[
f^{(j)}(a)/j!.
\]
...
It does not assert
\[
f(a+h)=\sum_{j=0}^\infty\frac{f^{(j)}(a)}{j!}h^j.
\]
...
The function
\[
\theta(t)=\begin{cases}e^{-1/t},&t>0,\\0,&t\leq0\end{cases}
\]
is $C^\infty$ on $\R$, with $\theta^{(k)}(0)=0$ for every $k\geq0$.
\end{theorem}
\begin{proof}
Each differentiation creates powers of
\[
1/t,
\]
 but exponential
...
\]
Each is a polynomial in
\[
1/t
\]
 times the same decaying exponential.
On $t>0$, induction gives
\[
\theta^{(k)}(t)=P_k(1/t)e^{-1/t},
\]
...
g_k(t)/t\to0
\]
 as $t\to0^+$ by the same estimate with one extra power,
...
b(x)=\theta(1-\|x\|_2^2)
\]
will be a smooth function that vanishes outside the closed unit ball once the
...
Show that
\[
\theta(t)/(\theta(t)+\theta(1-t))
\]
 is smooth, equals zero for
...
\section{Convexity, Concavity, and Inequalities}
...
of the two smaller chord slopes, with weights
\[
(z-x)/(y-x),\qquad (y-z)/(y-x).
\]
 Convexity orders the smaller slopes; their weighted
...
Since
\[
(y-z)/(y-x)=t,\qquad (z-x)/(y-x)=1-t,
\]
 this is the chord inequality.
...
\label{def:concavity-inflection}
A function \(f\) is \textbf{concave} on an interval when \(-f\) is convex.
An interior point \(c\) is an \textbf{inflection point} when the function
changes concavity at \(c\).
\end{definition}
Applying \cref{thm:convex-derivative-criteria} to \(-f\) shows that a
differentiable function is concave exactly when \(f'\) is nonincreasing,
and \(f''\leq0\) implies concavity. A sign change of \(f''\) across \(c\)
is a useful sufficient condition for an inflection point when the relevant
derivatives exist. The equation \(f''(c)=0\) alone is not sufficient:
\(x^3\) changes concavity at zero, while \(x^4\) does not.

\begin{theorem}[Finite Jensen inequality]
\label{thm:finite-jensen}
Let \(f\) be convex on an interval \(I\). If \(x_i\in I\),
\(\lambda_i\geq0\), and
\[
...
\end{theorem}
\begin{proof}
For $n=1$ the assertion is equality. The assertion for $n=2$ is the
definition of convexity. Assume it for
\(n-1\). If \(\lambda_n=1\), the result is equality. Otherwise put
\[
\Lambda=1-\lambda_n,\qquad
y=\sum_{i=1}^{n-1}\frac{\lambda_i}{\Lambda}x_i.
\]
The induction hypothesis gives
\[
f(y)\leq\sum_{i=1}^{n-1}\frac{\lambda_i}{\Lambda}f(x_i).
\]
Apply two-point convexity to
\(\Lambda y+\lambda_nx_n\), then substitute this bound. The concave form
follows by applying the result to \(-f\).
\end{proof}

\begin{corollary}[Weighted arithmetic--geometric mean inequality]
\label{cor:weighted-amgm}
If \(x_i>0\), \(\lambda_i\geq0\), and
\(\sum_i\lambda_i=1\), then
\[
\boxed{\prod_{i=1}^n x_i^{\lambda_i}
\leq\sum_{i=1}^n\lambda_i x_i.}
\]
If every weight is positive, equality holds exactly when all \(x_i\) are
equal; zero-weight terms are irrelevant. The conclusion extends to
nonnegative \(x_i\) by treating a positive-weight zero factor separately.
\end{corollary}
\begin{proof}
The logarithm is strictly concave because
\[
(\log x)''=-1/x^2<0.
\]
The concave form of Jensen gives
\[
\log\!\left(\sum_i\lambda_i x_i\right)
\geq\sum_i\lambda_i\log x_i
=\log\!\left(\prod_i x_i^{\lambda_i}\right).
\]
Apply the increasing exponential. Strict concavity gives the equality
condition.
\end{proof}
For equal weights this becomes
\[
\sqrt[n]{x_1\cdots x_n}\leq\frac{x_1+\cdots+x_n}{n}.
\]
Appendix~\ref{app:foundations} gives an independent proof of this equal-weight
case by Cauchy forward--backward induction, starting from the elementary
two-variable result \cref{prop:two-variable-amgm}.

\begin{corollary}[Young's inequality]
\label{cor:young-inequality}
If \(p,q>1\) and \(1/p+1/q=1\), then for \(a,b\geq0\),
\[
\boxed{ab\leq\frac{a^p}{p}+\frac{b^q}{q}.}
\]
\end{corollary}
\begin{proof}
Apply weighted AM--GM to \(a^p,b^q\) with weights \(1/p,1/q\).
\end{proof}

\begin{theorem}[Finite H\"older inequality]
\label{thm:finite-holder}
For conjugate exponents \(p,q>1\) and real numbers \(a_i,b_i\),
\[
\boxed{
\sum_{i=1}^n|a_ib_i|
\leq
...
Put
\[
A=\left(\sum_i|a_i|^p\right)^{1/p},
\qquad
B=\left(\sum_i|b_i|^q\right)^{1/q}.
\]
If \(A=0\) or \(B=0\), every term on the corresponding side is zero.
Otherwise apply Young's inequality \cref{cor:young-inequality} to
\[
\frac{|a_i|}{A},\qquad\frac{|b_i|}{B},
\]
and sum:
\[
\sum_i\frac{|a_ib_i|}{AB}
\leq\frac1p\sum_i\frac{|a_i|^p}{A^p}
 \frac1q\sum_i\frac{|b_i|^q}{B^q}
=\frac1p+\frac1q=1.
\]
Multiplication by \(AB\) proves the result.
\end{proof}

For \(p=q=2\), H\"older first gives
\[
\sum_{i=1}^n|a_ib_i|
\leq
...
Together with
\[
\left|\sum_i a_ib_i\right|\leq\sum_i|a_ib_i|,
\]
this is the familiar finite scalar Cauchy--Schwarz inequality:
\[
\boxed{
...
\left(\sum_i b_i^2\right)^{1/2}.}
\]
Here it appears as the \(p=2\) case of H\"older. Chapter~\ref{ch:topology-linear-algebra}
retains an independent proof and develops its geometric interpretation
...
Prove from the difference quotient that
\[
(1/x)^{\prime}=-1/x^2
\]
...
\begin{exercise}
Differentiate $(x^2+1)e^x$, $\sin(x^2)$, $\log(1+x^2)$, and
\[
(x+1)/(x^2+1).
\]
 State domains and identify the rule used at each step.
\end{exercise}
...
\[
\log(1+2x)/x,,\qquad (1-\cos(2x))/x^2
\]
 as $x\to0$, using the established elementary limits.
\end{exercise}
...
\begin{exercise}
Use Taylor\textquotesingle s theorem to approximate $e^{0.1}$ by
\[
1+0.1+0.1^2/2
\]
 and prove an error bound using $e^t\leq e$ on $[0,0.1]$.
\end{exercise}
...
F_n(x)=x+x^2/n
\]
 on $[-1,1]$, verify the sequence differentiation
