...
\[
E_a'(x)=\exp(x\log a)\log a=a^x\log a.
\]
...
satisfies
\[
a^x=\exp\!\left(\frac{\log y}{\log a}\log a\right)
=\exp(\log y)=y.
\]
...
\]
This is the practical role of arbitrary-base logarithms: they recover an
unknown exponent even when the base is not $e$.
\end{example}
...
\]
The coordinate progression proved in the preceding section shows that on
$0\leq t\leq2\pi$ the curve starts at $(1,0)$ and, in the familiar
orientation of the plane, makes one counterclockwise traversal. On the same
interval, $(\cos2t,\sin2t)$ has speed two and traverses the same algebraic
locus twice. A parametrization therefore records speed and multiplicity as
...
(1,0)\to(0,1)\to(-1,0)\to(0,-1)\to(1,0)
\]
at $t=0,\pi/2,\pi,3\pi/2,2\pi$ shows that both parametrizations move
...
\]
The zero and special-value results in
\cref{prop:trigonometric-special-values} imply that $\cos s=1$ exactly for
$s\in2\pi\Z$. Hence $\theta-\phi\in2\pi\Z$. Conversely, $2\pi$-periodicity
gives equality whenever the angles differ by such a multiple. For $r=0$,
both coordinates vanish for every angle.
\end{proof}
