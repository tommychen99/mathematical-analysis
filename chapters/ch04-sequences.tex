...
The proof starts instead by choosing a natural number
\[
N>1/\eps
\]
 and
then checks
\[
n\geq N\Rightarrow \frac1n\leq\frac1N<\eps.
\]
...
\eps/2
\]
 to each summand. Different allocations are possible; each
...
x=1/7,
\]
 integer division successively gives
...
d_n\leq1/7<d_n+10^{-n}.
\]
 The shrinking error, rather than the
repeating pattern, gives
\[
d_n\to1/7
\]
...
\[
s_n=\sum_{j=1}^n d_j10^{-j}
\]
converge to a real number in $[0,1]$,
...
\end{theorem}
\begin{proof}
Suppose $a_n\to L$ and $a_n\to M$. Given $\eps>0$, choose $N_1,N_2$ such that
\[
\abs{a_n-L}<\eps/2
\]
 for $n\geq N_1$ and
...
\abs{a_n-M}<\eps/2
\]
 for $n\geq N_2$. For $n\geq\max\{N_1,N_2\}$, the triangle inequality gives
\[
...
a_n=1/n
\]
 converges to $0$. For a given $\eps>0$, \cref{thm:archimedean} provides
\[
N>1/\eps,
\]
 and then $n\geq N$ implies
\[
...
Then
\[
|b_n|\geq|b|-|b_n-b|>|b|/2>0,
\]
 so the quotient is defined.
Choose $N_1$ for
\[
|a_n-a|<\eps|b|/4
\]
...
\eps=(a-b)/3.
\]
 Choose $N_1,N_2$ for the respective errors
...
For fixed $n\in\N$, the estimate
\[
0<\frac1{k^n}\leq\frac1k
\]
 holds
for $k\geq1$. Given $\eps>0$, choose
\[
N>\max\{1,1/\eps\};
\]
 every
...
f(k)/g(k)=k^{n-m}H_k.
\]
 For $n<m$, write this as

\[
H_k/k^{m-n}
\]
 and use limit arithmetic; for $n=m$ it is $H_k$.
For $n>m$ and $c>0$, choose $N_1$ such that
\[
|H_k-c|<c/2
\]
 for
...
\]
Every $k\geq N$ satisfies
\[
k^{n-m}H_k>(c/2)k>M.
\]

For $c<0$, apply this positive argument to
\[
-f(k)/g(k),
\]
 using
...
\[
(3k^2-k)/(2k^2+5)\to3/2,,\qquad (-k^3+2)/(k+1)\to-\infty.
\]
 The leading coefficients decide the
...
and retained after some step. For error below
\[
1/10
\]
 gram, $N=7$
...
a_n=1+1/n,
\]
 the candidate $L=0$ fails: take $\eps_0=1$ and
...
The order completeness of the real numbers now becomes a working tool.
...
\end{definition}
We write $a_n\uparrow L$ when $(a_n)$ is increasing and $a_n\to L$, and
$a_n\downarrow L$ when $(a_n)$ is decreasing and $a_n\to L$. Similarly,
$a_n\uparrow+\infty$ and $a_n\downarrow-\infty$ record monotone divergence
to the indicated infinite limit.
...
Then $B=\max\{|L|+1,|a_1|,\ldots,|a_{N_0}|\}$ bounds every $|a_n|$. Conversely, let $s=\sup\set{a_n:n\in\N}$. Given $\eps>0$, $s-\eps$ is not an upper bound, so $a_N>s-\eps$ for some $N$. For $n\geq N$,
\[
s-\eps<a_N\leq a_n\leq s,
\]
 and hence $\abs{a_n-s}<\eps$. The decreasing statement follows on applying the increasing statement to $(-a_n)$.
\end{proof}

The theorem immediately constructs one of the central constants of analysis.
In elementary algebra the expression
\[
(1+1/n)^n
\]
 appears in repeated
...
a_n=(1+1/n)^n,
\]
 so $a_n\to e$. Three altered expressions reduce
...
to prove the broader principle
\[
(1+x/n)^n\to e^x
\]
...
\end{theorem}
\begin{proof}
If $a_n\to L$, choose $N$ with
\[
\abs{a_n-L}<\eps/2
\]
 for $n\geq N$ and, for all $m,n\geq N$, write
...
|a_k-L|<\eps/2
\]
 for $k\geq N$. For every $n\geq N$, all terms in
the $n$th tail lie between
\[
L-\eps/2,\qquad L+\eps/2.
\]
 Thus

\[
|i_n-L|\leq\eps/2<\eps,\qquad |s_n-L|\leq\eps/2<\eps.
\]
...
a_n=(-1)^n+1/n,
\]
 the even subsequence tends to $1$ and the odd
...
b_n=2+(-1)^n/n,
\]
 both tail barriers tend to $2$, so the sequence
...
|s_{N_j}-s|<1/j,
\]
 where $k_0=0$. Then the definition of
supremum supplies $k_j\geq N_j$ with

\[
s_{N_j}-1/j<a_{k_j}\leq s_{N_j}.
\]
 Thus $k_j>k_{j-1}$ and
...
|a_{k_j}-s|<2/j,
\]
 proving convergence. For the lower barrier $i$, first
choose $N_j>k_{j-1}$ with
\[
|i_{N_j}-i|<1/j,
\]
 then choose $k_j\geq N_j$
with
\[
i_{N_j}\leq a_{k_j}<i_{N_j}+1/j.
\]
...
\item Whenever
\[
[a_1,b_1]\supseteq[a_2,b_2]\supseteq\cdots,\qquad \lim_{n\to\infty}(b_n-a_n)=0,
\]
...
Prove directly that
\[
3/(n+2)\to0,\qquad (2n+1)/(n+1)\to2.
\]
 For each, show the backward discovery of $N$ separately from the forward proof.
\end{hexercise}
...
\[
((k+1)/(k^3+2)),,\qquad ((-2k^4+k)/(k^2+1))
\]
 as $k\to\infty$. Identify the degrees and leading coefficients and justify that denominators are nonzero on a tail.
\end{hexercise}

\begin{hexercise}{seq-geometric}
Give explicit thresholds proving
\[
(3/4)^n\to0,\qquad (3/2)^n\to+\infty.
\]
 Determine whether
\[
(-3/2)^n
\]
 has either infinite limit.
\end{hexercise}
...
An approximation begins with error at most $6$ units. Each correction reduces the error magnitude to at most one third of its preceding magnitude. Formulate a sequence inequality, prove convergence of the errors to zero, and find a number of corrections guaranteeing error below
\[
1/100
\]
 unit.
...
Disprove the proposed limit $1$ for
\[
a_n=2+1/n
\]
 using the negated definition. Then prove that $2+(-1)^n$ has no limit. Explain why these are different tasks.
\end{hexercise}

\begin{exercise}
Find the limsup and liminf of
\[
(-1)^n+2/n
\]
 and exhibit subsequences attaining them. Prove from the tail bounds that no other subsequential limit is possible.
\end{exercise}

\paragraph{A common strategy for monotone recursive sequences.}
First find a bound preserved by the recurrence, and prove it by induction.
Then establish monotonicity and apply monotone convergence. Only after
convergence is known should one pass to the recurrence, using the limit
laws already established, to identify the limit. Later, continuity will
often package this last step conveniently.
...
a_{n+1}=(a_n+2)/3.
\]
 Prove monotonicity and boundedness, deduce convergence, and determine the limit. Which step uses completeness?
\end{exercise}
